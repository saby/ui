...
export function goUpByControlTreeInner(
    ...
    if (!withFocusCallbacksObjects) {
        addControlsToFlatArray(closestControl, controlTree);
    ...
    addControlsToFlatArray(closestControl, flatArray);
...
function addControlsToFlatArray(control: TControlOrCompat, array: TControl[]): void {
    ...
    const parent = getFocusParent(control, array);

    checkOpener(parent);

    if (parent) {
        addControlsToFlatArray(parent, array);
    }
}

function getFocusParent(control: TControlOrCompat, array: TControl[]): TControlOrCompat | null {
    ...
    let result =
        ...
        options?.parent ||
        control?.getParent?.();
    // Gоддержим дальшейший подъём по дереву через чистый реакт компонент, если у него есть DOM элемент в поле _container.
