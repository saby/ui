...
export function getClosestControlInner(target: IWrapHTMLElement): TControl | undefined {
    if (!isBrowserPlatform) {
        return;
    }
    let element: IWrapHTMLElement | undefined = target?.jquery ? target[0] : target;
    while (element && element !== document.documentElement && element !== document.body) {
        ...
        element = element.parentNode as IWrapHTMLElement;
    }
}
