...
function isolateScope(scope: any, data: any, propertyName: any): any {
    if (!scope[PATCHED_FLAG_PREFIX + propertyName]) {
        ...
        if (parentValueProperty !== undefined) {
            // игнорирум ChildrenAsContent, чтобы при его использовании не ломать scope="{{_options}}"
            if (propertyName === 'content' && parentValueProperty?.isChildrenAsContent) {
                if (data.content) {
                    if (isObject(data)) {
