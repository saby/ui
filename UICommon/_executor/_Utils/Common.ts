...
var requireIfDefined = function requireIfDefined(tpl) {
     ...
     * @returns {*}
     */
    checkExistingModule = function checkExistingModule(
        tpl,
        includedTemplates,
        _deps
    ) {
    ...
    moduleNameCheckProceed = function maxNameLengthCheck(
        tpl,
        includedTemplates,
        _deps
    ) {
...
export function escape(entity) {
    if (isString(entity)) {
        entity = entity.replace(
            ampRegExp,
            function escapeReplace(match, offset, str) {
                if (str[offset + 1] === '#') {
                    return match;
                }
                return '&amp;';
...
export function unescape(str: any): any {
    ...
    return unescapeASCII(str).replace(
        unescapeRegExp,
        (_, entity: string) => unescapeDict[entity]
    );
}
...
export function escapeParenthesis(entity) {
    if (isString(entity)) {
        return entity.replace(
            regExpToParenthesisReplace,
            function escapeReplace(tag) {
...
export function plainMergeAttr(inner, object) {
    ...
    const res = {
        ...
        domNodeProps: {},
        key: controlKey,
        attributes: Attr.processMergeAttributes(
            inner.attributes,
            object.attributes
        ),
...
export function isOptionalString<T = unknown>(tplOrigin: T) {
    return (
        typeof tplOrigin === 'string' && tplOrigin.indexOf('optional!') === 0
    );
}
...
export function isControlClass<T = IControl>(
    controlClass: any
): controlClass is T {
...
export function isTemplateClass<K = TemplateFunction>(
    controlClass: any
): controlClass is K {
    const prototype = controlClass && controlClass.prototype;
    if (prototype && typeof prototype !== 'undefined') {
...
export function isTemplateArray<K = TemplateFunction>(
    array: any
): array is ITemplateArray<K> {
    return (
        Array.isArray(array) &&
...
export function isControl(control) {
    return (
        control && control.constructor && isControlClass(control.constructor)
    );
}
...
export function splitModule(string: string): IGeneratorNameObject {
    var fullName =
            string.indexOf('ws:') === 0 ? string.replace('ws:', '') : string,
...
export function isTplFunction(
    tpl: unknown
): tpl is ITplFunction<TemplateFunction> {
 ...
 */
export type Deps<T = IControl, K = TemplateFunction> = Record<
    string,
...
export function isDefaultExport(obj: unknown): boolean {
    if (obj && typeof obj === 'object') {
        return (
            obj.hasOwnProperty('__esModule') && obj.hasOwnProperty('default')
        );
    }
    return false;
}
...
export function disableCompat() {
    let disableCompat =
        (process && process.domain && process.domain.req.disableCompat) ||
        disableCompatCache;
