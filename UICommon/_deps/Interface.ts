...
export type TCollectedDepsRawKeys = keyof ICollectedDepsRaw;
...
export interface IModuleDependenciesRaw<TKey extends string | number = number, TLinks = number> {
    nodes: Record<TKey, INodeDescription>;
    links: Record<TKey, TLinks[]>;
    nodesList: string[];
}

export interface IModuleDependencies {
    nodes: Record<string, INodeDescription>;
    links: Record<string, string[]>;
}

export interface IModulesDeps extends IModuleDependencies {
...
export interface IOptionalBundles {
...
export const TYPES: Record<RequireJSPlugin | 'css', TTypesValue> = {
    tmpl: {
        type: 'tmpl',
        plugin: 'tmpl',
        hasDeps: true,
