...
import { ICollectedDepsRaw, IDeps, ICollectedFiles, ILocalizationResources } from './Interface';
...
export class DepsCollector {
     ...
     * @param features - список модулей, у которых  есть модули-фичи
     */
    constructor(
        private modDeps: Record<string, IDeps>,
        private modInfo: Record<string, unknown>,
        private bundlesRoute: Record<string, string>,
        private optionalBundles: Record<string, string> | undefined,
        private features: Record<string, string> = {}
    ) {}

    collectDependencies(depends: IDeps = [], unpack: IDeps = []): ICollectedFiles {
        const deps: string[] = ['RequireJsLoader/autoload'];
        depends
        ...
        collectDependencies(allDeps, deps, this.modDeps, this.modInfo, this.features);
    ...
    collectI18n(files: ICollectedFiles, deps: ICollectedDepsRaw): void {
        ...
        const regionCode = controller.currentCountry;
        ...
        files.js.push(controller.loadingsHistory.languages[langCode]);
        files.js.push(controller.loadingsHistory.regions[regionCode]);

        const i18nDeps = deps.i18n;
        if (!i18nDeps) {
            return;
        }

        for (const moduleModule of Object.keys(i18nDeps)) {
            const UIModuleName = i18nDeps[moduleModule].moduleName.split('/')[0];
            ...
            if (loadedResources.hasOwnProperty(langCode)) {
                this.addLocalizationResource(currentBundlesRoute, files, loadedResources[langCode]);
            }
        }
    }
...
/**
 * Получить список пакетов, которые не будут вставлены в теле страницы в script и link.
 * Модули из этих пакетов будут вставлены как отдельные модули.
 */
function getExtractBundlesList(): string[] {
    const extractBundles = cookie.get('extractBundles');
    if (!extractBundles) {
        return [];
    }
    return extractBundles.split(',');
}
