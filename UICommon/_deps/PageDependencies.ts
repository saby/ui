...
export class PageDependencies {
     ...
     *                          статичных страниц. Этот аргумент используется при генерации статичных страниц *html.tmpl
     */
    private _collectDeps(
        initDeps: IDeps = [],
        staticPageBundles?: IModulesDeps
    ): ICollectedFiles {
        if (isDebug()) {
            return getDebugDeps();
        }
...
function initDefaultPageDependencies() {
    const { links, nodes, bundles, optionalBundles } = getModulesDependencies();
...
function getSerializedData(): ISerializedData {
    ...
    logger.info(
        `state serialization completed in ${Date.now() - startSerialization} ms`
    );
