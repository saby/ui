...
function isSpecialDepsKey(key: string): key is keyof typeof SPECIAL_DEPS {
    return SPECIAL_DEPS.hasOwnProperty(key);
}
...
export function collectDependencies(
    ...
    for (let i = 0; i < curNodeDeps.length; i++) {
        ...
        const moduleType = module.typeInfo.type as TCollectedDepsRawKeys;
        let allDepsOfSameType = allDeps[moduleType];
        if (!allDepsOfSameType) {
            allDepsOfSameType = {};
            allDeps[moduleType] = allDepsOfSameType;
        }
        if (!allDepsOfSameType[node]) {
            if (!(skipDep && !!module.typeInfo.canBePackedInParent)) {
                allDepsOfSameType[module.fullName] = module;
            }
...
export function getAllPackagesNames(
    ...
    extractBundles: string[]
): IDepPackages {
    const packs = getEmptyPackages();
    const isUnpackModule = getIsUnpackModule(unpack);
    mergePacks(packs, getPacksNames(all.js, isUnpackModule, bRoute, extractBundles));
    mergePacks(packs, getPacksNames(all.tmpl, isUnpackModule, bRoute, extractBundles));
    mergePacks(packs, getPacksNames(all.wml, isUnpackModule, bRoute, extractBundles));

    packs.css = getCssPackages(all.css, isUnpackModule, bRoute, extractBundles);
    return packs;
}
...
function getEmptyPackages(): IDepPackages {
    const packages = {} as IDepPackages;
    const typesKeys = Object.keys(TYPES) as RequireJSPlugin[];
...
function getPacksNames(
    allDeps: Record<string, IModuleInfo> = {},
    ...
    for (const moduleName of Object.keys(allDeps)) {
        ...
        if (!bundleName && isSpecialDepsKey(moduleName)) {
            bundleName = bundlesRoute[SPECIAL_DEPS[moduleName]];
        }
        if (!bundleName || skipBundleName(bundleName, extractBundles)) {
            continue;
        }
        delete allDeps[moduleName];
        const ext = getExt(bundleName) as RequireJSPlugin;
    ...
    for (const moduleName of Object.keys(allDeps)) {
        ...
        //const ext = type as RequireJSPlugin;
        const packageName = plugin ? (moduleName.split(plugin + '!').pop() as string) : moduleName;
        if (unpackBundles.indexOf(packageName) !== -1) {
            continue;
        }
...
function getIsUnpackModule(unpack: IDeps): (moduleName: string) => boolean {
    ...
    const isWsCore = (unpackModuleName: string, dependModuleName: string): boolean => {
...
function mergePacks(result: IDepPackages, addedPackages: Partial<IDepPackages>): void {
    const addedPackagesKeys = Object.keys(addedPackages) as RequireJSPlugin[];
    for (const pack of addedPackagesKeys) {
        if (result[pack] === undefined) {
            result[pack] = {};
        }
        const addedPackage = addedPackages[pack];
        if (!addedPackage) {
            continue;
        }
        for (const key in addedPackage) {
            if (addedPackage.hasOwnProperty(key)) {
                result[pack][key] = addedPackage[key];
            }
        }
    }
}
...
function skipBundleName(bundleName: string, extractBundles: string[]): boolean {
    if (extractBundles.length === 0) {
        return false;
    }

    for (const name of extractBundles) {
        if (bundleName.indexOf(name) > -1) {
            return true;
        }
    }
    return false;
