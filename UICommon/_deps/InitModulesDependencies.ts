...
export class ModulesDependencies {
        ...
        for (const name of Object.keys(this.modules)) {
            ...
            if (this.modules[name].features) {
                ...
                for (const mModulePath in mFeatures) {
                    ...
                    // если указан полый путь фичи, напр. "MyModule/FeatureName": "MyFeature/FeatureName"
                    if (mFeatures[mModulePath].indexOf('/') >= 0) {
                        features[mModulePath] = mFeatures[mModulePath];
                        continue;
                    }

                    // если указан только модуль фичи, напр. "MyModule/FeatureName": "MyFeature"
        ...
        if (optBundlesModuleNames.length) {
            ...
            optBundles = optBundlesModuleNames
                ...
                .map(({ bundles }: IOptionalBundles) => {
                    const newOptBundle: Record<string, string> = {};
     ...
     */
    private requireModuleDeps(moduleName: string): IModulesDeps {
