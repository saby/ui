...
import { IFocusElementProps, TPropsGetter, IFocusHTMLElement } from './IFocus';
import { canAcceptFocusControlOnElement } from './FocusCompatible';

import { isElementVisible } from 'UICommon/Utils';

const CANDIDATE_SELECTOR = ['a[href]', 'link', 'button', 'input', 'select', 'textarea'];
...
function getStyle(element: HTMLElement, style: keyof CSSStyleDeclaration) {
...
export function getElementProps(element: IFocusHTMLElement): IFocusElementProps {
    const classList = element.classList;
    const isHidden: boolean =
        getStyle(element, 'display') === 'none' || getStyle(element, 'visibility') === 'hidden';
    ...
    const tabCycling: boolean =
        element['ws-tab-cycling'] === 'true' || element.getAttribute('ws-tab-cycling') === 'true';

    const tabIndexAttr = element.getAttribute('tabindex');
    let tabIndex: number = tabIndexAttr === null ? NaN : parseInt(tabIndexAttr, 10);
...
function firstElementChild(element: HTMLElement): HTMLElement | null {
    return element.firstElementChild ? (element.firstElementChild as HTMLElement) : null;
}

function lastElementChild(element: HTMLElement): HTMLElement | null {
    return element.lastElementChild ? (element.lastElementChild as HTMLElement) : null;
}

function previousElementSibling(element: HTMLElement): HTMLElement | null {
    return element.previousElementSibling ? (element.previousElementSibling as HTMLElement) : null;
}

function nextElementSibling(element: HTMLElement): HTMLElement | null {
    return element.nextElementSibling ? (element.nextElementSibling as HTMLElement) : null;
}
 ...
 * @param reverse
 */
function compareIndexes(index1: number, index2: number, reverse: boolean): -1 | 0 | 1 {
        ...
        if (!next) {
            ...
            while (parent !== contextElement && !next) {
                ...
                if (next && !isElementVisible(next)) {
                    // Если встретили невидимый элемент - идём к следующему соседу, а не родителю.
                    parent = next;
                    next = undefined;
                    continue;
                }
...
function startChildElement(parent: HTMLElement, reverse: boolean): HTMLElement | null {
...
function find(
    ...
    let nearestTabIndex: number | null = null;
    let foundDelegated: HTMLElement | null = null;
    let savedDelegated: HTMLElement | null = null;
    let fromElementTabIndex: number | null = null;
    ...
    if (fromElement) {
        ...
        next =
            (reverse ? lastElementChild(contextElement) : firstElementChild(contextElement)) ||
            contextElement;
    }

    const fromTabIndex = (fromElementTabIndex ?? givenFromElementTabIndex) as number;

    function canDelegate(nextElement: HTMLElement, nextElementProps: IFocusElementProps): boolean {
        if (nextElementProps.delegateFocusToChildren && nextElement.childElementCount) {
            if (canAcceptFocusControlOnElement(nextElement)) {
                ...
                foundDelegated = findInner(nextElement, reverse, propsGetter, tabbable);
            }
        }
    ...
    for (stage = 0; stage !== 2 && !result; stage++) {
        while (next !== contextElement && next !== fromElement && !result) {
            ...
            if (nextProps.enabled && nextProps.tabStop) {
                cmp = compareIndexes(nextProps.tabIndex, fromTabIndex, reverse);
                if (cmp === 0 && stage === 0) {
            ...
            if (fromTabIndex === -1 && nearestElement) {
        ...
        if (next === contextElement && stage === 0) {
            ...
            if (
                fromElement &&
                ((reverse === false && fromTabIndex > 0) ||
                    (reverse === true && fromTabIndex !== 1 && fromTabIndex !== -1))
            ) {
                next = startChildElement(contextElement, reverse) as HTMLElement;
            }
        }
        if (stage === 0) {
            startFromFirst = true;
    ...
    if (!result && nearestElement) {
        ...
        if ((nearestTabIndex as number) >= 0) {
            result = nearestElement;
        }
    }
...
function getValidatedWithContext(
    ...
    if (validatedElement !== rootElement) {
        parent = validatedElement.parentNode as HTMLElement;
...
function checkElement(element: HTMLElement, paramName: string): void {
    ...
    const hasParentNode: boolean = element === document.documentElement || !!element.parentNode;
    assert(
        element && element.ownerDocument && hasParentNode && element.nodeType === Node.ELEMENT_NODE,
...
export function findWithContexts(
    ...
    let validated = getValidatedWithContext(fromElement, rootElement, propsGetter, tabbable);
    let result: HTMLElement | null = validated.element;
