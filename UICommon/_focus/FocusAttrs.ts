...
const fromFocusAttrsToAreaPropsMap: Record<TFocusAttrName, keyof IFocusProps> = {
...
export function extractAttributesForFocusArea(cfg: IFocusAttributes): IFocusProps {
    ...
    for (let i = 0; i < focusAttrList.length; i++) {
        ...
        if (newKey) {
            extractedAttributes[newKey] = cfg[attrName];
        }
        delete cfg[attrName];
    }
...
const attributesToFocusRootFlag: (keyof IFocusAttributes)[] = [
    'ws-creates-context',
    'ws-delegates-tabfocus',
    'ws-tab-cycling',
    'ws-no-focus',
...
export function hasFocusRootAttributes(cfg: IFocusAttributes): boolean {
    for (const attributeToFocusRootFlag of attributesToFocusRootFlag) {
