...
function focusInner(element: HTMLElement, cfg: IFocusConfig): boolean {
    ...
    const undoScrolling = makeResetScrollFunction(element, cfg.enableScrollToElement ?? false);
    try {
...
/**
 * Фокусировка конкретного элемента
 * @public
 * @param {HTMLElement} [element] Какой элемент будет сфокусирован
 * @param {IFocusConfig} [cfg] Конфигурация фокусировки
 * @returns {boolean} сфокусировался ли элемент
 */
export function focus(element: HTMLElement, cfg?: IFocusConfig): boolean {
    const innerCfg: IFocusConfig = {
