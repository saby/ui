...
const isNull = (prop: string | null): prop is null => {
...
export function restoreEntity(element: IHTMLElement): IRestoredEntity {
    ...
    if (isNull(name) || isNull(href) || isNull(theme) || isNull(themeType)) {
...
function cutFromResourсePrefix(href: string): string | null {
    if (!href) {
        return null;
    }
