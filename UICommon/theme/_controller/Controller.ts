...
export class Controller {
    ...
    async getActiveTheme(
        ...
        // TODO: нужно перевести загрузку default темы из Page/Controller в UI/theme:Controller
        if (config.uuid === 'default') {
            return;
        }

        const styleLink = getResourceUrl(`${THEME_PACKAGE_BUILDED}/${config.uuid}.css`);
        this._get(styleLink, config.uuid, styleLink, THEME_TYPE.MULTI);
        return {
            uuid: config.uuid,
            classList: [
                `t-${config.uuid}`,
    ...
    applyStyles(_activeTheme: IActiveTheme): void {
        return;
    }
