...
import { Logger, isUnitTestMode, getResourceUrl, escapeHtml } from 'UICommon/Utils';
...
import type { IBackgroundImage } from './IBackgroundImage';
...
type TThemeAplyKey = keyof typeof THEME_APPLICATIONS;
export interface IActiveTheme extends TAny {
    isFromDistribution?: true;
    selector?: string;
    uuid?: string;
    version?: number;
    themeApply?: (typeof THEME_APPLICATIONS)[TThemeAplyKey];
...
export class Controller {
    ...
    ): Promise<ICssEntity | void> {
        if (isUnitTestMode()) {
            /** В режиме юнит тестов не будем грузить CSS */
    ...
    async getActiveTheme(
        ...
        let responseJson: { styles?: string[]; properties?: TThemePropertiesObject };
        try {
            responseJson = await import(`json!${THEME_PACKAGE_BUILDED}/${selector}`);
        } catch (e) {
            return {
                isFromDistribution: true,
                error: e,
        ...
        const activeTheme: IActiveTheme = {
            isFromDistribution: true,
            themeApply: THEME_APPLICATIONS.palette, // Возможно, нужна задача положить это в responseJson.
            selector,
            uuid: selector,
            classList: responseJson?.styles || [`t-${selector}`],
            properties: Model.fromObject(
                responseJson?.properties || { staticTheme: calculateStaticTheme(selector) }
            ),
        };
        this.applyStyles(activeTheme);
        return activeTheme;
    }
    ...
    private collectCssLinks(): void {
        ...
        for (let i = 0; i < links.length; i++) {
            const linkEntity = restoreEntity(links[i] as IHTMLElement);
            if (linkEntity) {
                this.set(linkEntity);
            }
        }
...
// TODO Можно будет удалить в 6100 после выполнения https://online.sbis.ru/opendoc.html?guid=2558f9ba-0b19-4f5a-a0de-89f945dccfe5&client=3
function calculateStaticTheme(selector: string): string {
    if (selector === 'default__dark') {
        return 'default__dark';
    }
    if (selector === 'retail__dark') {
        return 'retail__dark-medium';
    }
    if (selector === 'retail__light') {
        return 'retail__light-medium';
    }
    return 'default';
}
