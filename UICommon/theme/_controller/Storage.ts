...
export class EntityStorage {
    constructor(private getStore: () => IStore<IEntities> = createEntityStore()) {}
    /**
    ...
    get(cssName: string, themeName: string): ICssEntity {
        const cssStore = this.getStore().get(cssName);
        const cssEntity = cssStore?.[themeName];
        // Проверка через has делает лишнее получение стора.
        // Плюс при использовании has сложно объяснить ts, что после этой проверки Entity гарантированно существует.
        if (!cssStore || cssEntity === undefined) {
     ...
     */
    async remove(cssName: string, themeName: string): Promise<boolean> {
        const isRemoved = await this.get(cssName, themeName).remove();
        if (isRemoved) {
...
const createAliasStore = (data: IAliases) => {
    return createStore<IAliases>(createDefaultAliasStore, AliasStore.label, data);
};
