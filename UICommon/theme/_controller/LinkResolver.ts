...
function createRequireRoutes(): any {
    return {
        ...
        'react/jsx-runtime':
            'React/third-party/react/jsx-runtime/react-jsx-runtime.production.min',
...
class LinkResolver {
    ...
    constructor(
        ...
        if (
            appRoot &&
            !(resourceRoot && resourceRoot.indexOf(appRoot) !== -1)
        ) {
    ...
    resolvePathToCss(moduleName: string, themeFull: string): string {
        ...
        const modificator =
            splittedThemeName.length === 2 ? splittedThemeName[1] : null;
    ...
    isNewTheme(moduleName: string, theme: string): boolean {
        ...
        if (
            iModuleInfo &&
            iModuleInfo.newThemes &&
            iModuleInfo.newThemes[moduleName]
        ) {
            return (
                iModuleInfo.newThemes[moduleName].indexOf(fixedThemeName) !== -1
            );
        }
        return false;
    }
    ...
    generateFullConfig(moduleName: string, linkConfig: any): ILinkInfo {
        ...
        const needTheme =
            linkConfig.theme && linkConfig.ext === 'css' && !isPackage;
