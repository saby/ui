...
export class BuildTouchHandlers implements ITouchHandlers {
    ...
    handlerTouchmove(event: ITouchEvent): void {
        ...
        LongTapController.checkThreshold(event);
    }
    ...
    handlerTouchend(event: ITouchEvent): void {
        ...
        // @ts-ignore
        FastTouchEndController.clickEmulate(event.target, event, this);
    ...
    handlerClick(event: MouseEvent): void {
        ...
        const hasSelection =
            selection && selection.type === 'Range' && eventTarget.contains(selection.focusNode);
        ...
        const resolvedElement = getElement(eventTarget);
        ...
        try {
            userSelectIsNone =
                window && window.getComputedStyle
                    ? window.getComputedStyle(resolvedElement).getPropertyValue('user-select') ===
                      'none'
                    : true;
        } catch (e) {
        ...
        const isTargetNotEmpty =
            window && eventTarget?.textContent && eventTarget.textContent.trim().length > 0;
