...
export class BuildTouchHandlers implements ITouchHandlers {
    ...
    handlerTouchend(event: ITouchEvent): void {
        ...
        FastTouchEndController.clickEmulate(event.target as Element, event, this);
    ...
    handlerClick(event: MouseEvent): void {
        ...
        try {
            userSelectIsNone =
                window && window.getComputedStyle
                    ? window.getComputedStyle(resolvedElement as HTMLElement)['user-select'] ===
                      'none'
                    : true;
        } catch (e) {
        ...
        const isTargetNotEmpty =
            window && (event.target as HTMLElement)?.textContent.trim().length > 0;
