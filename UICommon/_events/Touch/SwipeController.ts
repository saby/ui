...
import { SWIPE_MIN_DISTANCE, SWIPE_MAX_DISTANCE, SWIPE_THRESHOLD } from './TouchConstants';
...
export class SwipeController {
    ...
    static resetState(): boolean {
        ...
        // Маскировка того, что из resetState не может вернуться truthy. Возможно, нужен рефакторинг условия выхода в initState.
        return false;
    }

    static initState(event: ITouchEvent): IEventState | void {
    ...
    private static detectSwipe(event: ITouchEvent): string | undefined {
        ...
        if (swipeState && swipeState.target) {
            ...
            if (swipeDirection) {
                ...
                const swipeCustom = new CustomEvent('swipe', {
                    detail: createEventDetails(swipe, eventTarget),
                    bubbles: true,
                });
                eventTarget.dispatchEvent(swipeCustom);
                this.resetState();
            }
        }
    }
