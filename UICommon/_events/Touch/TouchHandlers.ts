...
import { LONG_TAP_MIN_DURATION } from './TouchConstants';
...
export class TouchHandlers {
    ...
    static shouldUseClickByTapOnClick(event: MouseEvent): void {
        ...
        if (TouchHandlers.shouldUseClickByTap(eventTarget)) {
            const idx = TouchHandlers.getClickStateIndexForTarget(
                TouchHandlers.fixSvgElement(eventTarget)
            );
    ...
    static shouldUseClickByTapOnTouchstart(event: ITouchEvent): void {
        ...
        if (TouchHandlers.shouldUseClickByTap(eventTarget)) {
            ...
            const target = TouchHandlers.fixSvgElement(eventTarget);
    ...
    static shouldUseClickByTapOnTouchend(event: ITouchEvent): void {
        if (
            ...
            setTimeout(() => {
                ...
                const idx = TouchHandlers.getClickStateIndexForTarget(
                    TouchHandlers.fixSvgElement(event.target as Element)
                );
                if (idx > -1 && TouchHandlers.clickStateTarget[idx].touchId < lastTouchId) {
                    ...
                    const clickEventObject = TouchHandlers.generateClickEventFromTouchend.call(
                        this,
                        event as TouchEvent
                    ) as MouseEvent;
    ...
    private static shouldUseClickByTap(target: Element): boolean {
        ...
        return !!(
            ...
            (
                (constants.browser.isDesktop && !target.closest('.wnc-sabyget-composite__shop')) ||
    ...
    private static generateClickEventFromTouchend(event: TouchEvent): IMouseEventInitExtend {
    ...
    private static fixSvgElement(element: EventTarget): HTMLElement {
        return (element as SVGElement).ownerSVGElement
            ? ((element as SVGElement).ownerSVGElement as unknown as HTMLElement)
