...
type TFastEventList = {
    [key: string]: new (type: string, properties: IMouseEventInitExtend) => MouseEvent | TouchEvent;
};

let fastEventList: TFastEventList;
function getFastEventList(): TFastEventList {
    if (!fastEventList) {
        fastEventList = {
            touchend: TouchEvent,
...
export class FastTouchEndController {
    ...
    static clickEmulate(
        ...
        // для хрома на iphone/ipad по-прежнему нужно эмулировать фаст тач
        ...
        let clickEvent;
        for (const eventName of Object.keys(fastEventList)) {
            ...
            clickEvent = new fastEventList[eventName](
                eventName,
                this.createMouseEvent(eventName, nativeEvent, touch)
            );
            // @ts-ignore
        ...
        if (!hasNoFocus(nativeEventTarget) && document.activeElement === lastActiveElement) {
    ...
    private static useNativeTouchEnd(targetElement: Element, nativeEvent: TouchEvent): boolean {
        ...
        if (detection.isMobileIOS && detection.safari && this.findHref(targetElement)) {
            return true;
        }
        ...
        if (this.isNativeList(activeElement) || this.isContentEditable(activeElement)) {
            return true;
        }
