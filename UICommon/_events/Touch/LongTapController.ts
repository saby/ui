...
let longTapState: any;
let handlerName;

export class LongTapController {
    private static tapTimeout: ReturnType<typeof setTimeout>;
    ...
    static initState(event: ITouchEvent, callbackFn: Function): IEventState | void {
    ...
    static detectState(event: ITouchEvent): boolean | void {
        if (longTapState && longTapState.target) {
            ...
            if (isLongTap) {
                ...
                longTap = this.eventCoordPatch(longTap, event.touches?.[0] as Touch);
