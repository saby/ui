/* eslint-disable */
import type { default as SyntheticEvent } from './SyntheticEvent';
...
const originDOMEventNames: Record<string, string | undefined> = {
 ...
 * @returns {boolean}
 */
export function isEvent(titleAttribute: string) {
 ...
 * @returns {string}
 */
export function getEventName(eventAttribute: unknown[]) {
 ...
 * @returns {boolean}
 */
export function checkPassiveFalseEvents(eventName: string): boolean {
 ...
 * @returns {boolean}
 */
export function isSpecialBodyEvent(eventName: string): boolean {
 ...
 * @returns {boolean}
 */
export function fixUppercaseDOMEventName(name: string) {
 ...
 * returns {Function}
 */
export function tmplNotify(this: any, _event: Event, eventName: string, ...args: unknown[]) {
    /**
 ...
 * returns {void}
 */
export function proxyModelEvents(component: any, model: any, eventNames: string[]) {
    eventNames.forEach((eventName: string) => {
        model.subscribe(eventName, (_event: Event, value: unknown) => {
 ...
 * returns {void}
 */
export function keysHandler(
    event: SyntheticEvent<KeyboardEvent>,
    keys: Record<string, string | number>,
    handlerSet: Record<string, any>,
    scope: object,
    dontStop: boolean
): void {
        ...
        if (keys.hasOwnProperty(action)) {
          ...
          */
            if (
                ...
                if (!dontStop) {
                    if ((event.target as HTMLElement).closest('.ws-dont-stop-native-event')) {
                        // @ts-ignore
...
export function checkBindValue(event: any, value: string): boolean {
    ...
    const checkNested = (obj: any, propName: string[], index: number): boolean => {
        if (obj === undefined) {
            return false;
        }
        if (obj === null) {
        ...
        if (propValue in obj && propName.length === index + 1) {
            return true;
        }
        const propInObj = obj[propValue];
        if (Array.isArray(propInObj)) {
            // могли сделать bind на массив внутри объекта, надо проверить что все поля совпадают
            let checkArray = [];
            for (let i = 0; i < propInObj.length; i++) {
                checkArray.push(checkNested(propInObj[i], propName, index + 1));
            }
            return checkArray.indexOf(false) <= -1;
        }
        if (propInObj instanceof EntityRecord) {
            return true;
        }
        return checkNested(propInObj, propName, index + 1);
    };
    ...
    const checkRecord = (value: string): boolean => {
        ...
        if (
            ...
            (record instanceof EntityRecord && typeof record.get(cleanRecordName) === 'undefined')
        ) {
            wrongBind(value);
        }
        return true;
