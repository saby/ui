...
export function checkBindValue(event, value): boolean {
    const checkNested = (obj, propName, index) => {
        ...
        if (Array.isArray(obj[propName[index]])) {
            ...
            for (let i = 0; i < obj[propName[index]].length; i++) {
                checkArray.push(
                    checkNested(obj[propName[index]][i], propName, index + 1)
                );
            }
    ...
    const checkRecord = (value: string): boolean => {
        ...
        const record =
            value.indexOf('_options') === 0
                ? event.data?._options?.record
                : event.data?.record;
        ...
        if (
            ...
            (record instanceof Record &&
                typeof record.get(cleanRecordName) === 'undefined')
        ) {
            wrongBind(value);
        }
        return true;
