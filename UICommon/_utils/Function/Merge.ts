...
function cloneOrCopy(
    ...
    if (
        typeof hashExtender[key] === 'object' &&
        hashExtender[key] !== null &&
        config.clone
    ) {
        /**
...
function mergeInner(
    ...
    if (
        ...
        for (const i in hashExtender) {
            ...
            if (hashExtender[i] && !!hashExtender[i].$$typeof) {
                ...
                if (
                    ...
                    if (hash[i] instanceof Date) {
                        if (hashExtender[i] instanceof Date) {
                            ...
                            hash[i] =
                                hashExtender[i] instanceof Array ? [] : {};
                        }
        ...
        canBeRepalcedWith(hashExtender, config) &&
        !config.preferSource
    ) {
        hash = hashExtender;
    }
