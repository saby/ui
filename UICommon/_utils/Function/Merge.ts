...
function cloneOrCopy(
    ...
    hashExtender: Record<string, unknown>,
    key: string,
    config: IConfig,
    path: IPath
): void {
    ...
    const hashExtenderValue = hashExtender[key];
    if (typeof hashExtenderValue === 'object' && hashExtenderValue !== null && config.clone) {
        /**
         * Если к нам пришел объект и можно клонировать
         * Запускаем мерж того, что пришло с пустым объектом (клонируем ссылочные типы).
         */
        if (isMergeableObject(hashExtenderValue)) {
            hash[key] = mergeInner(
                hashExtenderValue instanceof Array ? [] : {},
                hashExtenderValue,
                config,
                key,
                path
         ...
         */
        if (canBeRepalcedWith(hashExtenderValue, config)) {
            hash[key] = hashExtenderValue;
        }
    }
}
