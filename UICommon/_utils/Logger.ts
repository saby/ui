...
import { goUpByControlTree, getClosestControl } from 'UICommon/NodeCollector';
...
const prepareStack = (stackNode: { [key: string]: any }): string => {
    ...
    const excludeControls: Record<string, boolean> = {
        ...
        if (typeof moduleName === 'string' && !excludeControls[moduleName]) {
            ...
            if (moduleName === 'Controls/Container/Async') {
                ...
                const asyncTemplateName =
                    (opts && 'templateName' in opts && opts.templateName) || 'not detected';
    ...
    if (isBrowserPlatform && data && data._container) {
        ...
        const arrayControls = goUpByControlTree(data._container);
 ...
 * @return {Object}
 */
const warn = (msg: string = '', errorPoint?: any): object => {
...
const error = (
    ...
    let data = '';
    let typeError = errorType || 'CONTROL ERROR';

    // если нет информации по ошибке, создадим сами. Только на клиенте
    if (isBrowserPlatform && !errorInfo) {
    ...
    if (!isBrowserPlatform) {
        data += addAdditionalInfo(errorPoint, errorInfo);
    }

    if (errorInfo) {
        logger().error(typeError, data, errorInfo);
    } else {
        logger().error(typeError, data);
    }
    ...
    if (
        ...
        const erEvent = new ErrorEvent('JS_ERROR', {
            error,
            message: `${typeError},
                     info: ${data}
...
/**
 * Временный способ для того, чтобы определить где был вызван Logger.error
 */
function addAdditionalInfo(errorPoint?: object | Node | any, errorInfo?: object | any): string {
    const serachMsg = 'На Сервисе Представления запрещено делать RPCJSON вызов';

    if (errorPoint?.message?.includes(serachMsg) || errorInfo?.message?.includes(serachMsg)) {
        const pointErr = new Error('Определение точки выведения ошибки в логи');
        return '\n' + pointErr.stack + '\n';
    }
...
const asyncRenderErrorLog = (errorObject: any, errorPoint?: object | Node | unknown): object => {
    const message = `Async function was rejected ${
        errorObject ? `with message "${errorObject.message || errorObject}"` : ''
    }`;
