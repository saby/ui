...
export function getProxyChildren<T>(moduleName?: string): T | {} {
    ...
    return new Proxy(
        {},
        {
            ...
            set(target: Record<string, unknown>, prop: string, value: Record<string, unknown>) {
                // value - это DOM-элемент или контрол
                if (value?._unmounted) {
                    Logger.error(
                        `Попытка задать _children ${prop} на размонтированном контроле ${value._moduleName}, 
                        Надо встать точкой остановы в место, где задается _children и проверить почему это произошло. 
                        Если эту ошибку не исправить - такое поведение может привести к утечкам памяти!`,
                        value
                    );
                }
