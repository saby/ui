/* eslint-disable consistent-this, private-access */
define('UICommon/_state/Serializer', [
   ...
   'UICommon/_state/Types'
], function (require, Utils, Env, deserializeTemplate, Library, SerializerTypes) {
   /**
    */
      ...
      if (!module) {
         ...
         parts.path.forEach(function (element, index) {
            if (!(element in module)) {
               ...
               throw new Error(
                  'The module "' + parts.name + '" doesn\'t export element "'
                  + element + '" which suppose to be found within path "'
   ...
   Serializer.prototype._patterns = {
      ...
      ContentOption: /^CONTENT_OPTION,(\d+),/,
   };
         ...
         if (isObject && value.hasOwnProperty(SERIALIZED_KEY)) {
            switch (value[SERIALIZED_KEY]) {
                  ...
                  if (typeof value.id === 'number' && self._functionStorage[value.id]) {
                     ...
                     if (module) {
                        ...
                        try {
                           ...
                           throw new Error(
                              'Parsing function declaration "' + declaration
                              + '" failed. Original message: ' + e.message
                           );
                        }
               ...
               case TYPE_NAN:
                  ...
                  break;
               default:
                  throw new Error('Unknown serialized type "' + value[SERIALIZED_KEY] + '" detected');
            }
         }
            ...
            switch (result) {
               ...
               default:
                  for (var key in self._patterns) {
                     if (self._patterns.hasOwnProperty(key) && self._patterns[key].test(result)) {
                        switch (key) {
                           case 'Date':
   ...
   Serializer.getFuncFromDeclaration = function getFuncFromDeclaration(declaration) {
      ...
      if (module) {
         ...
         try {
            ...
            throw new Error(
               'Parsing function declaration "' + declaration + '" failed. Original message:' + e.message
            );
         }
   ...
   Serializer.prototype._resolveLinks = function () {
      for (var i = 0; i < this._unresolvedLinks.length; i++) {
         ...
         if (index === -1) {
            throw new Error(
               'Can\'t resolve link for property "' + link.name + '" with instance id "' + link.value.id + '".'
            );
         }
