...
describe('UICommon/theme/_controller/Controller', () => {
    ...
    describe('get', () => {
        itIf(constants.isBrowserPlatform)(
            'Метод возвращает Promise<Link> на клиенте',
            () => {
        ...
        itIf(constants.isServerSide)(
            'Метод возвращает Promise<LinkPS> на СП',
            () => {
                const getting = controller.get(cssName);
                assert.instanceOf(getting, Promise);
                return getting.then((css) => {
        ...
        itIf(constants.isBrowserPlatform)(
            'Загруженные стили не запрашиваются повторно',
            () => {
                return controller
        ...
        itIf(constants.isBrowserPlatform)(
            'Стили загружаются отдельно для каждой темы',
            () => {
    ...
    describe('getAll', () => {
        itIf(constants.isBrowserPlatform)(
            'Метод возвращает Link[] на клиенте',
            () => {
    ...
    describe('define', () => {
        ...
        it('Метод getAll возвращает те же css-сущности при запросе алиасов и оригинальных имен', () => {
            ...
            return Promise.all([
                Promise.all([
                    controller.get(aliasName),
                    controller.get(aliasName2),
                ]),
                Promise.all([
                    controller.get(originalName),
                    controller.get(originalName2),
                ]),
        ...
        it('Метод remove удаляет по алиасу', () => {
            ...
            return controller
                ...
                .then((isRemoved) => {
                    ...
                    assert.isFalse(
                        controller.has(aliasName),
                        'Алиас остался в хранилище'
                    );
                    assert.isFalse(
                        controller.has(originalName),
                        'Оригинал остался в хранилище'
                    );
                });
        });
    });
