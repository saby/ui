...
import { collectObjectVersions, getChangedOptions } from 'UICommon/_vdom/Options';
import { TClosure } from 'UI/Executor';

function createInternalsCollection(list: [number, unknown][]) {
    const internal: Record<string, unknown> = { };

    list.forEach(([key, value]) => {
        internal[`__dirtyCheckingVars_${key}`] = value;
    });

    return internal;
}

function createContentOptionTmpl(fn: Function, internal: object = { }) {
    const contentOption = fn.bind(undefined);

    contentOption.isWasabyTemplate = false;
    contentOption.internal = {
        ...internal,
        __dirtyCheckingVars_000: fn
    };

    return contentOption;
}

function createContentOptionWml(fn: Function, isVdom: boolean, internal: object = { }) {
    const contentOption = fn.bind(undefined);
    const internalsCollection = {
        ...internal,
        __dirtyCheckingVars_000: fn
    };
    ...
    return TClosure.createDataArray([{
        func: contentOption,
        internal: internalsCollection,
        isWasabyTemplate: true
    }], 'stub-template-name', true, isVdom);
}

function createVersionableArray(array: any[] = [], version: number = 0) {
    const value = Array.from(array) as any;

    value._arrayVersion = version;
    value.getArrayVersion = function() {
        return this._arrayVersion;
    };
    value.increaseVersion = function() {
        this._arrayVersion++;
    };
...
describe('UICommon/_vdom/Options', () => {
    const date = new Date();
    ...
    describe('collectObjectVersions new', () => {
        ...
        it('should handle undefined as input object', () => {
            ...
            expect(versions).toEqual(emptyObject);
        });
        ...
        it('should collect possible scope object versions of internal option', () => {
            ...
            const versions = collectObjectVersions(createInternalsCollection([
        ...
        it('should collect versionable object type versions from internals', () => {
            ...
            const versions = collectObjectVersions(internal);

            expect(versions).toEqual({
                __dirtyCheckingVars_0: 123,
                __dirtyCheckingVars_1: 234,
                __dirtyCheckingVars_2: 345
            });
        });
        it('should collect versionable object type versions from content option internals', () => {
            ...
            expect(versions).toEqual({
                'opt2;0;__dirtyCheckingVars_0': 123,
                'opt2;0;__dirtyCheckingVars_1': 234,
                'opt2;0;__dirtyCheckingVars_2': 345,
                'opt3;0;__dirtyCheckingVars_0': 123,
                'opt3;0;__dirtyCheckingVars_1': 234,
                'opt3;0;__dirtyCheckingVars_2': 345
            });
        });
    });
    ...
    describe('getChangedOptions', () => {
        ...
        it('should not detect option insertion for compound check', () => {
            ...
            const changes = getChangedOptions(next, prev, false, { }, true, '', true);
        ...
        it('should not detect internal changes 2', () => {
            ...
            const changes = getChangedOptions(next, prev, true);
        ...
        describe('versionable values', () => {
            it('should not detect changes of versionable array with equal reference and version', () => {
                ...
                const changes = getChangedOptions(next, prev, false, versionsStorage);
            ...
            it('should detect changes of versionable array with equal reference and new version', () => {
                ...
                versionableArray.increaseVersion();
        ...
        describe('template data types', () => {
            it('should not detect ws:Array changes', () => {
                ...
                const changes = getChangedOptions(next, prev, false, {}, true, '', false, ['opt1']);
            ...
            it('should not detect ws:Function changes', () => {
                ...
                const changes = getChangedOptions(next, prev, false, {}, true, '', false, ['opt1', 'opt2']);
