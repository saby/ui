        ...
        compiler
            ...
            .then(function (artifact) {
                try {
                    ...
                    assert.strictEqual(
                        artifact.nodeName,
                        'wml!Compiler/Compiler/Template'
                    );
    ...
    it('Wml plugin detection', (done) => {
        ...
        compiler
            ...
            .then(function (artifact) {
                try {
                    assert.strictEqual(
                        artifact.text.indexOf(
                            "define('wml!Compiler/Compiler/Template'"
                        ),
                        0
                    );
                    assert.strictEqual(
        ...
        compiler
            ...
            .then(function (artifact) {
                try {
                    assert.strictEqual(
                        artifact.text.indexOf(
                            "define('tmpl!Compiler/Compiler/Template'"
                        ),
                        0
                    );
                    assert.strictEqual(
                        artifact.nodeName,
                        'tmpl!Compiler/Compiler/Template'
                    );
                    done();
                } catch (error) {
                    done(error);
    ...
    it('Create dictionary', (done) => {
        ...
        compiler
            ...
            .then(function (artifact) {
                try {
                    assert.isTrue(
                        artifact.hasOwnProperty('localizedDictionary')
                    );
                    assert.strictEqual(
                        artifact.localizedDictionary[0].type,
                        'auto'
                    );
                    assert.strictEqual(
                        artifact.localizedDictionary[0].key,
                        'Hello'
                    );
                    assert.strictEqual(
                        artifact.localizedDictionary[0].context,
                        ''
                    );
                    assert.strictEqual(
                    ...
                    assert.strictEqual(
                        artifact.localizedDictionary[1].type,
                        'manual'
                    );
                    assert.strictEqual(
                        artifact.localizedDictionary[1].key,
                        'Goodbye'
                    );
                    assert.strictEqual(
                        artifact.localizedDictionary[1].context,
                        ''
                    );
                    assert.strictEqual(
