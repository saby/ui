...
import * as Attributes from 'Compiler/_core/Attributes';
import { createTextProcessor } from 'Compiler/_core/Text';
...
function validateValueAttributes(
    textAttributes: string,
    name: string,
    hasAttributesOnly: boolean
) {
...
describe('Compiler/core/Attributes', () => {
    describe('Helpers', () => {
        ...
        it('getAttributeName() #1', () => {
            assert.strictEqual(
                Attributes.getAttributeName('attr:class'),
                'class'
            );
        });
    ...
    describe('process', () => {
        describe('hasAttributesOnly = true', () => {
            it('Empty attribute value', () => {
                ...
                assert.strictEqual(
                    Object.keys(attributes.attributes).length,
                    1
                );
                assert.strictEqual(Object.keys(attributes.options).length, 0);
                assert.isTrue(
                    attributes.attributes.hasOwnProperty('attr:attribute')
                );
                assert.strictEqual(
                    attributes.attributes['attr:attribute'].__$ws_value.length,
                    1
                );
                assert.strictEqual(Object.keys(attributes.events).length, 0);

                assert.instanceOf(
                    attributes.attributes['attr:attribute'],
                    Ast.AttributeNode
                );
            ...
            it('Attribute boolean (without prefix)', () => {
                ...
                assert.strictEqual(Object.keys(attributes.events).length, 0);
                assert.isTrue(
                    attributes.attributes.hasOwnProperty('attr:allowfullscreen')
                );
            ...
            it('Duplicate attributes', () => {
                ...
                assert.strictEqual(Object.keys(attributes.events).length, 0);
                assert.isTrue(
                    attributes.attributes.hasOwnProperty('attr:value')
                );
            });
            it('Bind', () => {
                const attributes = processAttributes(
                ...
                assert.isTrue(
                    attributes.events.hasOwnProperty('bind:attribute')
                );

                assert.instanceOf(
                    attributes.events['bind:attribute'],
                    Ast.BindNode
                );
                const bindNode = <Ast.BindNode>(
                    attributes.events['bind:attribute']
                );
    ...
    describe('validateValue', () => {
        it('Receive value', (done) => {
            try {
                const value = validateValueAttributes(
                    'attribute="value"',
                    'attribute',
                    false
                );
