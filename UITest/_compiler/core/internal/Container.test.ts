import {Container, ContainerType, InternalNode, InternalNodeType} from 'Compiler/core/internal/Container';
...
function assertInternal(actual: InternalNode, expected: IInternalStandard): void {
   ...
   const actualPrograms = actual.storage.getMeta().map(meta => meta.node.string).sort();
...
function makeConditionalBlocked(node: IInternalStandard, useTest: boolean = true) {
...
describe('Compiler/core/internal/Container', () => {
   describe('Tree of container nodes', () => {
      ...
      describe('processing container index', () => {
         ...
         beforeEach(() => {
            component1 = globalContainer.createContainer(ContainerType.COMPONENT);
            contentOption1 = component1.createContainer(ContainerType.CONTENT_OPTION);
            component2 = contentOption1.createContainer(ContainerType.COMPONENT);
            contentOption2 = component2.createContainer(ContainerType.CONTENT_OPTION);
            conditional1 = contentOption2.createContainer(ContainerType.CONDITIONAL);
         ...
         it('cycle in content option #2', () => {
            assert.strictEqual(cycle1.getProcessingContainerIndex(), contentOption2.index);
         });
         it('conditional in content option #2', () => {
            assert.strictEqual(conditional1.getProcessingContainerIndex(), contentOption2.index);
         });
         it('content option #2 in component #2', () => {
            assert.strictEqual(contentOption2.getProcessingContainerIndex(), contentOption2.index);
         });
         it('component #2 in content option #1', () => {
            assert.strictEqual(component2.getProcessingContainerIndex(), contentOption1.index);
         });
         it('content option #1 in component #1', () => {
            assert.strictEqual(contentOption1.getProcessingContainerIndex(), contentOption1.index);
         });
         it('component #1 in global', () => {
            assert.strictEqual(component1.getProcessingContainerIndex(), globalContainer.index);
         });
         it('conditional in template', () => {
            assert.strictEqual(conditional2.getProcessingContainerIndex(), template.index);
         });
         it('cycle in template', () => {
            assert.strictEqual(cycle2.getProcessingContainerIndex(), template.index);
         });
   ...
   describe('Tree of internal nodes', () => {
      ...
      beforeEach(() => {
         globalContainer = new Container(null, ContainerType.GLOBAL);
         globalContainer.registerProgram(parse('a'), ProgramType.SIMPLE, null);

         component = globalContainer.createContainer(ContainerType.COMPONENT);
         component.registerProgram(parse('b'), ProgramType.ATTRIBUTE, 'attr-b');
         component.registerProgram(parse('c'), ProgramType.OPTION, 'opt-c');

         contentOption = component.createContainer(ContainerType.CONTENT_OPTION);
         contentOption.addIdentifier('content');

         component2 = contentOption.createContainer(ContainerType.COMPONENT);
         component2.registerProgram(parse('d'), ProgramType.ATTRIBUTE, 'attr-d');
         ...
         contentOption.registerProgram(parse('f'), ProgramType.SIMPLE, null);
         contentOption.registerProgram(parse('content.g'), ProgramType.SIMPLE, null);
         ...
         elseIfNode = contentOption.createContainer(ContainerType.CONDITIONAL);
