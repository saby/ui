import { Serializer as UISerializer } from 'UI/State';
import { Model, Record } from 'Types/entity';
import { List } from 'Types/collection';
import { NEW_TYPE } from 'Types/_serializer/Types';

// eslint-disable-next-line
type Serializer = any;
// eslint-disable-next-line
const Serializer = UISerializer as any;
...
describe('UI/State:Serializer', () => {
    ...
    describe('.serialize()', function () {
        ...
        it('сериализация анонимной функции', () => {
            const data = {
                func: () => {},
            };
            let serialized;
            try {
                serialized = JSON.stringify(data, serializer.serializeStrict);
            } catch (err: Error) {
                expect(err.message).toMatch('Невозможно сериализовать функцию');
            }

            expect(serialized).not.toBeDefined();
        });
    });
    ...
    describe('.deserialize()', function () {
        ...
        describe('deserialize date', () => {
            ...
            it('десериализация Date, не трогает строку с датой', () => {
                // Здесь проверка поведения Serializer как она используется в StateReceiver
                // В StateReceiver приходится так ухищряться чтобы дата в виде строки не превращалась в объект Date при десериализации

                const obj = {
                    date: new Date(),
                    dateStr: '2024-06-18T11:20:33.769Z',
                };

                const origToJSON = Date.prototype.toJSON;
                // @ts-ignore
                Date.prototype.toJSON = function () {
                    return { $serialized$: NEW_TYPE.DATE, state: origToJSON.call(this) };
                };
                const ser = JSON.stringify(obj, serializer.serialize);
                Date.prototype.toJSON = origToJSON;

                Serializer.setIgnorePatternName('DATE');
                const result = JSON.parse(ser, serializer.deserialize);
                Serializer.deleteIgnorePatternName('DATE');

                expect(result.date).toBeInstanceOf(Date);
                expect(result.date).toEqual(obj.date);
                expect(result.dateStr).toStrictEqual(obj.dateStr);
            });

            it('Обычное поведение Serializer с датами', () => {
                const date = new Date('1995-12-17T01:02:03');
                const dateStr = date.toJSON();
