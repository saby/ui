...
export class AdaptiveInitializer extends React.Component<any, IAdaptiveInitializerState> {
    ...
    private ignoreBodyScroll() {
        const blacklist = ['/page/calendar', '/page/month', '/page/year'];
        ...
        blacklist.find((badPathname) => {
            if (pathname.indexOf(badPathname) !== -1) {
                result = true;
                return true;
            }
        });
    ...
    protected operateScrollOnBody() {
        if (ScrollOnBodyStore.read('enabled') && !this.ignoreBodyScroll()) {
            enableBodyScroll();
        } else {
            disableBodyScroll();
        }
    ...
    private subscribeToPopupManager() {
        ...
        this.popupManagerChannel.subscribe('managerPopupCreated', this._popupCreatedHandler, this);
        this.popupManagerChannel.subscribe(
    ...
    private unsubscribeToPopupManager() {
        ...
        this.popupManagerChannel.unsubscribe(
            'managerPopupCreated',
            this._popupCreatedHandler,
            this
        );
        this.popupManagerChannel.unsubscribe(
            'managerPopupDestroyed',
    ...
    private _isSlidingPanel(popup) {
        const lastPopupType = popup.controller.TYPE;
        return lastPopupType === 'SlidingPanel';
    }
    private _popupCreatedHandler(_e, _config, popupList) {
        const lastPopup = popupList.at(popupList.getCount() - 1);
        ...
        if (this._isSlidingPanel(lastPopup)) {
            enableBodyScroll();
            return;
        }
        disableBodyScroll();
    }
    private _popupDestroyedHandler(_e, _config, popupList) {
