 ...
 * @public
 */
export default abstract class Async extends Control<
    IAsyncOptions,
    TAsyncStateReceived
> {
    /**
    ...
    protected _beforeMount(options: IAsyncOptions): Promise<void> | void {
        ...
        if (
            ...
            !ModulesLoader.isLoaded(options.templateName)
        ) {
            return this._loadContentAsync(
                options.templateName,
                options.templateOptions
            );
        }

        this.error = this._loadContentSync(
            options.templateName,
            options.templateOptions
        );
        if (this.error) {
