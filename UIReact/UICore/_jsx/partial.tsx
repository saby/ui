...
function getUniqueKey(component: IComponent, props: IControlOptions): string | number {
    ...
    // TODO https://online.sbis.ru/doc/5280c4e7-6f2c-48bd-a281-a58e8f5f5597
    if (props.item && props.item.key && props.item.listInstanceName) {
        return props.item.key;
    }
    // Ещё один костыль для ключа RS васаби внутри реакта. Так близко к выпуску так безопаснее.
    // По месту поправить не получится, потому что ключ из кода вида <Wasaby key={uniqueKey} /> не доберётся сюда.
    // В 6100 бы уже закодить решение, которое позволит удалить оба костыля.
    // TODO https://online.sbis.ru/doc/5280c4e7-6f2c-48bd-a281-a58e8f5f5597
    if (props.id && props.ContentRender && props.fullWidth) {
        return props.id;
    }
