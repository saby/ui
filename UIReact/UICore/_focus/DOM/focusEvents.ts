...
let nextActiveElement: HTMLElement = null;

let keyPressedData: IKeyPressedData = null;
...
function mouseDownHandler(event: MouseEvent): void {
    ...
    event.preventDefault = function noFocusPreventDefault(): void {
        ...
        if (!hasNoFocus(this.target)) {
            ...
            logger.warn(
                'Вызван preventDefault у события mousedown в обход атрибута ws-no-focus!'
            );
        }
        if (detection.isIE) {
            clickFakeElement();
