...
export class GeneratorVdom extends Generator implements IGenerator {
    ...
    prepareDataForCreate(
        ...
        const controlProperties =
            Scope.calculateScope(scope, Common.plainMerge) || {};
    ...
    protected calculateOptions(
        ...
        originRef:
            | string
            | React.MutableRefObject<Control>
        ...
        if (!isServerSide) {
            ...
            if (
                ...
                config.viewController.childrenRefsCreator
            ) {
                chainOfRef.add(
    ...
    private calculateInheritOptions(
        ...
        if (typeof options.readOnly === 'undefined') {
            options.readOnly =
                config.data.readOnly || config.data.props?.readOnly || false;
        }
        if (typeof options.theme === 'undefined') {
            options.theme =
                config.data.theme || config.data.props?.theme || 'default';
        }
        if (typeof options.pageData === 'undefined') {
            options.pageData =
                config.data.pageData || config.data.props?.pageData || null;
        }
        if (typeof options.Router === 'undefined') {
            options.Router =
                config.data.Router || config.data.props?.Router || null;
        }
        if (typeof options.isAdaptive === 'undefined') {
            options.isAdaptive =
                config.data.isAdaptive ||
                config.data.props?.isAdaptive ||
                false;
        }
        if (typeof options.adaptiveMode === 'undefined') {
            options.adaptiveMode =
                config.data.adaptiveMode ||
                config.data.props?.adaptiveMode ||
                null;
        }
        return options;
    }
