...
import { wasabyToReactAttrNames } from './wasabyToReactAttrNames';
...
const cutBase64Property: (base64str: string) => {
    ...
    while (str.includes(dataImageSegment) && str.includes(base64Segment)) {
        ...
        const leftEdge =
            substringStart -
            str.substring(0, substringStart).split(semicolon).pop().length;
        const rightEdge =
            substringEnd +
            str.substring(substringEnd).split(semicolon).shift().length;
        ...
        result.style[
            formatStringToCamelCase(
                substring.substring(0, propertyNameEdge).trim()
            )
        ] = substring.substring(propertyNameEdge + 1).trim();
        result.str = str =
            str.substring(0, leftEdge) + str.substring(rightEdge);
    }
...
/**
 * Шаблоны замены стандартных и wasaby атрибутов в реактовские аналоги.
 * Стандартные и wasaby атрибуты хранятся как ключи.
 * Аттрибуты React хранятся как значения.
 * Под стандартными атрибутами понимаются такие атрибуты как class (вместо className в реакте), spellcheck и т.д.
 * Также добавлены атрибуты wasaby (например templateName, fixCompatible)
 */
const replacementTemplates = wasabyToReactAttrNames;

/**
 * Стандартные и wasaby атрибуты в html элементах, хранятся как ключи в replacementTemplates.
 */
const keysReplacementTemplates = Object.keys(replacementTemplates);

/**
 * Шаблоны замены реакт атрибутов (в camelCase) в wasaby атрибуты
 * Напр. className превратится в class
 */
const reactToWasabyAttrNames = keysReplacementTemplates.reduce(
    (result, key) => {
        result[wasabyToReactAttrNames[key]] = key;
        return result;
    },
    {}
);

const keysReactToWasabyAttrNames = Object.keys(reactToWasabyAttrNames);

// boolean аттрибуты, которые не нужно приводить к строке
const SPECIAL_BOOL_ATTRS = ['checked', 'readOnly'];
 ...
 */
export function wasabyAttrsToReactDom<
...
>(
    ...
    const convertedAttributes: P = Object.isFrozen(attributes)
        ? { ...attributes }
        : attributes;
...
export function reactAttrsToWasabyDom<T = HTMLElement>(
    ...
    const convertedAttributes = Object.isFrozen(attributes)
        ? { ...attributes }
        : attributes;
    for (const key of Object.keys(attributes)) {
        if (keysReactToWasabyAttrNames.includes(key)) {
