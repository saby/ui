...
import { CreateFocusRef } from '../Refs/CreateFocusRef';
...
export class CreateTagVdom implements IGeneratorComponent {
    ...
    create<T extends HTMLElement, P extends React.HTMLAttributes<T>>(
        ...
        if (!isServerSide) {
             ...
             */
            fullEvents =
                Attr.mergeEvents(attrToDecorate.events, attrs.events) || {};
        }
        ...
        let chainOfRef;
        const convertedAttributes2 =
            _FocusAttrs.extractFocusAttributes(mergedAttrs);
        if (!isServerSide) {
            ...
            const сreateFocusRef = new CreateFocusRef(convertedAttributes2);
            ...
            if (
                ...
                if (attrToDecorate.refForContainer) {
                    chainOfRef.add(
                        new CreateOriginRef(attrToDecorate.refForContainer)
                    );
                }
            }
        }
        ...
        if (tagName === 'invisible-node') {
            if (!isServerSide) {
                chainOfRef.add(
                    new CreateInvisibleNodeRef(control, isContainerNode)
                );
            }
        ...
        if (flatChildren.for) {
            ...
            return React.createElement<P, T>(
                tagName,
                mergedAttrs,
                flatChildren
            );
        }
        return React.createElement<P, T>(tagName, mergedAttrs, ...flatChildren);
    }
            ...
            const eventCallback = (event, ...args) => {
                for (const item of eData) {
                    ...
                    try {
                        ...
                        const eventContext = preparedContext.apply(
                            item.viewController
                        );
                        if (item.handler) {
                            const handler = item.handler.apply(
                                item.viewController
                            );
    ...
    private calculateEventObject(event) {
        ...
        if (
            !event.nativeEvent ||
            (event.nativeEvent && event.nativeEvent.type !== event.type)
        ) {
...
function prepareNativeReact(event) {
    const ev = event;
    event.fn = ev;
