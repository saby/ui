...
function getNameFields(props: ITempleWrapperProps): TFields {
    ...
    return [
        ...
        template.prototype?._moduleName || '',
    ];
}
...
function getChangedOptions(_prevProps: ITempleWrapperProps, _nextProps: ITempleWrapperProps) {
...
function getChangedAttributes(_prevProps: ITempleWrapperProps, _nextProps: ITempleWrapperProps) {
...
function getChangedInternal(_prevProps: ITempleWrapperProps, _nextProps: ITempleWrapperProps) {
...
export class TemplateWrapperClass extends React.Component<ITempleWrapperProps> {
    ...
    private shouldComponentUpdateRelease(nextProps: Readonly<ITempleWrapperProps>): boolean {
        ...
        const changedOptions = getChangedOptions(this.props, nextProps);
        if (changedOptions) {
            return true;
        }
        const changedAttributes = getChangedAttributes(this.props, nextProps);
        if (changedAttributes) {
            return true;
        }
        const changedInternal = getChangedInternal(this.props, nextProps);
        if (changedInternal) {
            return true;
        }
        return false;
...
export function createElementForTemplate<P = {}>(
    ...
    const _$stateToCompareProps = {
        ...
        internalVersions: Options.collectObjectVersions(template.internal || {}),
