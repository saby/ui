...
import { IFocusChangedConfig, IFocusAreaProps, FocusArea } from 'UICore/Focus';
...
export default class ReactElementIntoTemplate {
    ...
    private create<P = {}>(
        ...
        const focusAreaEvents = props.attrs._$focusAreaEvents;
        delete props.attrs._$focusAreaEvents;

        if (focusAreaEvents.onActivated) {
            focusAreaProps.onActivated = (cfg: IFocusChangedConfig) => {
                focusAreaEvents.onActivated(fakeFocusSyntheticEvent, cfg);
            };
        }
        if (focusAreaEvents.onDeactivated) {
            focusAreaProps.onDeactivated = (cfg: IFocusChangedConfig) => {
                focusAreaEvents.onDeactivated(fakeFocusSyntheticEvent, cfg);
            };
        }
    ...
    private static createProps<P>(templateScope: P, templateAttrs: ITemplateAttrs): P & TJsxProps {
        let props: P & TJsxProps = ReactElementIntoTemplate.copyFromPrototype(templateScope);
        /* Нужно добавить key,
        ...
        // Нужно протащить до FocusArea, но извлечь до prepareEvents. После использования удалится.
        props.attrs._$focusAreaEvents = {};
        ReactElementIntoTemplate.executeFocusEvents(
            props.attrs._$focusAreaEvents,
    ...
    private static executeFocusEvents(
        focusAreaEvents: Record<string, Function>,
        events: TFocusEventObject
    ): void {
