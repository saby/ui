...
export default class ReactElementIntoTemplate {
    ...
    private static prepareEventArgs(event, item, args) {
        ...
        // TODO: удалить костыль для события onResult, в wasaby контролах не правильная сигнатура обработчика
        // https://online.sbis.ru/opendoc.html?guid=f78abf64-5b08-49e6-b681-a3b3db17b1e6&client=3
        if (item.originalName === 'result' && !(event instanceof SyntheticEvent)) {
            return [event];
        }
