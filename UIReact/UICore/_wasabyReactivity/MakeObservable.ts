...
import {
    ...
    ReactiveUpdateManager,
} from './ReactiveUpdateManager';
import {
    observeArrayChange,
...
function createReactiveUpdateManager(
    instance: TControl
): ReactiveUpdateManager {
    const reactiveUpdateManager = new ReactiveUpdateManager();
    reactiveUpdateManager.setUpdater(instance.controlUpdater);
...
function getReactiveUpdateManager(instance: TControl): ReactiveUpdateManager {
    return (
        controlsUpdateMap.get(instance) || createReactiveUpdateManager(instance)
    );
}
...
function observeMutableTypes(
    ...
    pauseErrorWrongUpdater(() => {
        if (Array.isArray(value)) {
            observeArrayChange(value, instance.controlUpdater, propName);
            return;
        }
...
function observeValue(instance: TControl, propName: string) {
    ...
    Object.defineProperty(instance, propName, {
        ...
        set(newValue: unknown): void {
            ...
            if (
                curValue !== newValue &&
                !(Number.isNaN(curValue) && Number.isNaN(newValue))
            ) {
                ...
                if (this.isCloneOfControl) {
                    ...
                    unobserveMutableTypes(
                        instance.scopeReactiveValues[propName],
                        instance
                    );
