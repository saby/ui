...
function isHTMLElementWithControlObj(
    element: HTMLElement
): element is IHTMLElementWithControlObj {
...
function sortedAddControlObj(
    controls: IControlObj[],
...
function removeControlObj(
    controls: IControlObj[],
    controlToRemove: IControl
): void {
    if (!controls) {
        return;
    }
...
export function removeControlObjFromContainer(
...
export function prepareControls(
    ...
    if (
        isHTMLElementWithControlObj(lastHTMLElement) &&
        lastHTMLElement !== container
    ) {
