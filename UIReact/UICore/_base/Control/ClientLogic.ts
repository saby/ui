...
import { Logger, needToBeCompatible, isDebug, isNewEnvironment } from 'UICommon/Utils';
...
import { applyBodyScroll } from 'UICore/Adaptive';
...
> extends BaseLogic<TOptions, TState> {
    ...
    private errorIfNoContainer(): void {
        if (!this.inst._container) {
            // Достаточно одной ошибки для каждого контрола.
            this.errorIfNoContainer = EMPTY_FUNC;
            Logger.error(
                `Контрол не был смонтирован в DOM-дерево. Скорее всего, ошибка связана с ref.
            Возможные причины проблемы:
            - Вероятно, дело не в ref, а в пустом результате выполнения шаблона. Проверьте результат вызова _template или сам шаблон.
            ...
            - Вы передаете React-компонент в wasaby-контрол в виде контентной опции. И этот React-компонент не прокидывает ref.
    ...
    protected _beforeFirstRender(options: TOptions): void {
        ...
        this._internalOptionsVersions = Options.collectObjectVersions(options._$internal);
    ...
    componentDidMount(checkApiRedefined): void {
        ...
        // @ts-ignore
        if (this.inst.props?.adaptiveMode?.device?.isPhone()) {
    ...
    componentDidUpdate(checkApiRedefined): void {
        ...
        // @ts-ignore
        this._optionsVersions = Options.collectObjectVersions(this.inst._options);
        // @ts-ignore
    ...
    >(
        ...
        if (needHydrate) {
            if (isDebug()) {
                ...
                domElement.innerHTML = domElement.innerHTML.replace(/^\s+|\s+$/g, '');
            }
