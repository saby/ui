...
import { AdaptiveInitializer, applyBodyScroll } from 'UICore/Adaptive';
...
> extends BaseLogic<TOptions, TState> {
    ...
    private errorIfNoContainer(): void {
        if (!this.inst._container && !this.inst._destroyed) {
            ...
            - В корень контентной опции передается несколько корней (например через ws:for), следует создавать только один корневой элемент.
    ...
    componentDidMount(checkApiRedefined): void {
        ...
        // @ts-ignore
        if (detection.isPhone && this.inst.context?.isScrollOnBody) {
    ...
    >(
        ...
        const reactElement = compatible
            ? React.createElement(AdaptiveInitializer, {}, React.createElement(ctor, cfg))
            : React.createElement(ctor, cfg);
