...
import purifyInstance from './Purifier/purifyInstance';
...
> extends BaseLogic<TOptions, TState> {
     ...
     */
    private _optionsVersions: Options.IVersions;
    /**
     * Версии опций для служебных опций контрола.
     */
    private _internalOptionsVersions: Options.IVersions;
    ...
    constructor(inst: Control<TOptions, TState>, context?: TWasabyContext) {
        ...
        this._shouldComponentUpdateDecorator =
            new ShouldComponentUpdateDecorator();
        ...
        // @ts-ignore
        this.inst._children = getProxyChildren.apply(this.inst);
        // Если нет физического родителя - считаем корнем и создаём новый экземпляр.
        this.inst._$wasabyUpdater =
            this._physicParent?._$wasabyUpdater || new WasabyUpdater();
        this.callForceUpdate = (changedFieldName?: string) => {
            this.inst._forceUpdate(changedFieldName);
        };
    ...
    protected _beforeFirstRender(options: TOptions): void {
        ...
        this._optionsVersions = Options.collectObjectVersions(options);
        this._internalOptionsVersions = Options.collectObjectVersions(
            options._$internal
        );
    ...
    protected calculateAttributes(): Record<string, unknown> {
        ...
        if (this.inst._options._$attributes?.refForContainer) {
            ...
            chainOfRef.add(
                new CreateOriginRef(
                    this.inst._options._$attributes.refForContainer
                )
            );
        }
    ...
    activate(cfg: IFocusConfig = {}): boolean {
        ...
        const container = this.inst._container;
        // @ts-ignore
        return !!container && activate(this.inst._container, cfg);
    }
    ...
    componentDidMount(checkApiRedefined): void {
        ...
        const afterMountFn = () => {
            ...
            this._beforeMountDecorator.triggerAfterMount(
                newOptions as { onAfterMount?: Function }
            );
            ...
            this._shouldComponentUpdateDecorator
                ...
                .fireForceUpdate(() => {
                    ...
                    this.inst._forceUpdate('_delayedUpdateDuringMount');
                });
        ...
        const promise = this._beforeMountDecorator
            .waitMyOwnAsyncMount()
            .then(() => {
                if (this.inst._destroyed) {
                    return;
    ...
    shouldComponentUpdate(
        ...
        const { newOptions } = this._prepareDataBeforeUpdate(
            newProps,
            newState
        );
        ...
        const hasChangesState = () => {
            return (
                this.versionStateDecorator.hasChangesState(
                    newState,
                    this.inst.state
        ...
        const hasChangesOptions = () => {
            const oldProps =
                this._optionsForBeforeUpdateBeforeMount || this.inst.props;
            return !!Options.getChangedOptions(
                newProps,
                oldProps,
                newProps._$compound,
        ...
        if (shouldComponentUpdate.needUpdate) {
            this._beforeUpdateRun.exec(
                newOptions,
                shouldComponentUpdate.isOptionsChanged
            );
        }
        ...
        if (shouldComponentUpdate.canContinueExecute) {
            ...
            try {
                // @ts-ignore
                if (
                    this.inst._shouldUpdate === _shouldUpdate ||
                    this.inst._mounted
                ) {
    ...
    componentDidUpdate(checkApiRedefined): void {
        ...
        // @ts-ignore
        this._optionsVersions = Options.collectObjectVersions(
            this.inst._options
        );
        // @ts-ignore
        this._internalOptionsVersions = Options.collectObjectVersions(
    ...
    destroy(): void {
        ...
        try {
            ...
            if (this.inst._mounted) {
                ...
                (this.inst as any).unmountCallback &&
                    (this.inst as any).unmountCallback();
            }
        } catch (err) {
            Logger.error(err.message, this.inst, err);
        ...
        this.inst.controlNodesAfterUnmount = extractControlNodeFromContainer(
            this.inst
        );
    ...
    mountToDom(element: HTMLElement, cfg: TOptions): void {
        ...
        if (!this.VDOMReady) {
            ...
            // @ts-ignore игнорируем совместимость
            ClientLogic.createControl(
                this.inst.constructor,
                cfg,
                this.inst._container
            );
        }
        if (cfg) {
    ...
    private _callBeforeUpdate(
        ...
        try {
            ...
            this.pauseReactive(() => {
                ...
                if (
                    typeof this._optionsForBeforeUpdateBeforeMount !==
                    'undefined'
                ) {
                    // @ts-ignore
                    this.inst._options =
                        this._optionsForBeforeUpdateBeforeMount;
    ...
    >(
        ...
        const compatible = ClientLogic.configureCompatibility(
            domElement,
            cfg,
            ctor
        );
        ...
        const reactElement = React.createElement(ctor, cfg);
        ...
        if (needHydrate) {
            if (isDebug()) {
                // убрать сообщение об ошибке из-за \n в тексте под дебагом
        ...
        const result = ReactDOM.render(reactElement, domElement);

        if (result && result.hasOwnProperty('UNSAFE_isReact')) {
            return result as unknown as Control;
        }
    }
    ...
    static destroyControl(
        ...
        const container: HTMLElement = element.hasOwnProperty('length')
            ? element[0]
            : element;
        ...
        if (rootContainers.has(container)) {
            ...
            try {
                ...
                Logger.error(
                    'Не смогли демонтировать контрол ' + control._moduleName,
                    control
                );
            }
