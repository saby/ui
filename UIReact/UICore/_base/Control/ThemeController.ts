import { EMPTY_THEME } from 'UICommon/theme/controller';
...
export default class ThemeController {
    ...
    constructor(config: IThemeControllerConfig) {
        ...
        this._instThemes = Array.isArray(config.instThemes) ? config.instThemes : [];
        this._instStyles = Array.isArray(config.instStyles) ? config.instStyles : [];
    }
    ...
    updateTheme(themeName?: string): Promise<void> | null {
        ...
        if (isBrowserPlatform && !this._isDeprecatedCSS() && !this._isCSSLoaded()) {
    ...
    private _isDeprecatedCSS(): boolean {
        const isDeprecatedCSS = !!this._instThemes.length || !!this._instStyles.length;
