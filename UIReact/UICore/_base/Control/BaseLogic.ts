...
let wasabyInReactIndex: number = 1;
const rskeyStoreName = 'rskeyStore';
...
> {
    ...
    constructor(inst: Control<TOptions, TState>, context?: TWasabyContext) {
        ...
        if (this.rskey.endsWith(DEFAULT_REACT_RSKEY_ENDING)) {
            const rskeyStore = getStore(rskeyStoreName);
            if (!rskeyStore.get('alreadyWasRender')) {
                // Для каждого нового запроса сбрасываем счётчик.
                rskeyStore.set('alreadyWasRender', 'true');
                wasabyInReactIndex = 1;
            }
