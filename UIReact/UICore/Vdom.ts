...
export const Synchronizer = {
    mountControlToDOM(
        ...
        Logger.warn('В сборке на Реакте нет метода mountControlToDOM, нужно использовать createControl');
    },
    unMountControlFromDOM(control: Control, element: HTMLElement | HTMLElement[]): void {
        Logger.warn('В сборке на Реакте нет метода unMountControlFromDOM, нужно реализовать в другом месте');
    },
...
export const Debug = {
    vdomToHTML(vdom: unknown, context: unknown): Promise<unknown> {
        Logger.warn('В сборке на Реакте нет метода vdomToHTML, нужно реализовать или удалить везде');
    }
}
