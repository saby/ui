...
function prepareBind(
    ...
    const ev = function (eventObj, value: string): void {
        ...
        if (
            ...
            (!eventObj.isBubbling() ||
                (eventObj.isBubbling() && eventObj.nativeEvent.target === eventObj.nativeEvent.currentTarget))
        ) {
            return;
        }
...
function prepareNativeReact(handler: Function, element: HTMLElement, eventName: string, control: Control): void {
...
function clearNotifyListeners(bindedFn, key, clearHandler, notifyEvents) {
    ...
    // итерации const key of notifyEvents.keys() не правильно компилируется в es5
    // поэтому используем forEach
    const _notifyEvents: HTMLElement[] = Array.from(notifyEvents.keys());
    for (const key of _notifyEvents) {
        if (!key.parentNode) {
            notifyEvents.delete(key);
        }
    };
...
function addListener(event, element, eventName, bindedFn, control) {
    ...
    const callbackFn = (e) => {
        const hasInMap = control._$notifyEvents.get(e.currentTarget)?.get(e.type)?.[
            getEventKey(event)
        ];
        if (!hasInMap) {
            return;
        ...
        if (
            ...
            (!bindedFn.control ||
                bindedFn.control._moduleName === 'native React component')
        ) {
            result = bindedFn(...args.slice(1));
        } else {
            if (
                e.type === hasInMap?.event?.originalName?.toLowerCase() &&
                e.detail.control &&
                e.detail.type !== hasInMap.event.originalName
            ) {
                Logger.error(
                    `Ошибка вызова _notify! Вызван _notify("${e.detail.type}"), подписка была добавлена на событие ${hasInMap.event.originalName}. Следует переименовать подписку в шаблоне.
    ...
    mapWorker(event, elementToSubscribe, eventName, callbackFn, control);
}

function mapWorker(event, element: Element, eventName: string, callbackFn: TCallbackFn, control: Control) {
    const name = eventName.slice(3).toLowerCase();
...
function getFromControlMapEvent(eventMap, element: HTMLElement, eventName: string) {
    ...
    const hasEvents = hasElement.get(eventName.toLowerCase());
    if (!hasEvents) {
      return;
    }
    return hasEvents;
...
function getEventHandlers(
    ...
    for (const controlNode of controlNodes) {
        const controlEvent = getFromControlMapEvent(controlNode.control._$notifyEvents, controlElement, eventName);
        if (!controlEvent) {
            continue;
        }
    ...
    const foundHandlers = eventProps.map(function (eventHandler) {
        ...
        const foundIndex =
            eventControl._container === controlElement
                ? getNumberId(eventControl._instId)
                : -1;
        return {
            index: foundIndex,
            eventHandler,
