    ...
    if (event.nativeEvent.isTrusted) {
        ...
        const findIndexFunction = (parent) => {
            return (
                ...
                parent._moduleName === 'Controls/popupTemplateStrategy:Popup' ||
        ...
        for (let i = 0; i < parents.length; i++) {
            ...
            if (!parent._destroyed && parent._$defaultActions?.[key]) {
        ...
        if (!needStop) {
            // вызываем только если не в попапе, в попапе ограничивается только им обработка клавиш
            if (popupIndex === -1 && document.body._$defaultActions?.[key]) {
                ...
                document.body._$defaultActions[key].action(nativeEvent);
                needStop = true;
            }
        }
    }
