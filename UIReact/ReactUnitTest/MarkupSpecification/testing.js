...
define('ReactUnitTest/MarkupSpecification/testing', [
   ...
   function buildFile(
      ...
      return new Promise(function (resolve, reject) {
         require(getFileDependencies(
            test.domain,
            fileName
         ), function (html, standard) {
   ...
   function executeChecker(checker, artifact) {
      switch (checker.type) {
         ...
         case 'error':
            ...
            return checker(
               artifact.executionConfig.isVdom,
               artifact,
               undefined,
               artifact.errors
            );
   ...
   function check(checkers, artifact) {
      if (Array.isArray(checkers)) {
         return checkers.map((checker) => {
            return executeChecker(checker, artifact);
         });
      }
      ...
      if (Array.isArray(checkers)) {
         assert.isFalse(
            checkers.some((checker) => {
               return !checker.isPreventLogger;
            })
         );
         return checkers.map((checker) => {
            return executeChecker(checker, error);
         });
      }
   ...
   function canRun(test) {
      return (
         (test.onlyClient && typeof window !== 'undefined') || !test.onlyClient
      );
   }
