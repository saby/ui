...
describe('UIReact/UICore/_base/Control jsdom', () => {
    ...
    describe('хуки жизненного цикла jest', () => {
        ...
        it('Порядок _afterMount и _afterUpdate хуков в сложном обновлении', async () => {
            ...
            const afterMountSpy = jest.spyOn(
                ControlToCreate.prototype,
                '_afterMount'
            );
            const afterUpdateSpy = jest.spyOn(
                ControlToUpdate.prototype,
                '_afterUpdate'
            );
            ...
            const afterMountLastCall =
                afterMountCallOrder[afterMountCallOrder.length - 1];
            const afterUpdateCallOrder =
                afterUpdateSpy.mock.invocationCallOrder;
            const afterUpdateLastCall =
                afterUpdateCallOrder[afterUpdateCallOrder.length - 1];
    ...
    describe('хуки жизненного цикла', () => {
        ...
        it('_beforeUpdate вызывается до вызова шаблона', async () => {
            ...
            > {
                ...
                render(): React.ReactNode {
                    return (
                        <ControlWithState testOption={this.state.childOption} />
                    );
                }
            }
            // отрисовываем компонент и сразу дожидаемся _afterMount
    ...
    describe('Опции на клиенте', () => {
        ...
        it('undefined удаляется из defaultProps ', async () => {
            ...
            > {
                ...
                render(): React.ReactNode {
                    return (
                        <TestDefaultOptions maxValue={this.state.maxValue} />
                    );
                }
            }
    ...
    describe('Атрибуты', () => {
        ...
        describe('сохраняем в атрибутах className из react props', () => {
            describe('в react вставляем wasaby-контрол без класса на корневой ноде', () => {
                it('прямая вставка', () => {
                    // region Setup
                    const reactControl = React.forwardRef(function ReactControl(
                        props,
                        ref?
                    ) {
                        return (
