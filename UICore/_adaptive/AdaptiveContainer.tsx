...
import { cloneElement, createRef, Ref, RefObject, isValidElement } from 'react';
...
export class AdaptiveContainer extends React.PureComponent<IAdaptiveContainerProps> {
    ...
    private _ref: Ref<HTMLDivElement> | undefined;
    private _containerRef: RefObject<HTMLDivElement> | undefined;
    ...
    constructor(props: IAdaptiveContainerProps) {
        ...
        if (this._isDebug) {
            ...
            this._ref = ChainOfRef.both<HTMLDivElement>(props.forwardedRef, containerRef);
        }
    }
    ...
    checkValues() {
        ...
        const currentContainer = this._containerRef?.current;
        if (this._isDebug && currentContainer?.children.length && !this._errorIsShown) {
            const propWidth = this.props.width || this.props.containerClientWidth;
            if (typeof propWidth === 'number') {
                const correctWidthParent = currentContainer.parentElement?.clientWidth || 0;
                const correctWidthChild = currentContainer.children[0].clientWidth;
                if (
                ...
                const dataWidth = currentContainer.getAttribute('data-width');
            ...
            if (typeof propHeight === 'number') {
                const correctHeightParent = currentContainer.parentElement?.clientHeight || 0;
                const correctHeightChild = currentContainer.children[0].clientHeight;
                if (
                ...
                const dataHeight = currentContainer.getAttribute('data-height');
    ...
    render(): JSX.Element | null {
        ...
        const childrenFromProps = this.props.children;
        const children = !isValidElement<{ ref?: Ref<HTMLDivElement> }>(childrenFromProps)
            ? childrenFromProps
            : cloneElement(childrenFromProps, {
