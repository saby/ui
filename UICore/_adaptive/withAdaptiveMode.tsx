...
import {
    AdaptiveModeContext,
    getDefaultAdaptiveMode,
    TAdaptiveModeContext,
} from './AdaptiveModeContext';
import { AdaptiveModeType } from './AdaptiveModeClass';

function warn(adaptiveModeFromContext: AdaptiveModeType | undefined) {
    if (!adaptiveModeFromContext) {
        logger.warn(
            'UI/Adaptive warn: контекст не был инициализирован. ' +
                'Как следствие, адаптивность будет работать некорректно. ' +
                'Это бывает, когда на странице нет UI/Bootstrap (всталвяется в SbisEnvUI/Bootstrap, ' +
                'на pagex-страницах, либо надо вручную оборачивать корень страницы, в том числе в юнитах и демках). '
        );
    }
}
/**
...
export function withAdaptiveMode<T extends ComponentType>(WrappedComponent: T): T {
    ...
    class ComponentWithAdaptiveMode extends Component<any, any> {
        ...
        render() {
            const adaptiveModeFromContext: AdaptiveModeType = this.context?.adaptiveMode;
            warn(adaptiveModeFromContext);
            ...
            return (
                ...
                <WrappedComponent
                    {...this.props}
                    ref={this.ref}
                    adaptiveMode={adaptiveModeFromContext}
                />
            );
        }
 ...
 * @public
 */
export function useAdaptiveMode(fromAdaptiveInitializer: boolean = false): AdaptiveModeType {
    const adaptiveModeFromContext = useContext(AdaptiveModeContext)?.adaptiveMode;
    if (!fromAdaptiveInitializer) {
        warn(adaptiveModeFromContext);
    }
    return adaptiveModeFromContext ?? getDefaultAdaptiveMode();
}
