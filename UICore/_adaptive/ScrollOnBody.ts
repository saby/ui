...
function preventTouchMoveOnIOS(e: Event) {
    // пока в 5100 этот код откатим, т.к. похоже слайдинг панели не тестировали
    // лучше жить с незначительной ошибкой виде свозного проскрола на ios
    // чем выпускать непротестированный функционал
    // if (!detection.isMobileSafari) {
    //     return;
    // }
    // // на мобильном safari есть странное поведение, когда overflow: hidden не работает
    // // поэтому для SlidingPanel будем блокировать скролл через событие
    // const lastPopup = cachedPopupList.at(cachedPopupList.getCount() - 1);
    // const lastPopupType = lastPopup.controller.TYPE;
    // if (lastPopupType === 'SlidingPanel') {
    //     e.preventDefault();
    // }
}
