// @ts-nocheck Множество API конктретных контролов и использования protected полей. Лучше потом переписать нормально.
/**
...
// проверяем тип последней открытой панели, поведение меняется на ios в зависимости от версии ОС и вида окна
function checkSlidingPanel(): boolean {
    if (!detection.isMobileSafari || detection.IOSVersion < 16) {
        return false;
    }
    if (!cachedPopupList || cachedPopupList?.getCount() === 0) {
    ...
    return lastPopupType === 'SlidingPanel' && !lastPopup.popupOptions.maximize;
}
...
function enableBodyScroll() {
    ...
    document.body.addEventListener('touchstart', detectTouchPosition);
...
function disableBodyScroll() {
    ...
    document.body.removeEventListener('touchstart', detectTouchPosition);
    document.body.removeEventListener('touchmove', preventTouchMoveOnIOS);

    scrollOnBodyState = false;
}

export let scrollOnBodyUnfrozen: boolean | undefined;
/**
...
/**
 * Вызов функции с явной разморозкой скролла на боди переды вызовом коллбэк-функции
 * нужно в случаях когда скролл заморожен (например открыта шторка),
 * но какой-то дествие пользователя должно вызывать скролл страницы
 */
export function withForceScrollUnfreeze(callback: Function) {
    const freezed = !scrollOnBodyUnfrozen;
    if (freezed) {
        scrollOnBodyUnfrozen = true;
    }
    callback();
    scrollOnBodyUnfrozen = freezed;
}
...
function preventTouchMoveOnIOS(e: Event) {
    if (!checkSlidingPanel()) {
        return;
    }
    if (checkHorizontalMove(e)) {
        return;
    }
    e.preventDefault();
}

function stopPropagationTouchMoveOnIOS(e: TouchEvent) {
    if (!checkSlidingPanel()) {
        return;
    }
    if (checkHorizontalMove(e)) {
...
function detectTouchPosition(e: TouchEvent) {
    const touch = e.touches[0];
    startX = touch.clientX;
    startY = touch.clientY;
}

function checkHorizontalMove(e: TouchEvent) {
    const touch = e.touches[0];
