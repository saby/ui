...
export class AdaptiveInitializerInner extends Component<any, IAdaptiveInitializerState> {
    ...
    constructor(props: any, context: any) {
        ...
        // здесь нужно это делать, чтобы isAdaptive был учтен в getAdaptiveModeForLoaders
    ...
    private updateFreeze(_e: unknown, _config: unknown, popupList: any) {
...
export const AdaptiveInitializer = forwardRef(function (props: any, ref) {
...
export function getAdaptiveModeForLoaders() {
    const cache = getStore<Record<string, AdaptiveModeClass | undefined>>(
        'AdaptiveInitializer'
    ).get('getAdaptiveModeForLoaders_cache');
    // метод должен использоваться только при первоначальной загрузке страницы,
    // любые другие случаи должен обслуживать useAdaptiveMode. Так что первый созданный adaptiveMode
    // может быть переиспользован, аспекты не должны измениться при последующих вызовах метода
    if (cache) {
        // на случай если используются метод на клиенте и что-то из аспектов успело измениться
        const newAspects = Storage.getInstance().get();
        const newAdaptiveMode = cache._cloneIfNeed(newAspects);
        getStore<Record<string, AdaptiveModeClass | undefined>>('AdaptiveInitializer').set(
            'getAdaptiveModeForLoaders_cache',
            newAdaptiveMode
        );
        return cache;
    }
    ...
    const adaptiveMode = new AdaptiveModeClass(aspects);
    getStore<Record<string, AdaptiveModeClass | undefined>>('AdaptiveInitializer').set(
        'getAdaptiveModeForLoaders_cache',
        adaptiveMode
    );
