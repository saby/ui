...
export const REACT_FAKE_CONTROL = {
    ...
    _$firedNotifyEvents: new WeakSet(),
};

type TCallbackFn = (e: any) => void;

let REACT_CALLBACK_ID = 0;
function isNativeCallback(event: unknown): event is TReactEvent {
    return typeof event === 'function';
}

function isBindValue(event: any): event is ITemplateBindEvent {
    return 'bindValue' in event;
}

function bindEvent(event: any): void {
...
function bindFn(fn: any, event: any) {
    fn = fn.bind({
        viewController: event.viewController,
        data: event.data,
    });
...
function prepareNativeReact(
    ...
    // в ситуации с вложенностью react => WasabyHOC => react1 => WasabyHOC1 => react2 => WasabyHOC2
    // событие с react пролетает на все хоки через NotifyRef, надо регистрировать только 1 экзеипляр обработчика
    if (!element.__$reactScopeEvents) {
        element.__$reactScopeEvents = new WeakMap();
    }
    const savedControl = element.__$reactScopeEvents.get(handler);
    if (savedControl && savedControl !== control) {
        return;
    }

    let fakeKey: any = 'reactCallback$' + control?._moduleName + '$' + control?._instId;
    if (fakeKey === 'reactCallback$native React component$undefined') {
        fakeKey += '$' + REACT_CALLBACK_ID;
        REACT_CALLBACK_ID += 1;
    }

    addListener(fakeKey, element, eventName, handler, control);
    element.__$reactScopeEvents.set(handler, control);
}

function prepareWasabyEvent(
    event: ITemplateEventBase,
    metaContex: Function | undefined,
    metaHandler: TEventHandler | undefined,
    element: HTMLElement | Control,
    eventName: string,
    control: Control
): TWasabyEvent {
    const ev = function (this: { viewController: any; data: any }, eventObj: SyntheticEvent): void {
        ...
        const preparedContext = (event.context || metaContex) as Function;
        const eventContext = preparedContext.apply(this.viewController);
        const handler = event.handler
            ? event.handler.apply(this.viewController)
            : (metaHandler as TEventHandler).apply(this.viewController, [event.value]);
        ...
        const getAttributes = (args: unknown[]) => {
            let _args = args;
...
function getElementToSubscribe(element: HTMLElement): Element {
    return element.nodeName === 'INVISIBLE-NODE' ? (element.parentNode as Element) : element;
}
...
function clearNotifyListeners(
    bindedFn: any,
    key: string,
    clearHandler: Function,
    notifyEvents: Map<any, any>
) {
...
function eventNameResolver(event: ITemplateEventBase, eventName: string) {
...
function addListener(
    ...
    const elementToSubscribe = getElementToSubscribe(element as HTMLElement) as HTMLElement;
    const callbackFn = (e: any) => {
        ...
        if (
            ...
            // событие не должно само себя вызывать, такое возможно когда событие вешают на зок, который таким же событием стреляет
            if (args.control !== bindedFn.control) {
...
function getFromControlMapEvent(
    eventMap: Map<HTMLElement, Map<string, Record<string, { event?: unknown }>>>,
    element: HTMLElement,
    eventName: string
) {
...
function getEventHandlers(control: Control, controlElement: HTMLElement, eventName: string) {
    const result: {
        okHandlers: unknown[];
        blockHandlers: unknown[];
    } = { okHandlers: [], blockHandlers: [] };
    ...
    const hasEvents: Record<string, { event?: unknown }> = {};
    ...
    for (const _eventName of Object.keys(hasEvents)) {
        // если reactCallback значит колбэк из реакта
        // его в массив добавлять не надо
        if (_eventName.startsWith('reactCallback')) {
            continue;
        }
    ...
    const foundHandlers = eventProps.map(function (eventHandler: any) {
...
// мутирует events
export function resolveEventName(events: unknown) {
    if (!isObjectValue(events)) {
        return;
    }
...
function getArgs<T>(e: any, ..._: any[]): T {
 ...
 */
export function checkWasabyEvent(fn: any) {
    if (!fn) {
        return undefined;
    }
...
type TNodeMap = Map<string, Record<string, { removeListener: Function; event: unknown }>>;
type TNotifyEventsMap = Map<HTMLElement, TNodeMap>;

/**
 * Метод для создания фейкового контрола из реакта
 * делать через {...REACT_FAKE_CONTROL} нельзя, т.к. вложенные объекты не меняются по ссылке
 */
export function createReactFakeControl() {
    return {
        _destroyed: false,
        _mounted: true,
        _moduleName: 'native React component',
        UNSAFE_isReact: true,
        _$notifyEvents: new Map() as TNotifyEventsMap,
        _$firedNotifyEvents: new WeakSet(),
    };
}
