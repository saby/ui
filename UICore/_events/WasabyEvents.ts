...
export default class WasabyEventsReact {
    ...
    setEventHook(
        ...
        const domElement: HTMLElement = (element as any)?._container || element;
    ...
    removeReactNativeEvent(
        element: TElement | Control,
        events: { [key: string]: Function[] | Function }
    ): void {
        const reactScopeEvents: WeakMap<object, unknown> | undefined = (element as any)
            .__$reactScopeEvents;
        // очищаем нативные react-подписки из weakMap,
        // это нужно в случае когда контрол, который вставляет EventSubscriber сразу монтирутется и удаляется
        // и чтобы при повторном маунте такого контрола события снова зарегистрировались,
        // т.к. weakMap мусоросбощиком не успевает очиститься
        if (!reactScopeEvents) {
            return;
        }
        for (const eventName of Object.keys(events)) {
            reactScopeEvents.delete(events[eventName]);
        }
    }
    ...
    static mergeEvents(currentEvents: TEventObject, newEvents: TEventObject): TEventObject {
