...
/**
 * Контекст, содержащий подписки на всплывающие события
 */
...
export default forwardRef(function EventSubscriber(
    ...
    for (const [propName, propValue] of propsEntries) {
        if ((propName as string).search(eventRegexp) !== -1 && typeof propValue === 'function') {
            let originalFn = propValue;
            if (propValue.name.includes('bound ')) {
                const functionKey = propName + propValue.name;
                if (boundFunctionMap.has(functionKey)) {
                    originalFn = boundFunctionMap.get(functionKey);
                } else {
                    boundFunctionMap.set(functionKey, propValue);
                }
            }
            events[propName] = originalFn;
        }
    }
    ...
    const wasabyContextValue = useMemo(() => {
        ...
        if (wasabyContext.bubblingEvents) {
            ...
            for (const item of Object.keys(events)) {
                ...
                if (boundFunctionMap.has(item + events[item].name)) {
                    continue;
                }
    ...
    useEffect(() => {
        return () => {
            ...
            for (const item of Object.keys(events)) {
                ...
                const wasabyContextValue = wasabyContext.bubblingEvents;
                if (wasabyContextValue) {
                    wasabyContextValue[item] = wasabyContextValue[item]?.filter(
                        (handler: Function) => {
                            if (boundFunctionMap.has(item + events[item].name)) {
                                return false;
                            }
                            return handler !== events[item];
                        }
                    );
                    if (wasabyContextValue[item]?.length === 0) {
                        delete wasabyContextValue[item];
                    }
                }
            }
            boundFunctionMap.clear();
        };
    });
    ...
    if (DOMType.includes(children.type) || (rest.fakeDOM && shouldWrap(children))) {
        ...
        // если указан fakeControl, значит обернули div, но события регистрироваться на DOM не должны
        if (rest.fakeDOM || rest.fakeControl) {
            return (
                <WasabyContextManager bubblingEvents={wasabyContextValue}>
                    <BubblingEventContext.Provider value={contextValue}>
                        {cloneElement(children, {
                            ref: rest.fakeControl ? ref : mergeRefs([ref, notifyRef]),
                        })}
