...
import type { IWasabyContextValue } from 'UICore/Contexts';
...
export function notifyFromReact<T>(
    ...
    args?: unknown[],
    bubbling?: boolean,
    wasabyContext?: IWasabyContextValue
): unknown {
    if (!element) {
        return;
    }
    ...
    if (
        bubbling &&
        wasabyContext?.bubblingEvents?.[reactEventName] &&
        !event.detail[0].isStopped()
    ) {
        const handlers = wasabyContext.bubblingEvents[reactEventName];
        for (const handler of handlers) {
            if (event.detail[0].isStopped()) {
                break;
            }
            event.result = handler(...finalArgs);
        }
    }
