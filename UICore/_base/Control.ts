...
import { IControlState, TWasabyOverReactProps, TAnyControl } from './interfaces';
...
// @ts-ignore js файл без ts описания.
...
const Logic: typeof BaseLogic = typeof window === 'undefined' ? ServerLogic : ClientLogic;
    ...
    implements IWasabyControl
{
    ...
    // @ts-ignore хорошо бы написать HTMLElement | null, но страшно сломать чужой ts. Много где используется _container.
    ...
    _moduleName: string;
    /**
     * Название контрола в библиотечной нотаци.
     * Если отсутствует, значит не состоит в библиотеке, или библиотека не реквайрилась
     * (не попали в RequireJsLoader/config)
     * @private
     */
    _$moduleNameInLibrary: string;
    ...
    /**
     * Нужно для переноса _children родителя из оберток над чистым реактом,
     * чтобы работал вызов методов с инстанса контрола
     * @private
     */
    ...
    constructor(props: TOptions | TControlConfig = {}, context?: TWasabyContext) {
    ...
    protected initControl(_props: TOptions | TControlConfig = {}, context?: TWasabyContext) {
     ...
     * @see https://wi.sbis.ru/doc/platform/developmentapl/interface-development/ui-library/control/#life-cycle-phases
     */
    protected _afterMount(_options?: TOptions): void {
        // Do
    }
     ...
     * @see https://wi.sbis.ru/doc/platform/developmentapl/interface-development/ui-library/control/#life-cycle-phases
     */
    protected _componentDidMount(_options?: TOptions): void {
        // Do
    }
     ...
     * @see https://wi.sbis.ru/doc/platform/developmentapl/interface-development/ui-library/control/#life-cycle-phases
     */
    protected _shouldUpdate(_options: TOptions): boolean {
        return true;
    }
     ...
     * @see https://wi.sbis.ru/doc/platform/developmentapl/interface-development/ui-library/control/#life-cycle-phases
     */
    protected _beforeUpdate(_newOptions?: TOptions): void {
        // Do
    }
     ...
     * @see https://wi.sbis.ru/doc/platform/developmentapl/interface-development/ui-library/control/#life-cycle-phases
     */
    protected _afterUpdate(_oldOptions?: TOptions): void {
        // Do
    }
     ...
     * @see https://wi.sbis.ru/doc/platform/developmentapl/interface-development/ui-library/control/#life-cycle-phases
     */
    protected _afterRender(_oldOptions?: TOptions): void {
        // Do
    }
     ...
     * @virtual
     */
    protected _componentDidUpdate(_oldOptions?: TOptions): void {
        // Do
    }
     ...
     * @virtual
     */
    protected _$react_componentDidMount(_oldOptions?: TOptions): void {
        // Do
    }
     ...
     * @virtual
     */
    protected _$react_componentDidUpdate(_oldOptions?: TOptions): void {
        // Do
    }
    ...
    static mixCompatible<TOptions extends TWasabyOverReactProps, TState extends TIState = void>(
    ...
    static destroyControl(control: Control, element: HTMLElement | [HTMLElement]): void {
    ...
    static es5Constructor(this: Control, props: any = {}, context?: any) {
        ...
        //@ts-ignore
        this._$firedNotifyEvents = new WeakSet();
...
function ignoreCases(moduleName: string) {
    let result = false;
    regExps.forEach((re) => {
...
function checkApiRedefined(control: TAnyControl): void {
    if (isUnitTestMode() || !isDebug()) {
        return;
    }
    const redefinedApi = checkReactApiRedefined(control, Control);
    if (redefinedApi.length) {
        ...
        if (redefinedApi[0] === 'contextType') {
            // исключение потому что FED не хочет переписывать и избавляться от ошибку, у них слишком много
            // https://online.sbis.ru/opendoc.html?guid=443cc0c8-1df3-42aa-a3fd-c1246d67c241&client=3
            if (!ignoreCases(control._moduleName)) {
                Logger.error(
                    'Проблема в ' +
                        control._moduleName +
                        '! ' +
                        ...
                        '(readOnly, theme, Router, refForContainer и тп).' +
