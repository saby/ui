// @ts-nocheck слишком много использования приватных полей, а модуль не идёт наружу.
...
function getControlClass() {
    if (!ControlClass) {
        ControlClass = requirejs('UI/Base').Control;
    }
    return ControlClass;
}
...
`
    ...
    componentDidMount(checkApiRedefined): void {
        ...
        const addHooksToQueue = () => {
            ...
            if (this.inst.state.hasError) {
                this.inst._$wasabyUpdater.registerErrorControl(this.inst);
            }
        };
    ...
    componentDidUpdate(checkApiRedefined): void {
        ...
        const fireAsyncHooks = () => {
            ...
            try {
                this.inst._afterUpdate(oldOptions);
            } catch (err) {
                Logger.error(err.message, this.inst, err);
            }
        };
    ...
    destroy(): void {
        ...
        try {
            ...
            } else if (this.inst._beforeUnmount !== getControlClass().prototype._beforeUnmount) {
                Logger.warn(
    ...
    private _callBeforeUpdate(options: TOptions, isOptionsChanged: boolean): void {
        if (!this.inst._mounted) {
            ...
            // если обновление прилетело еще до завершения маунта, надо будет в afterMountFn запланировать
            // принудительное обновление, для этого необходимо взвести флаг в _shouldComponentUpdateDecorator,
            // который позволит перерисовке случиться (а именно _optionsChangedDuringMount=true необходим)
            if (this.inst._moduleName === 'Controls/grid:View') {
                this._beforeUpdateRun.reset();
                const shouldComponentUpdate = this._shouldComponentUpdateDecorator
                    .reset(this.inst._mounted)
                    .addReason({
                        hasChangesOptions: () => true,
                        hasChangesState: () => true,
                        hasChangesAttrs: () => true,
                        hasChangesInternalOptions: () => true,
                    })
                    .exec();
            }
            return;
