...
export default class QueueControlSetStateCaller {
    private controlsToSkip?: Set<TAnyControl> = new Set();
    private plannedUpdates?: Map<TAnyControl, IControlUpdateInfo> = new Map();
    ...
    enqueue(instance: TAnyControl, order: number, setStateHandler: TFunction): void {
        if (this.plannedUpdates && !this.plannedUpdates.has(instance)) {
    ...
    dequeue(instance: TAnyControl): void {
    ...
    executeDelayed(): void {
        ...
        delay(() => {
            if (!this.plannedUpdates || !this.controlsToSkip) {
                return;
            }
    ...
    private sortInstancesFunction(
        first: [TAnyControl, IControlUpdateInfo],
        second: [TAnyControl, IControlUpdateInfo]
    ): number {
