...
export default class WasabyUpdater {
    ...
    planUpdate(instance: TAnyControl, updateHandler: TFunction): void {
        this.createRenderProcess();
        this.renderProcess?.enqueue(instance, this.getDepth(instance), updateHandler);
    }

    registerFirstMountControl(
        instance: TAnyControl,
        parent?: TAnyControl,
        unmountHook?: TFunction
    ): void {
        this.hooksCaller.registerControl(instance, parent, () => {
            if (unmountHook) {
                unmountHook();
            }
    ...
    registerUnmountControl(instance: TAnyControl): void {
        this.hooksCaller.unregisterControl(instance);
    }

    registerBeginRenderControl(instance: TAnyControl): void {
    ...
    registerErrorControl(instance: TAnyControl): void {
        this.hooksCaller.registerErrorRenderControl(instance);
    }
    ...
    private getDepth(sourceInst: TAnyControl): number {
        let depth: number = 0;
        let inst: TAnyControl = sourceInst;
