/**
 * @kaizen_zone 7d860f70-e142-4269-a5a7-7e940b8be4da
 */
import type Control from '../Control';

const reactComponentApi: (keyof Control)[] = [
    'state',
    'setState',
    'props',
    'context',
    'render',
    'shouldComponentUpdate',
    'componentDidMount',
    'componentDidUpdate',
    'componentWillUnmount',
    'getSnapshotBeforeUpdate',
    'forceUpdate',
];
const staticReactComponentApi: (keyof typeof Control)[] = [
    'contextType',
    'UNSAFE_isReact',
    'isWasaby',
];

export function checkReactApiRedefined(control: Control, BaseControl: typeof Control): string[] {
    const api = [];
    // todo отлавливать моменты, когда апи инициализируется прямо в состояние за исключением мест в UI/Base:Control
    for (const apiName of reactComponentApi) {
        if (control.constructor.prototype[apiName] !== BaseControl.prototype[apiName]) {
            api.push(apiName);
        }
    }
    const controlConstructor = control.constructor as typeof Control;
    for (const apiName of staticReactComponentApi) {
        if (controlConstructor[apiName] !== BaseControl[apiName]) {
            api.push(apiName);
        }
    }
    return api;
}
