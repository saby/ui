...
import { forwardRef, type ReactNode, Ref, useCallback } from 'react';
...
function KeyHook(props: IProps, ref: Ref<unknown>) {
    ...
    const catchEvent = useCallback(function catchEvent(event: KeyboardEvent): boolean {
        const key: number = 'which' in event ? (event.which as number) : event.keyCode;
