...
function modifyElementCreatorProps(
    _children: React.ReactElement<IElementCreatorProps, any>,
...
function swap(map: Record<string, string>): Record<string, string> {
...
function convertEvents(props: IControlOptions): TTemplateEventObject {
    const events: TTemplateEventObject = {};
    for (const eventName of reactEventList as (keyof IControlOptions)[]) {
        const eventFromProps = props[eventName];
        if (eventFromProps) {
            events[WASABY_EVENT_NAMES[eventName]] = [eventFromProps];
        }
    }
    if (props.customEvents || props.ignoreEventProps) {
        return { ...events, ...oldCustomEvents(props) };
    }
    const propsKeys = Object.keys(props) as (keyof IControlOptions)[];
...
function oldCustomEvents(props: IControlOptions): TTemplateEventObject {
    const events: TTemplateEventObject = {};
    if (!props.customEvents) {
        return events;
    }
    for (const eventName of props.customEvents as (keyof IControlOptions)[]) {
...
function checkEventException(eventName: string) {
