...
import { controller as EcosystemController } from 'I18n/i18n';
...
export interface IAsyncOptions extends IControlOptions {
    templateName: string;
    templateOptions: IControlOptions & object;
...
export default abstract class Async extends Control<IAsyncOptions, TAsyncStateReceived> {
    ...
    private ecosystemName: string = EcosystemController.currentLocaleConfig.ecosystem.name;
    protected defaultErrorMessage: string = `У ${this.ecosystemName} возникла проблема`;
    /**
    ...
    protected _beforeMount(
        ...
        _: unknown,
        receivedError: boolean | undefined
    ): Promise<TAsyncStateReceived | void> | TAsyncStateReceived | void {
        ...
        if (constants.isBrowserPlatform && !ModulesLoader.isLoaded(options.templateName)) {
            const loadingContent = this._loadContentAsync(
                options.templateName,
                options.templateOptions
            );
            if (options.isDemo) {
                // только в демке меняем поведение при оживлении на клиенте так,
                // чтобы при оживлении было построено то же самое, что на СП.
                if (receivedError) {
        ...
        // только в демке возвращаем признак ошибки построения на СП
        if (options.isDemo) {
            return this.error;
        }
    }
    ...
    protected _loadSync<T = unknown>(name: string): T | null {
        try {
    ...
    protected _insertComponent(tpl: unknown, options: IControlOptions, templateName: string): void {
        ...
        const opts = options || {};
        const optsKeys = Object.keys(opts) as (keyof IControlOptions)[];
        for (const key of optsKeys) {
