 ...
 */
import { useContext, ComponentType, createElement, forwardRef, Ref } from 'react';
...
export function withTheme<T extends IThemableComponent = IThemableComponent>(
    ...
    const ComponentWithTheme = (props: Optional<T, keyof IThemableComponent>, ref: Ref<any>) => {
    ...
    return forwardRef(ComponentWithTheme);
}

/**
 * Хук для получения значения темы. Если компонент может вставляться в Wasaby шаблон, нужно передать props.
 * @public
 */
