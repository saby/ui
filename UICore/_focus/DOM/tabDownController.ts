...
export default function allowNativeEventForControl(control: unknown): boolean {
    return (
        control.hasOwnProperty(allowNativeEventFlagName) &&
        control[allowNativeEventFlagName]
    );
}
...
class TabDownController {
    ...
    addTabDownHandler(element: HTMLElement): void {
        if (!this.elementsSet.has(element)) {
            ...
            element.parentNode?.addEventListener('keydown', this.handler);
        }
    }
    ...
    removeTabDownHandler(element: HTMLElement): void {
        if (this.elementsSet.has(element)) {
            ...
            element.parentNode?.removeEventListener('keydown', this.handler);
        }
    }
    ...
    private handler(event: KeyboardEvent): void {
        ...
        if (
            getParentFocusComponents(fromElement).find(
                allowNativeEventForControl
            )
        ) {
            return;
        }
