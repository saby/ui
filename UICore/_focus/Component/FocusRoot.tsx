...
import { IFocusAreaProps, focusParentRefName, IFocusAreaElement } from './FocusArea';
...
/**
 * Контрол выделяет область фокусировки. В этом области дочерние контролы могут:
 * - Перемещать фокус при нажатии на tab и задавать локальный порядок обхода по табиндексу.
 * - Устанавливать фокус на дочерний контрол с помощью
 * - Ловить события получения и потери фокуса
 * @remark
 * {@link https://n.sbis.ru/article/ff0c466c-2b19-4e61-9bf1-d44df43802d4 Статья по использованию}
 * @class UICore/Focus:FocusRoot
 * @public
...
export default forwardRef(function FocusRoot(
    ...
    const focusRootRef = useCallback(
        function focusRef(root: IFocusAreaElement): void {
            if (root) {
