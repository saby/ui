...
export default function callFocusChangedCallbacks(
    ...
    const fromParents: TFocusComponent[] = getParentFocusComponents(fromElement);
    ...
    const compatFocusChangedConfig = {
        // @ts-ignore _$to используется в Controls/_compatiblePopup/CompoundAreaForNewTpl/CompoundArea
        _$to: toParents.find((parent) => !!parent._notify),
        ...focusChangedConfig,
    };

    for (let i = 0; i < uniqueFromParents.length; i++) {
        callDeactivatedCallback(uniqueFromParents[i], uniqueFromParents[i - 1], focusChangedConfig);
    }
    for (let i = 0; i < uniqueToParents.length; i++) {
        callActivatedCallback(uniqueToParents[i], uniqueToParents[i - 1], compatFocusChangedConfig);
    }
}
