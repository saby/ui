...
function useCreateFocusCallbacksRef(config: {
    ...
    const createFocusCallbacks = useRef<CreateFocusCallbacksRef>();
    if (!createFocusCallbacks.current) {
        return (createFocusCallbacks.current = new CreateFocusCallbacksRef(
            onActivated,
            onDeactivated
        ));
    }
    const createFocusCallbacksRef = createFocusCallbacks.current;
    if (createFocusCallbacksRef.getOnActivated() !== onActivated) {
        createFocusCallbacksRef.setOnActivated(onActivated);
    }
    if (createFocusCallbacksRef.getOnDeactivated() !== onDeactivated) {
        createFocusCallbacksRef.setOnDeactivated(onDeactivated);
    }
    return createFocusCallbacksRef;
}
