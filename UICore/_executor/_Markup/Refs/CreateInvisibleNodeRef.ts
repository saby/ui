...
interface IPathcedNode extends HTMLElement {
...
export class CreateInvisibleNodeRef extends Responsibility {
            ...
            for (const key of Object.keys(events)) {
                ...
                for (const item of eData) {
                    ...
                    const fn = (e) => {
                        ...
                        return item.viewController[item.value].apply(item.viewController, args);
                    };
                    ...
                    if (typeof item.viewController === 'undefined') {
                        continue;
                    }
                    if (!item.viewController._$needRemoveBeforeUnmount) {
                        item.viewController._$needRemoveBeforeUnmount = [];
                    }
                    item.viewController._$needRemoveBeforeUnmount.push(removeNotifyListener);
                }
            }
        }
    }

    private transferControlNodes(node: IPathcedNode, parent: IPathcedNode): void {
    ...
    private patchControlNode(controlNode: IControlNode, newContainer: IPathcedNode): void {
        controlNode.element = newContainer;
    }

    private patchControl(controlNode: IControlNode, newContainer: IPathcedNode): void {
