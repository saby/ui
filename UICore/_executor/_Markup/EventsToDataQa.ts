...
import type { ITemplateBindEvent, ITemplateEventBase } from 'UICommon/Events';
...
export function addDataQaEventAttributes(
    ...
    let fullEvents: TTemplateEventObject = {};
    ...
    for (const eventName of Object.keys(fullEvents)) {
        ...
        for (const eventHandler of fullEvents[eventName]) {
            ...
            if (isTemplateBindEvent(eventHandler)) {
                const attrName = eventName.replace('on:', 'data-qa-').toLowerCase();
...
function isTemplateBindEvent(
    eventHandler: ITemplateEventBase | ITemplateBindEvent | Function
): eventHandler is ITemplateBindEvent {
    return eventHandler.hasOwnProperty('data');
}
