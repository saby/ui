...
import type { TranslatableString } from 'I18n/i18n';
import {
    reactEventList,
...
import { isForwardRef, isReactComponentType, isValidChildren } from './ReactTypesChecker';
...
export class WasabyDOMRefGeneration {
    ...
    generate(events: TEventObject, props: any, needActivator: boolean) {
        ...
        if (needActivator && props.name && props._logicParent?.childrenRefsCreator) {
            const prevRef = props._logicParent.childrenRefsCreator.getRef(props.name);
            const childrenRef =
                prevRef && prevRef.getWithActivator()
                    ? prevRef
                    : props._logicParent.childrenRefsCreator.createRef(props.name, true);
            chainOfRef.add(childrenRef);
            // todo раз уж тут добавляется реф, который вешает активатор,
            //  значит надо избавиться от такого же рефа (но без активатора) сгенеренного в генераторе выше для контрола
            //  он прилетает в составе forwardedRef и надо выяснить где он там находится и как его удалить
            //  здесь он просто добавляется всегда, так что в результате стреляет 2 зануления рефа и 2 навешивания элемента.
            //  сейчас реф сверху (без активатора) не выполняется потому что дизаблится по имени.
            //  он дизаблится потому что реф с активатором стреляет раньше
        }
...
export default class ReactElementIntoTemplate {
    ...
    private create<P = {}>(
        ...
        const moduleName = component.displayName || component.name || 'unknown';
        ...
        if (!isVdom) {
            try {
                ...
                const physicParentName = props._physicParent?._moduleName;
                const logicParentName = props._logicParent?._moduleName;
                const message =
                    `Ошибка при построении компонента ${moduleName}. Физический родитель ${physicParentName}. Логический родитель ${logicParentName}.\n\t` +
                    'В стеке ошибки более подробная информация о реально упавшем компоненте.';
    ...
    private createWasabyRef(
        ...
        return (
            new WasabyDOMRefGeneration(templateAttrs.refForContainer, forwardedRef)
                // @ts-ignore
                .generate(templateAttrs.events, props, needActivator)
        );
    }
    ...
    private prepareChildren(props: IControlOptions & TJsxProps): void {
        ...
        if (
            ...
            (!props.children ||
                (isValidChildren(props.children) && props.children.type === ContentAsChildren))
        ) {
    ...
    createChildrenAsContent<P = {}>(
        component: React.ForwardRefExoticComponent<P & IControlOptions>,
        props: P & IControlOptions & TJsxProps,
        templateAttrs: ITemplateAttrs,
        isVdom: boolean
    ) {
        ...
        finalProps.forwardedRef = this.createWasabyRef(
            templateAttrs,
            finalProps,
    ...
    private static prepareEvents<P>(props: P, eventData: Record<string, TWasabyEvent[]>): P {
        ...
        for (const key of events) {
                ...
                for (const item of eData) {
                    ...
                    try {
                        ...
                        if (context.UNSAFE_isReact && !isWasabyEventArgs) {
                            this.wrongCustomEventArguments(reactEventName, props._physicParent);
                        }
            ...
            // если реакт компонент обернут в wasabyHOC и одновременно передается prop.onEvent и в хоке добавляется подписка on:event
            // надо в react передать не prop.onEvent, а обертку, которая вызовет и prop.onEvent и подписку с хока
            if (props[reactEventName] && eData.length > 1) {
                props[reactEventName] = eventCallbackFn;
            }
            // проверяем, что prop.onEvent является честным react-колбэком
            if (
                props[reactEventName] &&
                !props[reactEventName].isWasabyEventCallback &&
                reactEventList.indexOf(reactEventName)
            ) {
                const oldEventName = getOldReactEventName(reactEventName);
                props[oldEventName] = (event, ...args) => {
                    this.wrongCustomEventArguments(reactEventName, props._physicParent);
                    eventCallbackFn(event, ...args);
                };
                props[oldEventName].generatedFromCode = true;
                continue;
            }
            props[reactEventName] = eventCallbackFn;
...
if (typeof window === 'undefined') {
    ReactElementIntoTemplate.convertStringChildren = function <P = {}>(
        ...
        if (isRkString(children)) {
            props.children = children.toLocaleString();
            return props;
        }
        if (Array.isArray(children)) {
            for (let i = 0; i < children.length; i++) {
                ...
                if (isRkString(child)) {
                    children[i] = child.toLocaleString();
                }
            }
        }
        return props;
...
function isRkString(child: unknown): child is TranslatableString {
    return !!child && (child as { i18n: boolean }).i18n === true;
}
