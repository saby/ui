...
import {
    ...
    ITplFunction,
    IGeneratorConfig,
    extendFromViewController,
} from 'UICommon/Executor';
import * as ModulesLoader from 'WasabyLoader/ModulesLoader';
import { TWasabyEvent, TTemplateEventObject, wasabyEventIntersection } from 'UICore/Events';
...
import ReactElementIntoTemplate from 'UICore/_executor/_Markup/Creator/ReactComponent';
import {
    isReactElement,
    isComponentClass,
    isForwardRef,
    ...
    isContextConsumer,
    isContextProvider,
} from 'UICore/_executor/_Markup/Creator/ReactTypesChecker';
...
export class Generator implements IGenerator {
    ...
    calcTpl(origin: TOrigin, config: IControlConfig): TtplType {
        const tpl: TtplType | Common.IDefaultExport<TtplType> = resolveTpl(
            origin,
            config.includedTemplates,
            config.depsLocal
        );

        if (Common.isDefaultExport(tpl)) {
            // @ts-ignore
            return tpl.default;
        }
        // @ts-ignore
    ...
    createControlNew<R = JSX.Element | string>(
        ...
        let fullEvents = {};
        if (!isServerSide) {
            const renamedEvents: TTemplateEventObject = calcOriginEventName(events);
            if (config?.attr?.events && config.mergeType === 'attribute') {
                fullEvents = Attr.mergeEvents(config.attr.events, renamedEvents);
            } else {
                fullEvents = renamedEvents;
            }
        }

        let tpl = this.calcTpl(origin, config);
        ...
        // опции.
        const resolvedOptionsExtended = ConfigResolver.clearedContentOptionScope(
            resolvedOptions,
            config
        );
        /*
        ...
        // Это поле потом удалим в ReactComponent, т.к. этот костыль нужен только в Wasaby-контролах.
        if (newOptions.key !== undefined && needSaveKey(tpl)) {
            newOptions._$key = newOptions.key;
        }

        const isChildrenAsContent = tpl?.isChildrenAsContent;
        const hasChildrenAsContent = newOptions.content?.isChildrenAsContent;
        if (isChildrenAsContent || hasChildrenAsContent) {
            ...
            newOptions.children =
                newOptions.children || config.data.children || config.data.props?.children;
            if (isChildrenAsContent && !newOptions.children) {
                Logger.error(
                    'При построении контентной опции ' +
                        'wasaby вставленной в чистом реакте должен найтись корректный children',
                    logicParent
                );
            }
        }
        ...
        if (isContextConsumer(tpl)) {
            return React.createElement(tpl, {}, options.children);
        }

        if (isContextProvider(tpl)) {
            return this.reactCreator.createContextProvider(tpl, newOptions, !!config.isVdom);
        }
        ...
        if (isChildrenAsContent) {
            return this.reactCreator.createChildrenAsContent(
                tpl,
                newOptions,
                templateAttributes,
                !!config.isVdom
        ...
        // функциональные компоненты на чистом реакте (в том числе обёрнутые в memo и/или forwardRef)
        if (isFunctionComponent(tpl) || isForwardRef(tpl)) {
    ...
    protected createWsControl(
        ...
        config: IControlConfig
    ): string | React.ComponentElement<IControlOptions, Control<IControlOptions, object>> {
        return this.processControl(createWsControl(tpl, newOptions, templateAttributes, config));
    }
...
function needSaveKey(tpl: TtplType): boolean {
    if (tpl && tpl.prototype && tpl.prototype._moduleName === 'Controls/Container/Async') {
        return false;
    }
    return true;
}
...
export function resolveTemplateArray(
    ...
    templateArray.forEach(
        (template: TemplateFunction | ITplFunction<TemplateFunction>, index: number) => {
...
function resolveTemplate(
    template: TemplateFunction | ITplFunction<TemplateFunction> | React.FunctionComponent,
    logicParent: Control<IControlOptions>,
    physicParent: Control<IControlOptions>,
    resolvedScope: IControlOptions,
    templateAttributes: IGeneratorAttrs,
...
function createWsControl(
    ...
    if (scope._$compound) {
        if (origin.prototype._moduleName !== 'Core/CompoundContainer' && !isUnitTestMode()) {
            Logger.error(
    ...
    let flatScope = extendFromViewController(config, scope);
...
function createTemplate(
    ...
    if (typeof scope.readOnly === 'undefined') {
        scope.readOnly = logicParent?.props?.readOnly ?? logicParent?.context?.readOnly;
    }
...
export function logResolverError(tpl: TemplateOrigin, parent: Control<IControlOptions>): void {
    ...
    if (typeof tpl === 'string' && tpl.split('!')[0] === 'wml') {
        ...
        Logger.error(
            'Ошибка при построение контрола. Проверьте существует ли шаблон ' + tpl,
            parent
        );
    }
}
...
export function calcOriginEventName(events): TTemplateEventObject {
    const renamedEvents: TTemplateEventObject = {};
    if (!!events) {
        // В процессе mergeEvents, например в addDataQaEventAttributes под кукой, meta станет enumerable: false.
        // Сдедовательно, не попадает в Object.keys. Добьёмся, чтобы и в этом случае не терять её.
        let hasEvents: boolean = false;
        const resolveResolvedEvents = (renamedEvents, events, name, eventName) => {
            if (renamedEvents[name]) {
                renamedEvents[name].push(...events[eventName]);
                return;
            }
            const lowerCaseName = eventName.toLowerCase();
            if (name !== eventName.toLowerCase() && events[name] && events[lowerCaseName]) {
                renamedEvents[name] = [...events[lowerCaseName]];
                events[lowerCaseName].skip = true;
                renamedEvents[name].push(...events[eventName]);
                return;
            }
            renamedEvents[name] = events[eventName];
        };
        for (const eventName of Object.keys(events)) {
            if (eventName === 'meta') {
                continue;
            }
            if (events[eventName].skip) {
                continue;
            }
            hasEvents = true;
            if (wasabyEventIntersection[eventName]) {
                renamedEvents[eventName] = events[eventName];
                continue;
            }
            // за счет этого преобразования работает вызов бинда как колбэк-функции из реатка
            // необходимость передвать объект синтетичского события определяется с помощью checkWasabyEvent
            if (eventName.includes('bind') || events[eventName][0].originalName?.includes('bind')) {
                // преобразуем bind:value в on:valueChanged
                const name = 'on:' + events[eventName][0].originalName.slice(5) + 'Changed';
                renamedEvents[name] = events[eventName];
                continue;
            }
            const name = events[eventName][0].originalName
                ? 'on:' + events[eventName][0].originalName
                : eventName;
            // возможна ситуация когда на контроле несколько событий с одинаковым именем, но в разном регистре
            // пока не будет изменена кодогенерация надо не потерять событие из шаблона
            resolveResolvedEvents(renamedEvents, events, name, eventName);
        }
        if (hasEvents && events.meta) {
            renamedEvents.meta = events.meta;
        }
    }
