...
export class CreateTagVdom implements IGeneratorComponent {
    ...
    create<T extends HTMLElement, P extends React.HTMLAttributes<T>>(
        ...
        if (!isServerSide) {
            ...
            if (typeof name === 'string' && name.length > 0 && !!control?.childrenRefsCreator) {
                const childrenRef =
                    control.childrenRefsCreator.getRef(name) ??
                    control.childrenRefsCreator.createRef(name);
...
// избегаем циклической зависиомсти с UICore/Events
