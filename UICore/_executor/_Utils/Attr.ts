...
/**
 * Мержит атрибуты типа object
 * @param parentAttrs
 * @param ownAttrs
 * @param name
 * @returns {object} смерженные атрибуты
 */
function mergeStyle(parentAttrs: IObjectAttributes, ownAttrs: IObjectAttributes): object {
    ...
    if (res) {
        const invalidKeys = Object.keys(res).filter((name) => {
            ...
            // В react валидно задавать размеры числом.
            if (!value.trim) {
                return false;
            }
            return !value.trim();
        });
        // react замораживает свои props, в wasaby для свойства style мы должны отдать чистый объект, без пустых свойств
        // поэтому если прикладники сделали бутерброд react-reactHOC-wasaby-reactHOC и прокинули через весь слой свойство style
        // мы должны разморозить объект style перед его очисткой
        if (invalidKeys && Object.isFrozen(res)) {
            res = { ...res };
        }
