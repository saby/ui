...
interface IVersionalHack extends IVersional {
    _version: number;
}
...
export function observeVersionChange(
    ...
    const reactiveUpdateCaller = getReactiveUpdateCaller(versionObject);
    ...
    if (reactiveUpdateCaller.setUpdater(callback, componentName)) {
        let currentValue = (versionObject as IVersionalHack)._version;
...
function getReactiveUpdateCaller(versionObject: IVersional): ReactiveUpdateManager {
    const reactiveUpdateCallerFromMap = versionChangeUpdateMap.get(versionObject);
    if (reactiveUpdateCallerFromMap) {
        return reactiveUpdateCallerFromMap;
    }
    const reactiveUpdateCallerNew = new ReactiveUpdateManager();
    versionChangeUpdateMap.set(versionObject, reactiveUpdateCallerNew);
...
export function unobserveVersionChange(
    ...
    if (reactiveUpdateCaller.unsetUpdater(callback)) {
        const currentValue = (versionObject as IVersionalHack)._version;
        Object.defineProperty(versionObject, '_version', {
            enumerable: true,
            configurable: true,
            writable: true,
