import { Head as AppHead, JSLinks, IPageTagAttrs } from 'Application/Page';
import type { JML } from 'Application/Page';
import { getResourceUrl, jsEscape, escapeHtml } from 'UI/Utils';
 ...
 * @returns
 */
const prepareMetaScriptsAndLinks = (
    tag: string,
    attrs: object
): { tag: string; attrs: IPageTagAttrs } => {
    return {
        tag,
        attrs,
    };
...
export function createMetaScriptsAndLinks(cfg: IHeadOptions): void {
    ...
    ([] as { tag: string; attrs: IPageTagAttrs }[])
        .concat(
            (cfg.meta || []).map((attr) => {
        ...
        .forEach((item: { tag: string; attrs: IPageTagAttrs }) => {
            (['href', 'src'] as const).forEach((field) => {
                const urlAttr = item.attrs[field];
                if (urlAttr && item.attrs.rel !== REL_CANONICAL) {
                    item.attrs[field] = getResourceUrl(urlAttr);
                }
            });
...
export function applyHeadJson(json: JML[]): void {
    ...
    json.forEach((data) => {
        const tag = data[0];
        let attrs: IPageTagAttrs = typeof data[1] === 'object' ? data[1] : {};
        ...
        const attrsKeys = Object.keys(attrs) as (keyof IPageTagAttrs)[];
        for (const attrsKey of attrsKeys) {
            const attrsValue = attrs[attrsKey];
            if (attrsValue) {
                if (attrsKey === 'href' || attrsKey === 'src') {
                    attrs[attrsKey] = getResourceUrl(attrsValue);
                    continue;
                }
                attrs[attrsKey] = escapeHtml(attrsValue);
            }
        }
