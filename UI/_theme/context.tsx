...
import { default as getThemeStorage } from './ThemeStorage';
...
// isUpperContext может быть только один
const FIRST_DEFAULT: IThemeContext = {
    update: () => undefined,
};
 ...
 *
 */
export const ThemeContext: Context<IThemeContext> = createContext(FIRST_DEFAULT);

export function useIsUpperScope() {
    const themeContext = useContext(ThemeContext);
    const isUpperScope = themeContext === FIRST_DEFAULT;
    return isUpperScope;
}
...
function ThemeScope(props: IThemeProps & { updateVer: number }): JSX.Element {
    ...
    if (firstPalleteTheme) {
        themeStorage.saveTheme(firstPalleteTheme);
    }

    const isFirstScope = contextValue === FIRST_DEFAULT;

    const value: IThemeContext = useMemo(() => {
        if (!firstPalleteTheme) {
            return isFirstScope ? DEFAULT : contextValue;
        }
        return {
    ...
    if (
        props.staticThemeName &&
        !fullClassList.includes('controls_theme-' + props.staticThemeName)
    ) {
        fullClassList.unshift('controls_theme-' + props.staticThemeName);
    }

    // хук только для того что бы повесить классы на body
    const classList = useThemeClassName(fullClassList, props.isUpperScope ?? false);
    ...
    /* в случаях:
        ...
        - не передан фон
    */
    let classes: string[] = isReactElement(props.children) ? [props.children.props.className] : [];
