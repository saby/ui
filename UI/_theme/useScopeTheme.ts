...
export const GENERATED_THEME = 'g-';

/**
 * Хук для вставки css темы.
 * Отвечает за загрузку темы, если тема была создана через дизайнер загрузка не производится
 * @param activeTheme
 * @returns [firstPalleteTheme, fullClassList] - [цветовая палитра, полный список классов тем]
 * @private
 */
export function useScopeTheme(
    ...
    useThemeEffect(() => {
        ...
        for (const theme of themeArr) {
            ...
            if (!selector || selector.includes(GENERATED_THEME)) {
                continue;
            }
