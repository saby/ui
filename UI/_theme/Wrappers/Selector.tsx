...
export default function ThemeWrapperSelector({
    selector,
    children,
    isUpperScope,
    ...
    useEffect(() => {
        ...
        Promise.all(
            ...
            // при загрузке темы может быть недоступен сервис тем,
            // надо защититься от бесконечного обращения к сервису тем
            const hasError = result.some((element) => {
                return element.error;
            });
            if (hasError) {
                return _version;
            }
            return setVersion((prevVersion) => prevVersion + 1);
        });
    });
