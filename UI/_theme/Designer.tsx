...
import { GENERATED_THEME } from './useScopeTheme';
...
const ThemeDesigner = forwardRef(function ThemeDesigner(props: IProps, ref: Ref<HTMLElement>) {
    ...
    const activeTheme = useMemo<IActiveTheme>((): IActiveTheme => {
        ...
        const fullThemeProperties = {
            ...themeProperties,
            ...props.colorSchemes,
        };
        const theme: IActiveTheme = {
            properties: Model.fromObject<IActiveTheme>(themeProperties),
            themeApply: THEME_APPLICATIONS.palette,
        };
        const needsNewSelector = !props.selector || props.selector.startsWith(GENERATED_THEME);
        const newSelector = needsNewSelector
            ? generateUniqueID(fullThemeProperties, GENERATED_THEME)
            : props.selector;

        theme.selector = newSelector;
        theme.version = Math.random();
    ...
    return (
        <ThemeScope activeTheme={activeTheme} noBackground1193214061={true}>
            <Background {...props} selector={activeTheme.selector} ref={ref as Ref<HTMLDivElement>}>
                {props.children}
            </Background>
        </ThemeScope>
    );
