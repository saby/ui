...
/**
 * Получение первого цвета градиента
 * @private
 */
...
export function calculateVaribalesFromThemeProperties(
    ...
    const { background, url_full: urlFull, logo, picture, texture, dominantColorRGB } = { ...themeProperties };
    ...
    const backgroundColor = background || (dominantColorRGB ? `rgb(${dominantColorRGB})` : void 0);
    if (backgroundColor) {
        styleObj['--brandbook_background-color'] = backgroundColor;
    }
