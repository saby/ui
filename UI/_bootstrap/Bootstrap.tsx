...
interface IBootstrapProps extends TInternalProps {
    ...
    loadHotKeys: () => Promise<Record<string, IActionStartOptions>>;
}
...
export default class Bootstrap extends React.Component<IBootstrapProps, IBootstrapState> {
    ...
    private actionHotKeys: Record<string, IActionStartOptions> = {};
    ...
    componentDidMount(): void {
        ...
        this.props.loadHotKeys?.().then((actionHotKeys) => {
    ...
    actionHotKeysHandler(event: React.KeyboardEvent<HTMLElement>): boolean {
        ...
        if (this.actionHotKeys[keys]) {
            const actionStarter = new ActionStarter(this.actionHotKeys[keys]);
            actionStarter.start();
            return true;
        }
        return false;
