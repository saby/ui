...
function ThemeScope(props: IThemeProps & { updateVer: number }): JSX.Element {
    ...
    const value: IThemeContext = useMemo(
        ...
        [firstPalleteTheme, themeContext]
    );
    ...
    let className = useThemeClassName(fullClassList, childrenClassName);
    ...
    return (
        <ThemeContext.Provider value={value}>
            {firstPalleteTheme.properties?.has('background') ? (
                <ThemeVariablesCompat activeTheme={firstPalleteTheme}>
                    {children}
                </ThemeVariablesCompat>
            ) : (
                children
            )}
...
function ThemeVariablesCompat(props: IThemeVariablesCompatProps): JSX.Element {
    ...
    const compatClassName = useRef<string | undefined>();
    useMemo(() => {
        ...
        if (!themePropertiesRecord || !classList?.length) {
            ...
            compatClassName.current = undefined;
            return;
        }
        const themeProperties: TThemePropertiesObject = factory(themePropertiesRecord).toObject();
        let styleStr = `.${classList.join('.')}{${calculateVaribalesFromThemeProperties(
            themeProperties
        )}}`;
        const bgStyledClassName = generateBackgroundStyledClassName(classList[0], themeProperties);
        if (bgStyledClassName) {
            styleStr += bgStyledClassName;
            compatClassName.current = ` ${COMPAT_BG_CLASS_NAME} tw-h-full`;
        }
    ...
    if (!compatClassName.current) {
        return props.children;
    }

    const childrenClassName = isReactElement(props.children) ? props.children.props.className : '';
    return cloneElement(props.children as ReactElement, {
        className: childrenClassName + compatClassName.current,
    });
}
...
const COMPAT_BG_CLASS_NAME = 'themeWrapper-compat_background';

/**
 * Формируем класс со стилем для фона
 */
function generateBackgroundStyledClassName(
    themeClassName: string,
    themeProperties: TThemePropertiesObject
): string {
    if (!themeProperties.background) {
        return '';
    }
    return (
        `.${themeClassName}.${COMPAT_BG_CLASS_NAME}{background-color:var(--brandbook_background-color);` +
        'background-image:var(--brandbook_background-image);}'
    );
}
