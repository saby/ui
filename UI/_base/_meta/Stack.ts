...
export default class Stack implements IMetaStackInternal {
    ...
    serialize(): string {
        const ser = this.getStore()
            .getKeys()
            .map((id) => {
                return (this.getStateById(id) as IMetaStateInternal).serialize();
            });
    ...
    private getStateById(id: string | undefined): IMetaStateInternal | null {
        return (id && this.getStore().get(id)) || null;
    }
     ...
     * @private
     */
    static restore(str: ISerializedMetaStack): IMetaStackInternal | null {
        if (!str) {
            return null;
        }
        const states: IStates = {};
        try {
            const statesArrBeforeFilter = (JSON.parse(str) as ISerializedMetaState[]).map(
                (stateSer) => {
                    return State.deserialize(stateSer);
                }
            );
            // ts сам не понимает, что после фильтра null не останется.
            const statesArr = statesArrBeforeFilter.filter((state) => {
                return state !== null;
            }) as IMetaStateInternal[];
