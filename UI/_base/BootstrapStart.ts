...
export default function startFunction(
    ...
    loadAsync<IControlConstructor<IControlOptions>>(moduleName).then((module): void => {
...
function hydrateState(previousAspects: IAspects): boolean {
    ...
    // если размеры outerWidth и innerWidth достаточно сильно отличаются - страница зумирована.
    // просто сравнить их нельзя, они могут немного отличаться из-за ширины вертикального скролла.
    // по какой-то причине на сервере зум может быть не учтен, на сервере построится неправильно, упадет гидратация.
    // это временный костыль с отключением гидратации при зуме, пока мы разбираемся с проблемой
    // https://online.sbis.ru/opendoc.html?guid=554f3f37-5c92-47ce-b60d-d266ec4fcd63&client=3
    // upd проверка на ширину window не всегда срабатывает для определения zoom,
    // поэтому еще проверим на pixelRatio, без зума число должно быть целым
    if (Math.abs(window.outerWidth - window.innerWidth) > 20 || window.devicePixelRatio % 1 !== 0) {
        return false;
    }
