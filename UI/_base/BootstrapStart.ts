...
function createControl(
    ...
    // на сервере
    if (needHydrate && document.body.querySelectorAll('[name="$$wasaby$async$loading"]').length) {
        needHydrate = false;
    }
    ...
    if (needHydrate && document.body.querySelectorAll('.textEditor_slate_Viewer').length) {
        needHydrate = false;
    }
...
/* TODO вернуть после решения проблем.
...
function hydrateState(): boolean {
    ...
    /* TODO вернуть после решения проблем.
    const disableHydrate: boolean = cookie.get('disableHydrate') === 'true' || disableHydrateParam || false;
    return !disableHydrate;
    */
    return false;
}
