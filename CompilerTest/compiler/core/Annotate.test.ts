...
describe('Compiler/_compiler/core/Annotate', () => {
    describe('Check passRef flag on inline template', () => {
        it('Should set flag on root node', () => {
            ...
            assert.isTrue(inlineTemplateNode.wsPassRef);
        });
        it('Should set flag through conditionals', () => {
            ...
            inlineTemplateNode = (ast[1] as Ast.IfNode).wsConsequent[0] as Ast.InlineTemplateNode;
            assert.isTrue(inlineTemplateNode.wsPassRef);

            assert.isTrue(ast[2] instanceof Ast.ElseNode);
            inlineTemplateNode = (ast[2] as Ast.ElseNode).wsConsequent[0] as Ast.InlineTemplateNode;
            assert.isTrue(inlineTemplateNode.wsPassRef);

            assert.isTrue(ast[3] instanceof Ast.ElseNode);
            inlineTemplateNode = (ast[3] as Ast.ElseNode).wsConsequent[0] as Ast.InlineTemplateNode;
            assert.isTrue(inlineTemplateNode.wsPassRef);
        });
        it('Should set flag through foreach node', () => {
            ...
            const inlineTemplateNode = (ast[1] as Ast.ForeachNode)
                .wsContent[0] as Ast.InlineTemplateNode;
            assert.isTrue(inlineTemplateNode.wsPassRef);
        });
        ...
        it('Should not set flag on root inside element', () => {
            ...
            const inlineTemplateNode = (ast[1] as Ast.ElementNode)
                .wsContent[0] as Ast.InlineTemplateNode;
            assert.isFalse(inlineTemplateNode.wsPassRef);
        });
        it('Should set flag on root in content option', () => {
            ...
            const inlineTemplateNode = (ast[1] as Ast.ComponentNode).wsContents.content
                .wsContent[0] as Ast.InlineTemplateNode;
            assert.isTrue(inlineTemplateNode.wsPassRef);
        });
        it('Should not set flag in content option inside element', () => {
            ...
            const inlineTemplateNode = (
                (ast[1] as Ast.ComponentNode).wsContents.content.wsContent[0] as Ast.ElementNode
            ).wsContent[0] as Ast.InlineTemplateNode;
            assert.isFalse(inlineTemplateNode.wsPassRef);
        });
        ...
        it('Should not set flag on root in template inside element', () => {
            ...
            assert.isTrue(ast[1] instanceof Ast.TemplateNode);
            inlineTemplateNode = ((ast[1] as Ast.TemplateNode).wsContent[0] as Ast.ElementNode)
                .wsContent[0] as Ast.InlineTemplateNode;
            assert.isFalse(inlineTemplateNode.wsPassRef);
