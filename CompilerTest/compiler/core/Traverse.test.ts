...
describe('Compiler/_compiler/core/Traverse', () => {
    it('DoctypeNode', () => {
    ...
    describe('ElementNode', () => {
        ...
        it('Attributes', () => {
            ...
            const tree = traverseTemplate(html);
            const elementNode = <Ast.ElementNode>tree[0];
            assert.strictEqual(Object.keys(elementNode.wsAttributes).length, 2);
            assert.strictEqual(Object.keys(elementNode.wsEvents).length, 1);

            assert.isTrue(elementNode.wsAttributes.hasOwnProperty('attr:class'));
            assert.isTrue(elementNode.wsAttributes.hasOwnProperty('attr:id'));
            assert.isTrue(elementNode.wsEvents.hasOwnProperty('on:click'));

            assert.instanceOf(elementNode.wsAttributes['attr:class'], Ast.AttributeNode);
            assert.instanceOf(elementNode.wsAttributes['attr:id'], Ast.AttributeNode);
            assert.instanceOf(elementNode.wsEvents['on:click'], Ast.EventNode);
        });
        it('Failure! Attributes', () => {
            const html = '<div attr:class="{{ 1 2 3 }}" on:click="{{ handler() }}"></div>';
            const tree = traverseTemplate(html);
            const elementNode = <Ast.ElementNode>tree[0];
            assert.strictEqual(Object.keys(elementNode.wsAttributes).length, 0);
    ...
    describe('ComponentNode', () => {
        ...
        it('Component attributes and options', () => {
            ...
            assert.strictEqual(Object.keys(componentNode.wsAttributes).length, 1);
            assert.strictEqual(Object.keys(componentNode.wsOptions).length, 1);
            assert.strictEqual(Object.keys(componentNode.wsEvents).length, 0);
            assert.strictEqual(Object.keys(componentNode.wsContents).length, 0);
            assert.isTrue(componentNode.wsAttributes.hasOwnProperty('attr:class'));
        ...
        it('Component event handlers', () => {
            ...
            const tree = traverseTemplate(html);
            const componentNode = <Ast.ComponentNode>tree[0];
            assert.strictEqual(Object.keys(componentNode.wsAttributes).length, 0);
            assert.strictEqual(Object.keys(componentNode.wsOptions).length, 0);
            assert.strictEqual(Object.keys(componentNode.wsEvents).length, 1);
            assert.strictEqual(Object.keys(componentNode.wsContents).length, 0);
            assert.isTrue(componentNode.wsEvents.hasOwnProperty('on:click'));
        });
        it('Component contents', () => {
         ...
         `;
            ...
            assert.strictEqual(Object.keys(componentNode.wsOptions).length, 0);
            assert.strictEqual(Object.keys(componentNode.wsEvents).length, 0);
            assert.strictEqual(Object.keys(componentNode.wsContents).length, 2);
            assert.isTrue(componentNode.wsContents.hasOwnProperty('content'));
            assert.isTrue(componentNode.wsContents.hasOwnProperty('contentOption'));
        });
        it('Doctype in Component 1', () => {
         ...
         `;
            ...
            assert.strictEqual(Object.keys(componentNode.wsOptions).length, 0);
            assert.strictEqual(Object.keys(componentNode.wsEvents).length, 0);
            assert.strictEqual(Object.keys(componentNode.wsContents).length, 1);
            assert.isTrue(componentNode.wsContents.hasOwnProperty('content'));
        });
    ...
    describe('PartialNode', () => {
        ...
        it('Inline template with invalid name', () => {
            const html = '<ws:template name="invalid template name!"><div></div></ws:template>';
            const tree = traverseTemplate(html);
            assert.strictEqual(tree.length, 0);
        });
        it('Dynamic partial', () => {
    ...
    describe('Data types', () => {
        describe('Data type directive', () => {
            ...
            `;
                ...
                assert.isTrue(functionNode.wsOptions.hasOwnProperty('arg1'));
                assert.isTrue(functionNode.wsOptions.hasOwnProperty('arg2'));
            });
            it('Number', () => {
                const optionTemplate = `
            <ws:option>
            ...
            `;
                ...
                const properties = (<Ast.ObjectNode>option.wsValue).wsProperties;
