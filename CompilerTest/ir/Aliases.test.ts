...
import InternalGenerator from 'Compiler/_ir/generator/impl/Internal';
...
describe('Compiler/_compiler/irCodegen/Synonyms', () => {
    ...
    describe('Compiler/_ir/generator/flow/Chain', () => {
        ...
        for (const [name, alias] of Aliases.CHAIN_ALIASES) {
            it(`method "${alias}" should redirect to "${name}"`, testMethod<Chain>(chain, name, alias));
        }
    });
    describe('Compiler/_ir/generator/Generator', () => {
        ...
        describe('Compiler/_ir/generator/impl/Internal', () => {
            const internal = new InternalGenerator();

            for (const [name, alias] of Aliases.GENERATOR_ALIASES) {
                it(`method "${alias}" should redirect to "${name}"`, testMethod<IGenerator>(internal, name, alias));
            }
        });
    });
