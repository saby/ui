...
describe('Markup attributes', () => {
    ...
    it('Валидация строки style', async () => {
        ...
        act(() => {
            instance = creator(InvalidStyleControl, {}, container) as InvalidStyleControl;
        });
    ...
    it('Валидация строки style normal', async () => {
        // region Setup
        act(() => {
            creator(NormalStyleTest, {}, container);
        });
        await tickWasabyUpdate();
    ...
    it('Строка style c base64 внутри', () => {
        ...
        expect(instance._children.onlyBase64.style.backgroundImage).toBe(styles.backgroundImage);

        expect(instance._children.otherPropBeforeBase64.style.color).toBe(styles.color);
        expect(instance._children.otherPropBeforeBase64.style.backgroundImage).toBe(
            styles.backgroundImage
        );

        expect(instance._children.otherAfterBeforeBase64.style.backgroundImage).toBe(
            styles.backgroundImage
        );
        expect(instance._children.otherAfterBeforeBase64.style.color).toBe(styles.color);

        expect(instance._children.otherPropsAroundBase64.style.color).toBe(styles.color);
        expect(instance._children.otherPropsAroundBase64.style.backgroundImage).toBe(
            styles.backgroundImage
        );
        expect(instance._children.otherPropsAroundBase64.style.margin).toBe(styles.margin);
    });

    it('data-component ставиться на корректный div', async () => {
        // region Setup
        act(() => {
            creator(DataComponentRoot, {}, container);
        });
        tick(0);
        // endregion
        await tickWasabyUpdate();
...
describe('attr: в wml и копирование props attrs в props', () => {
    ...
    test('react function attr:class', () => {
        act(() => {
            creator(WmlAttrsWasabyRoot, { reactFunction: true }, container);
        });

        expect(container).toMatchSnapshot();
    });

    test('react component attr:class', () => {
        act(() => {
            creator(WmlAttrsWasabyRoot, { reactComponent: true }, container);
        });
    ...
    test('прокидывание clearProps с className', () => {
        act(() => {
            creator(WmlAttrsWasabyRoot, { clearProps: true }, container);
        });
