...
import RestoreFocusRootReact from './Focus/RestoreFocus/RestoreFocusRootReact';
...
describe('ReactFocus', () => {
    ...
    describe('Восстановление фокуса', () => {
        ...
        it('Работает восстановление фокуса на FocusRoot в случае отсутствия Васаби контролов', async () => {
            act(() => {
                render(<RestoreFocusRootReact />, container);
            });
            expect(document.activeElement).toMatchSnapshot('Активный элемент - поле ввода');

            const user = userEvent.setup({
                delay: null,
            });
            const inputToFocus = await screen.findByTestId('Сфокусированное поле ввода');
            await act(async () => user.click(inputToFocus));

            expect(document.activeElement).toMatchSnapshot(
                'Восстановленный активный элемент после удаления поля ввода - FocusRoot'
            );
        });
        ...
        it('при восстановлении фокуса, если есть незамаунченный опенер в родителях, он не должен мешать', async () => {
            ...
            const firstInputElement = document.getElementById('firstInputElement');
            const parents = getParentFocusComponents(firstInputElement).filter(
                (parent) => !parent.isFocusCallbacksObject
            );
