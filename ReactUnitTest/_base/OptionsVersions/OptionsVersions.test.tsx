...
describe('Сохранение версий версионируемых опций до _beforeMount', () => {
    ...
    test('все контролы синхронные', async () => {
        ...
        expect(child1InitialOptionsVersions.has('record')).toBeTruthy();
        expect(child1InitialOptionsVersions.get('record')).toEqual(0);

        expect(parent.getChild1()._optionsVersions.has('record')).toBeTruthy();
        expect(parent.getChild1()._optionsVersions.get('record')).toEqual(1);
    });
});
