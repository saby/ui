...
//# region Lazy getters
    ...
    private lazyCoreSanitize;

    // eslint-disable-next-line @typescript-eslint/typedef
    private lazyViewDecorators;

    protected get coreSanitize() {
        if (!this.lazyCoreSanitize) {
            // eslint-disable-next-line core-deprecated-modules
            this.lazyCoreSanitize = require('Core/Sanitize');
        }

        return this.lazyCoreSanitize;
    }
...
//# region interface IPrivateMethods
    ...
    abstract makeFunctionSerializable(
        func: unknown,
        scope: unknown
    ): unknown;
