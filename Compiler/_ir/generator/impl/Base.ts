...
import type { IController } from '../flow/Controller';
...
export default abstract class Base implements IPrivateGenerator {
    protected constructor(
        ...
        protected readonly chainController: IController
    ) { }

//# region implementation of interface IGeneratorMinified

    e(context: IContext, value: unknown): unknown {
        return this.escape(context, value);
    }

    t(context: IContext, text: string, key: string): unknown {
        return this.createText(context, text, key);
    }

    d(context: IContext, text: string): unknown {
        return this.createDirective(context, text);
    }

    c(context: IContext, text: string): unknown {
        return this.createComment(context, text);
    }
    ...
    i(context: IContext, test: number, body: TemplateBody): IChain {
        return this.if(context, test, body);
    }
