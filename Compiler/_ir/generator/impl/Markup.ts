...
import type { IGenerator as UIGenerator } from 'UICommon/Executor';
import type { IControlConfig } from 'UICommon/Executor';

import type { IElementConfiguration, IComponentConfiguration, TControlMethod, IPrivateInternalGenerator } from '../Interface';
...
//# region implementation of interface INodeGenerator
     ...
     */
    escape(context: IContext, value: unknown): unknown {
     ...
     */
    createText(context: IContext, text: string, key?: string): unknown {
     ...
     */
    createDirective(context: IContext, text: string): unknown {
...
//# region implementation of interface IGenerator
    ...
    createContentOption(context: IContext, content: number, internalsMetaId?: number): IDataArray {
        ...
        const internal = this.internal.evalContentOptionInternal(context, content, internalsMetaId);
    ...
    if(context: IContext, test: number, body: TemplateBody): Chain {
        ...
        const defaultAlternateValue = this.createText(context, '');
...
//# endregion
    ...
    private createControlConfiguration(context: IContext, configuration: IComponentConfiguration): IControlConfig {
        const controlConfiguration: IControlConfig = {
            ...
            // TODO: выяснить, как он формируется и что в нем лежит
        ...
        if (configuration.i) {
            controlConfiguration.internal = this.internal.evalComponentInternal(context, configuration.i);
        }
