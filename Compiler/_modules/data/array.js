define('Compiler/_modules/data/array', [
   ...
   function generateInternal(
      ...
      if (internalFunctions) {
         return FSC.getStr(
            Internal.generate(injected.__$ws_internalTree, internalFunctions)
         );
      }
   ...
   function generateFunction(htmlPropertyName, html, string, injected) {
      ...
      var wsTemplateName =
         injected && injected.attribs && injected.attribs._wstemplatename;
               ...
               if (children[index].children) {
                  ...
                  if (typeFunction) {
                     ...
                     if (nameExists && nameExists.charAt && nameExists.slice) {
                        typeName =
                           nameExists.charAt(0).toUpperCase() +
                           nameExists.slice(1);
                     }
                     ...
                     if (types[typeName]) {
                        ...
                        errorHandler.error(
                           children[index].name +
                              " property can't be in the root of ws:array tag",
                           {
                              fileName: this.fileName
                           }
                        );
