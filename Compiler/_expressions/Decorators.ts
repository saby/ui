...
function splitControlPropName(name: any, separator: any, fieldName: any): any {
    ...
    try {
        ...
        throw new Error(
            'В качестве имени опции контрола был передан undefined'
        );
    }
    return resultArr;
}
...
class BindingObject {
    ...
    constructor(
        ...
        const propArr = splitControlPropName(
            controlPropName,
            SEPARATOR,
            contextFieldName
        );
        ...
        if (currentDirection) {
            currentDirection = FSC.wrapAroundExec(
                '"" +' + currentDirection + '+ ""'
            );
        }

        this.fieldName = FSC.wrapAroundExec(
            '"" +' + (contextFieldName || '""') + '+ ""'
        );
...
class BindNode {
    ...
    constructor(
        ...
        this.binding = new BindingObject(
            controlPropName,
            value,
            way,
            direction,
