...
EventChain.prepareEventChain = function prepareEventChain(
    originChain?: EventChain
): EventChain {
    if (!originChain) {
...
export function processEventAttribute(
    ...
    if (artifact.context !== defaultContext) {
        eventNode.context = FSC.wrapAroundExec(
            '(function(){ return ' + artifact.context + '; })'
        );
        eventNode.handler = FSC.wrapAroundExec(
            'function() { return ' + artifact.fn + '; }'
        );
    }
    ...
    if (calculateMeta) {
        const eventMeta = {
            isControl,
            context: FSC.wrapAroundExec(
                '(function(){ return ' + defaultContext + '; })'
            ),
