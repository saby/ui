...
export class Tokenizer implements ITokenizer {
            ...
            switch (this.state) {
                ...
                case State.BEFORE_ATTRIBUTE_NAME:
                    ...
                    switch (char) {
                        ...
                        case Characters.EQUALS_SIGN:
                            ...
                            this.error(
                                `Некорректный символ "${char}" в имени атрибута`
                            );
                        // fallthrough
                        default:
                ...
                case State.ATTRIBUTE_NAME:
                    ...
                    switch (char) {
                        ...
                        case Characters.LESS_THAN_SIGN:
                            // Parse error. Treat it as per the "anything else" entry below.
                            this.warn(
                                `Обнаружен некорректный символ "${char}" в имени атрибута`
                            );
                        // fallthrough
                        default:
                ...
                case State.RAW_TEXT_ESCAPED:
                    ...
                    switch (char) {
                        ...
                        case Characters.LESS_THAN_SIGN:
                            this.state =
                                State.RAW_TEXT_DOUBLE_ESCAPED_LESS_THAN_SIGN;
                            break;
                        case Characters.NULL:
                ...
                case State.RAW_TEXT_ESCAPED_END_TAG_NAME:
                    ...
                    switch (char) {
                        ...
                        case Characters.SPACE:
                            ...
                            if (
                                this.charBuffer.toUpperCase() ===
                                this.expectingEndTagName
                            ) {
                                this.state = State.BEFORE_ATTRIBUTE_NAME;
                                break;
                            }
                ...
                case State.RAW_TEXT_DOUBLE_ESCAPED_DASH:
                    ...
                    switch (char) {
                        case Characters.HYPHEN_MINUS:
                            ...
                            // Emit a HYPHEN-MINUS character token.
                            this.state =
                                State.RAW_TEXT_DATA_DOUBLE_ESCAPED_DASH_DASH;
                            break;
                        case Characters.LESS_THAN_SIGN:
                            // Switch to the raw text double escaped less-than sign state.
                            // Emit a LESS-THAN SIGN character token.
                ...
                case State.ESCAPABLE_RAW_TEXT:
                    ...
                    switch (char) {
                        case Characters.LESS_THAN_SIGN:
                            this.returnState = this.state;
                            this.state =
                                State.ESCAPABLE_RAW_TEXT_LESS_THAN_SIGN;
                            this.flushCharBuffer();
                            break;
                        case Characters.NULL:
                            // Parse error. Emit a REPLACEMENT CHARACTER character token.
    ...
    private completeAttributeName(): void {
        ...
        if (this.attributes[this.attributeName]) {
            this.error(
                `Атрибут "${this.attributeName}" уже определен на теге "${this.tagName}"`
            );
            this.attributeName = undefined;
        }
    }
    ...
    private emitComment(provisionalHyphens: number): void {
        const data = this.charBuffer.substr(
            0,
            this.charBuffer.length - provisionalHyphens
        );
    ...
    private finalize(): void {
        switch (this.state) {
            ...
            case State.SELF_CLOSING_START_TAG:
                this.error(
                    'Обнаружен конец файла при чтении тега. Данный тег будет отброшен'
                );
                break;
            case State.ATTRIBUTE_NAME:
                this.error(
            ...
            case State.MARKUP_DECLARATION_OCTYPE:
                if (this.index < 6) {
                    ...
                    this.error(
                        'Во время чтения конструкции doctype был достигнут конец файла'
                    );
                    this.emitDoctype();
                }
                break;
            ...
            case State.COMMENT:
                this.error(
                    'Во время чтения комментария был достигнут конец файла'
                );
                this.emitComment(0);
                break;
            case State.COMMENT_END:
