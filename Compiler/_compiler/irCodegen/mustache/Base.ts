    ...
    implements IExpressionVisitor<TContext, string>, IMustacheGenerator<string>
{
    ...
    visitProgramNode(node: ProgramNode, context: TContext): string {
        ...
        return this.wrapProgramBody(body, context);
    }

    visitEmptyStatementNode(_node: EmptyStatementNode, _context: TContext): string {
        return '';
    }
    ...
    visitThisExpressionNode(_node: ThisExpressionNode, _context: TContext): string {
    ...
    visitIdentifierNode(node: IdentifierNode, _context: TContext): string {
        return node.name;
    }
     ...
     * @protected
     */
    protected wrapProgramBody(body: string, context: TContext): string {
        if (context.shouldSanitize === true) {
            this.flags.hasMethodsReference = true;

            return this.methods.sanitize(body);
        }
