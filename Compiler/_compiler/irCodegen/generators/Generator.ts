...
import { wrapString, wrapArray, wrapSequence, toSafeMultilineComment } from '../types/String';
...
export default class Generator extends Base implements IGenerator<TPrimitive, string, string> {
    ...
    escape(value: string): string {
        return this.generateMethodCall('escape', [value]);
    }

    createText(text?: string, key?: string): string {
        if (typeof key === 'string') {
            return this.generateFormattedMethodCall('createText', [text as string, key]);
        }

        if (typeof text === 'string') {
            return this.generateFormattedMethodCall('createText', [text]);
        }

        return this.generateFormattedMethodCall('createText', []);
    }
    ...
    createComment(text: string): string {
        return this.generateMethodCall('createComment', [text]);
    }
    ...
    createInline(referenceId: number, name: string, configuration: string): string {
        return this.generateFormattedMethodCall('createInline', [
            'context',
            `${toSafeMultilineComment(name)} ${wrapNone(referenceId)}`,
            configuration
        ]);
    }
    ...
    evalExpression(referenceId: number, comment?: string): string {
        if (typeof comment === 'string') {
            return this.generateMethodCall('evalExpression', ['context', `${toSafeMultilineComment(comment)} ${referenceId}`]);
        }

        return this.generateMethodCall('evalExpression', ['context', referenceId]);
    }

    evalExpression2(referenceId: number, comment?: string): string {
        if (typeof comment === 'string') {
            return this.generateMethodCall('evalExpression2', ['context', `${toSafeMultilineComment(comment)} ${referenceId}`]);
        }

        return this.generateMethodCall('evalExpression2', ['context', referenceId]);
    }

    closeExpression(referenceId: number, comment?: string): string {
        if (typeof comment === 'string') {
            return this.generateMethodCall('closeExpression', ['context', `${toSafeMultilineComment(comment)} ${referenceId}`]);
        }
    ...
    closeBindExpression(referenceId: number, comment?: string): string {
        if (typeof comment === 'string') {
            return this.generateMethodCall('closeBindExpression', ['context', `${toSafeMultilineComment(comment)} ${referenceId}`]);
        }
