    ...
    private putTemplates(meta: IObjectProperty[], description: IEntryPointDescription<string>): void {
        const templates = description.templates.map((template) => {
            if (template.type === 'content') {
                return this.wrapContentBody(template.body as string, template.name as string);
            }

            if (template.type === 'template') {
                return this.wrapTemplateBody(template.name as string, template.body as string);
            }

            if (template.type === 'root') {
                return this.wrapRootBody(template.body as string);
            }
