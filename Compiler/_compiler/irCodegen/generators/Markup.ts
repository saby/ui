...
function toSafeJointContent(value: string[], separator: string): string {
    if (value.length === 0) {
        return undefined;
    }
...
function toReferenceId(referenceId: number, program: string): string {
...
function numberSorter(a: number, b: number): number {
    return a - b;
}

function toUniqueElements(elements: number[]): number[] {
    return Array.from(new Set(elements)).sort(numberSorter);
}
...
//# region Class properties
    ...
    private readonly chainGenerator: IChainGenerator<TPrimitive, string, string>;
    ...
    private readonly internalsMeta: string[];
...
//# endregion
     ...
     * @param {IChainGenerator<TPrimitive, string, string>} chain Conditional chain public generator.
    ...
    constructor(
        ...
        chain: IChainGenerator<TPrimitive, string, string>,
     ...
     * @returns {IDescription<string>} Description of future module.
     */
    generate(annotation: IAnnotation): IDescription<string> {
        ...
        return {
            ...
            exports: this.generateExports(annotation)
        };
    }
     ...
     * @param {IAnnotation} annotation Template annotation.
     */
    generateExports(annotation: IAnnotation): string {
        return this.entryPointGenerator.generate({
            ...
            internalsMeta: this.internalsMeta
        });
    }
...
//# region Attribute visitors
    ...
    visitContentOption(node: ContentOptionNode, context: IContext): string {
        // TODO: поддержать evalContentOption для tmpl при заданном type="string"
...
//# region HTML visitors
     ...
     * @param {IContext} context Current processing context.
     */
    visitDoctype(node: DoctypeNode, context: IContext): string {
        try {
     ...
     * @param {IContext} context Current processing context.
     */
    visitCData(node: CDataNode, context: IContext): string {
        try {
            this.formatter.enter();
     ...
     * @param {IContext} context Current processing context.
     */
    visitInstruction(node: InstructionNode, context: IContext): string {
        try {
            this.formatter.enter();
    ...
    /**
     * Visit comment node.
     * @param {CommentNode} node Current comment node.
     * @param {IContext} context Current processing context.
     */
    visitComment(node: CommentNode, context: IContext): string {
        return undefined;
    }
...
//# region Code flow visitors
    ...
    visitElse(node: ElseNode, context: IContext): string {
        return this.generateChain(node, context);
    }
...
//# region Data type visitors
    ...
    visitString(node: StringNode, context: IContext): string {
        ...
        return toSafeString(toSafeJointContent(content, ' + '));
    }
    ...
    visitValue(node: ValueNode, context: IContext): string {
        ...
        return toSafeJointContent(content, ' + ');
    }
...
//# region Extended text visitors
     ...
     * @param {IContext} context Current processing context.
     */
    visitTextData(node: TextDataNode, context: IContext): string {
    ...
    visitExpression(node: ExpressionNode, context: IContext): string {
        ...
        if (typeof meta.referenceId === 'number') {
            if (meta.shouldEscape === true) {
                return this.generator.escape(
                    this.generator.evalExpression(meta.referenceId, meta.program)
                );
            }

            return this.generator.evalExpression(meta.referenceId, meta.program);
        }
     ...
     * @param {IContext} context Current processing context.
     */
    visitTranslation(node: TranslationNode, context: IContext): string {
...
//# region Private methods
    ...
    private generateChain(node: IfNode | ElseNode, context: IContext): string {
        const test = this.compileExpression(node.wsTest, context.spawn(serviceExpressionContext));

        const consequent = visitAll(this, node.wsConsequent, context);
        const alternate = node.wsAlternate?.accept(this, context) ?? this.chainGenerator.fi();

        const body = this.generateTemplateBody(consequent);

        if (node instanceof IfNode) {
            return `${this.generator.if(toReferenceId(test.referenceId, test.program), body)}${alternate}`;
        }

        if (typeof test.referenceId === 'number') {
            return `${this.chainGenerator.elif(toReferenceId(test.referenceId, test.program), body)}${alternate}`;
        }

        return `${this.chainGenerator.else(body)}${alternate}`;
    }
    ...
    private generateBindings(bindings: IBindingConfiguration[]): string {
        return this.formatter.formatArray(bindings.map(binding => this.formatter.formatObject([
            {
                name: 'fieldName',
                value: this.generatePartialExpression(binding.fieldName)
            },
            {
                name: 'propName',
            ...
            },
            {
                ...
                value: binding.direction?.isTableFunction
                    ...
                    : wrapString(binding.direction.body)
            },
            {
    ...
    private putEventHandlerMeta(context: IContext): void {
        ...
        context.events.meta = this.formatter.formatObject([
            ...
            {
                name: 'context',
                value: 'defaultContextGetterFunction'
            },
            {
                name: 'handler',
    ...
    private generateOptions(node: BaseWasabyElement, context: IContext, escape: boolean): IObjectProperty {
        ...
        if (context.options.hasOwnProperty('scope')) {
    ...
    private allocateInternalsMeta(node: Ast, context: IContext): number {
        const meta = (node.wsContainer as ProgramsContainer).getInternalsMeta();

        if (meta.include.length === 0 && meta.exclude.length === 0) {
            return undefined;
        }

        // Critical!
        //   Запрос на коллекцию осуществляется после обработки содержимого,
        //   потому что необходимо, чтобы все внутренние Mustache-выражения были размещены в таблице.
        if (meta.exclude.some(refId => refId === -1)) {
            throw new Error('внутренняя ошибка: в exclude находится неразмещенное выражение');
        }

        const include: number[] = [];
        const exclude: number[] = meta.exclude;

        meta.include.forEach((program) => {
            const mustacheInfo = this.compileExpression(program, context.spawn({
                alwaysTableFunction: true
            }));

            // В коллекции мета описаний могут находиться несколько одинаковых выражений,
            // которые будут размещены в таблице по одному и тому же адресу.
            // Эти выражения могут иметь разный тип операций, в том числе и взаимоисключающий: include и exclude сразу.
            // Необходимо убирать из include выражения, которые также находятся и в exclude.
            if (exclude.indexOf(mustacheInfo.referenceId) > -1) {
                return;
            }

            include.push(mustacheInfo.referenceId);
        });

        const element = wrapArray([
            wrapArray(toUniqueElements(include)),
            wrapArray(toUniqueElements(exclude))
        ]);
        ...
        const elementIndex = this.internalsMeta.indexOf(element);
        if (elementIndex > -1) {
            return elementIndex;
        }

        const internalsMetaId = this.internalsMeta.length;

        this.internalsMeta.push(element);

        return internalsMetaId;
    }
//# endregion
}
