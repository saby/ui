...
export default function createGenerator(esVersion: number, isRelease: boolean, offset: number = 0): MarkupGenerator {
    ...
    const symbols = new Symbols(STRINGS_IDENTIFIER, isRelease);

    return new MarkupGenerator(formatter, ecmaScript, methods, generator, chain, entryPoint, symbols);
}
