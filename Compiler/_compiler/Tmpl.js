define('Compiler/_compiler/Tmpl', [
   'Compiler/_compiler/core/bridge',
   'Compiler/_compiler/utils/ErrorHandler',
   'Compiler/_compiler/modules/utils/common',
   'Compiler/_compiler/codegen/bridge',
   'Compiler/_compiler/codegen/JsTemplates',
   'Compiler/_compiler/utils/ModulePath',
   'Compiler/_compiler/html/Parser',
   'Compiler/_compiler/core/Tags',
   'Compiler/_compiler/core/deprecated/ComponentCollector',
   'Compiler/_compiler/core/StaticHelpers',
   ...
   function getFile(html, config, successCallback, failureCallback, ext) {
      ...
      template(html, getResolverControls(currentExt), config).handle(function (traversed) {
         try {
            ...
            if (!tmplFunc) {
               errorHandler.critical('Шаблон не может быть построен. Не загружены зависимости.', {
                  fileName: config.fileName
               });
            }
            var moduleName = ModulePath.replaceWsModule(config.fileName).replace(
               /\.(wml|tmpl)$/gi,
               EMPTY_STRING
            );
            ...
            var finalFile = JsTemplates.generateDefine(
               ...
               true,
               ECMAScript.createGenerator(config.ESVersion)
            );
            finalFile = JsTemplates.clearSourceFromTmplBlocks(finalFile);
