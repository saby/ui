...
import * as common from '../modules/utils/common';
import * as decorators from '../expressions/Decorators';
import * as N from '../expressions/Nodes';

const errorHandler = createErrorHandler(true);

export interface IExpressionVisitorContext {
    fileName: string;
    isControl: boolean;
...
function resolveIdentifierBase(
    ...
    if (node.name === 'context') {
        ...
        return `(!${genGetter(context, ['"context"'], useStrictGetter)} ? context : ${genGetter(
            'data',
            ['"context"'],
            useStrictGetter
        )})`;
...
export class ExpressionVisitor implements N.IExpressionVisitor<IExpressionVisitorContext, string> {
    ...
    visitCallExpressionNode(
        ...
        if (callee) {
            ...
            if (
                ...
                return genCallInternalFunction(callee, callContext, args, context.useStrictGetter);
            }
    ...
    visitConditionalExpressionNode(
        ...
        if (typeof node.alternate !== 'undefined') {
            ...
            isAlternateEmpty =
                ...
                (context.attributeName.includes && context.attributeName.includes('attr:'));
            if (!isAlternateEmpty) {
                alternate = 'undefined';
            }
        }
    ...
    visitDecoratorCallNode(node: N.DecoratorCallNode, context: IExpressionVisitorContext): string {
        context.caller = node.caller ? (node.caller.accept(this, context) as string) : undefined;
        ...
        const chainContextAsIdentifier = (decoratorAsChainContext as N.DecoratorChainCallNode)
            .identifier;
        if (
