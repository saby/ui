...
export function genCallInternalFunction(
    ...
    if (isStrict) {
        return `${T_HELPERS_NAME}.i/* callIFun */(${fn}, ${ctx}, [${args.join(',')}], collection)`;
    }
 ...
 * @returns {string} Code fragment.
 */
export function genCreateGenerator(
    isVdom: string,
    forceCompatible: string,
    config: string
): string {
...
/**
 * Generate substitution for registerIdentifiers function.
 */
export function genRegisterDeserializableIdentifiers(
    depsLocal: string,
    viewController: string
): string {
    return `${T_HELPERS_NAME}.rdi /* registerDeserializableIdentifiers */(${T_HELPERS_NAME}, ${depsLocal}, ${viewController});`;
}
