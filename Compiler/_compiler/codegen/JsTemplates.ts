...
import {
    ...
    genRegisterDeserializableIdentifiers,
} from './TClosure';
...
function generateHeadTemplate(generator: Generator): string {
    // WARNING: Любые изменения этого фрагмента должны согласовываться с десериализатором:
 ...
 */
export function generateFunctions(templateFunction: IMainTemplateFunction): string {
    let privateTemplates = '';

    if (templateFunction.internalFunctions) {
        for (let index = 0; index < templateFunction.internalFunctions.length; ++index) {
            privateTemplates += templateFunction.internalFunctions[index];
        }
    }

    if (templateFunction.contentOptionFunctions) {
        for (let index = 0; index < templateFunction.contentOptionFunctions.length; ++index) {
...
export function clearSourceFromTmplBlocks(text: string): string {
    ...
    while (start > -1) {
        ...
        clearedSource =
            clearedSource.substr(0, start) + clearedSource.substr(end + TMPL_BLOCK_END.length);
...
function replaceContentOptionName(
    ...
    if (hasVariable) {
        ...
        return source.replace(pattern, generateReturnValueFunction(`${pNameProperty}: pName,`));
    }
...
export function generateDefine(
    ...
    const mainTemplateFunctionName = getMainTemplateFunctionName(templateFunction);
    const template = functionToString(templateFunction, mainTemplateFunctionName);
    ...
    const internalFunctionDecl = `var __dirtyCheckingVars_ = ${generator.genArrowExpression(
        generator.genStringInterpolation('__dirtyCheckingVars_${id}'),
        ['id']
    )};`;

    return (
        `define('${nodeName}', ${JSON.stringify(finalDependencies)}, function(${parameters.join(
            ','
        )}) {` +
        'function debug() { debugger; }' +
        ...
        `templateFunction.reactiveProps = ${JSON.stringify(reactiveProperties)};` +
...
export function generateFor(
    ...
    return (
        ...
        generator.genArrowFunction(
            'var out = [];' +
                `${generator.genDynamicObjectPropertyAccessor(
                    'data',
                    'viewController'
                )} = viewController || null;` +
                '(function customForTemplateScope() {' +
                    `var templateCount = 0, contextInput = ${generator.genStringInterpolation(
                        `\${key}${cycleIndex}`
                    )}, itCount = 0;` +
...
export function generateTemplate(
    ...
    const initRkFunction = hasTranslations ? `var rk = ${genGetRk('filename')};` : EMPTY_STRING;
    const internalFunctionDecl = `var __dirtyCheckingVars_ = ${generator.genArrowExpression(
        generator.genStringInterpolation('__dirtyCheckingVars_${id}'),
        ['id']
    )};`;
    ...
    const source =
        ...
        `var markupGenerator = ${genCreateGenerator(
            'isVdom',
            'forceCompatible',
            'generatorConfig'
        )};` +
        ...
        internalFunctionDecl +
        `${getDynamicPropertiesDeclaration(generator)}` +
        `${genRegisterDeserializableIdentifiers('depsLocal', 'viewController')}` +
        initRkFunction +
        'funcContext = data;' +
        'if (typeof includedTemplates === "undefined") {' +
        'eval("var includedTemplates = undefined;");' +
    ...
    return (
        ...
        generator.genArrowFunction(
            'var scope = Object.create(data);' +
                ...
                TMPL_BLOCK_START +
                `bindFn.toJSON = ${generator.genArrowExpression(
                    `"TEMPLATEFUNCTOJSON=" + ${template}.toString()`
                )};` +
                TMPL_BLOCK_END +
 ...
 * @returns {string} Сгенерированный блок кода.
 */
export function generateInlineTemplateTmpl(
    name: string,
    body: string,
    generator: Generator
): string {
