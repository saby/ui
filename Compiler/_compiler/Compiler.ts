...
function isValidExtension(extension: string): boolean {
    return extension === 'wml' || extension === 'tmpl';
}
...
export class Compiler implements ICompiler {
    ...
    compile(text: string, config: IOptions): Promise<IArtifact> {
        return new Promise<IArtifact>(
            (resolve: (_: IArtifact) => void, reject: (_: IArtifact) => void) => {
                const artifact = this.compileSync(text, config);
    ...
    compileSync(text: string, config: IOptions): IArtifact {
        ...
        if (!isValidExtension(options.modulePath.extension)) {
    ...
    private generate(annotation: IAnnotation, options: IOptions, errorHandler: IErrorHandler): IDescription {
    ...
    private wrapWithModule(description: IDescription, options: IOptions, moduleType: string) {
