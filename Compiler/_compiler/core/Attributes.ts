...
import { ITextProcessor, TextContentFlags, ITranslationsRegistrar } from './Text';
import { INodeDescription } from '../i18n/Translator';
...
class AttributeProcessor implements IAttributeProcessor {
            ...
            if (attributes.hasOwnProperty(attributeName)) {
                ...
                if (isBind(attributeName)) {
                    ...
                    if (bindNode) {
                        ...
                        collection.events[`bind:${bindNode.wsProperty}`] = bindNode;
                        if (collection.options[bindNode.wsProperty]) {
                            this.errorHandler.warn(
                                `Атрибут "${attributeName}" тега "${options.parentTagName}" переопределяет атрибут "${bindNode.wsProperty}". Атрибут "${bindNode.wsProperty}" будет проигнорирован, его необходимо убрать`,
                                {
                ...
                if (isEvent(attributeName)) {
                    ...
                    if (eventNode) {
                        ...
                        collection.events[`on:${eventNode.wsEvent}`] = eventNode;
                    }
                    continue;
                }
                if (isAttribute(attributeName) || options.hasAttributesOnly) {
                    ...
                    if (attributeNode) {
                        ...
                        attributeNode.wsHasAttributePrefix = hasWasabyPrefix(attributeName);
                        if (collection.attributes.hasOwnProperty(`attr:${attributeNode.wsName}`)) {
                        ...
                        collection.attributes[`attr:${attributeNode.wsName}`] = attributeNode;
                    }
                    continue;
                }
                ...
                if (optionNode) {
                    ...
                    if (collection.events[`bind:${optionNode.wsName}`]) {
                        this.errorHandler.warn(
                            `Атрибут "${optionNode.wsName}" тега "${options.parentTagName}" переопределяет атрибут "bind:${optionNode.wsName}". Атрибут "${optionNode.wsName}" будет проигнорирован, его необходимо убрать`,
                            {
                                fileName: options.fileName,
                                position: node.position,
                            }
    ...
    private processAttribute(
        ...
        try {
            ...
            if (attributeValue === null) {
                ...
                return new Ast.AttributeNode(attribute, [new Ast.TextDataNode('')]);
            }
            ...
            if (attributeValue === null) {
                ...
                value = this.textProcessor.process(attributeValue, {
                    ...
                    translateText: nodeDescription
                        ? nodeDescription.isOptionTranslatable(attributeNode.name)
                        : false,
                    translationsRegistrar: options.translationsRegistrar,
                    position: attributeNode.position,
                });
                value = this.validateOptionWithExpectedExpressionOnly(attributeNode, value, options);
            }
    ...
    /**
     * Validate option "attribute" and "scope".
     * Temporal method.
     * @param attributeNode {Attribute} Html attribute node.
     * @param value {TText[]} Parsed attribute value.
     * @param options {IAttributeProcessorOptions} Attribute processor options.
     * @private
     */
    private validateOptionWithExpectedExpressionOnly(
        attributeNode: Nodes.Attribute,
        value: Ast.TText[],
        options: IAttributeProcessorOptions
    ): Ast.TText[] {
        // FIXME: необходимо выставить allowedContent = TextContentFlags.EXPRESSION перед обработкой значения.
        //  Но для начала необходимо вычистить ошибки из шаблонов, потому что такая проверка гарантированно положит сборку.
        //  Сначала выводим предупреждение, после чего включим вывод ошибки.
        if (attributeNode.name === 'scope' || attributeNode.name === 'attributes') {
            // Необходимо исключить пустые строки из набора. Такая очистка происходит по умолчанию,
            // если парсеру задается разрешенный контент TextContentFlags.EXPRESSION.
            const cleanValue = value.filter((node) => {
                if (node instanceof Ast.TextDataNode) {
                    if (node.wsContent.trim() === EMPTY_STRING) {
                        return false;
                    }
                }
            ...
            if (cleanValue.some(part => !(part instanceof Ast.ExpressionNode))) {
                this.errorHandler.warn(
                    `Атрибут "${attributeNode.name}" на теге "${options.parentTagName}" содержит невалидное значение: ожидалось значение, содержащее исключительно Mustache-выражение, получено значение "${attributeNode.value}"`,
                    {
                        fileName: options.fileName,
                        position: attributeNode.position,
                    }
