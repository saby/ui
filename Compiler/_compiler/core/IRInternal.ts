...
/**
 * Интерфейс мета-информации для вычисления internal выражений.
 *
 * @private
 */
export interface IInternalsMeta {

    /**
     * Коллекция неразмещенных синтетических выражений,
     * которые НЕОБХОДИМО вычислить при формировании internal.
     */
    include: ProgramNode[];

    /**
     * Коллекция размещенных в таблице выражений,
     * которые ЗАПРЕЩЕНО вычислять при формировании internal.
     */
    exclude: number[];
}
...
//# endregion
    ...
    attach(container: ProgramsContainer, identifiers: string[]): void {
        ...
        container.programs.forEach((meta) => {
            this.hoistProgram(meta.program, meta.type, meta.operation);
        });
    }
     ...
     */
    getInternalsMeta(): IInternalsMeta {
        return {
            include: this.programs
                .filter(meta => meta.operation === OperationType.INCLUDE)
                .map(meta => meta.program),

            exclude: this.programs
                .filter(meta => meta.operation === OperationType.EXCLUDE)
                .map(meta => meta.program.referenceId)
        };
    }
     ...
     * @private
     */
    private processProgram(program: ProgramNode, type: ProgramType, operation: OperationType): void {
        ...
        this.hoistProgram(program, type, operation);
    }
    ...
    private processBindProgram(program: ProgramNode): void {
        ...
        if (programs.length === 2) {
            ...
            this.processProgram(programs[0], ProgramType.BIND, OperationType.INCLUDE);
        }

        // Сохраним оригинальное выражение -- оно тоже нужно для internal
        this.processProgram(program, ProgramType.BIND, OperationType.INCLUDE);
    }
    ...
    private processIteratorProgram(program: ProgramNode): void {
        ...
        this.commitSyntheticPrograms(identifiers, ProgramType.ITERATOR);
        ...
        this.commitProgram(program, ProgramType.ITERATOR, OperationType.EXCLUDE);
    }
     ...
     * @private
     */
    private processIntersections(program: ProgramNode, type: ProgramType): boolean {
        ...
        this.commitSyntheticPrograms(identifiers, type);
     ...
     * @private
     */
    private hoistProgram(program: ProgramNode, type: ProgramType, operation: OperationType): void {
        if (operation !== OperationType.IGNORE) {
            this.commitProgram(program, type, operation);
        }

        // Необходимо на каждом контейнере проверять наличие пересечений по идентификаторам
        if (this.processIntersections(program, type)) {
            this.commitProgram(program, type, OperationType.EXCLUDE);

            return;
        }

        if (this.isHoistingAllowed()) {
            this.parent.hoistProgram(program, type, operation);
     ...
     * @private
     */
    private commitSyntheticPrograms(identifiers: string[], type: ProgramType): void {
        ...
        for (let index = 0; index < identifiers.length; ++index) {
            ...
            this.commitProgram(program, type, OperationType.INCLUDE);
        }
    }
     ...
     * @private
     */
    private commitProgram(program: ProgramNode, type: ProgramType, operation: OperationType): void {
        this.programs.push({
            program,
            type,
            operation
