...
const FORBIDDEN_IDENTIFIERS = ['...', '_options', '_container', '_children', '__setHTMLUnsafe', 'rk', 'debug'];
...
function optimizeInternal(node: InternalNode, storage: Set<string>): void {
    ...
    for (let index = 0; index < node.children.length; ++index) {
        ...
        const childStorage =
            child.type === InternalNodeType.BLOCK ? storage : new Set<string>(storage);
...
export class Container {
    ...
    private registerFloatProgram(program: ProgramNode): void {
        const identifiers = Walkers.collectIdentifiers(program);
...
export class InternalNode {
    ...
    private cleanStorage(identifiers: string[], options: ICollectorOptions): void {
        ...
        for (let index = 0; index < collection.length; ++index) {
            ...
            if (Walkers.containsIdentifiers(meta.node, identifiers)) {
                this.storage.remove(meta);

                const localIdentifiers = Walkers.collectIdentifiers(meta.node);
    ...
    private checkCleanConditional(identifiers: string[], options: ICollectorOptions): void {
        ...
        const hasFunctionCall = Walkers.containsFunctionCall(this.test.node);
        ...
        this.setType(
            this.type === InternalNodeType.ELSE_IF ? InternalNodeType.ELSE : InternalNodeType.BLOCK
        );
        this.test = null;
        if (this.next === null) {
            return;
    ...
    private dropAndAppend(identifiers: string[], allocator: IndexAllocator): void {
        ...
        const testIdentifiers = Walkers.collectIdentifiers(this.test.node);
