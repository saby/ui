...
export function parseComponentName(componentName: string): IPath {
    ...
    const physicalPath = physicalPathString.slice(physicalPathOffset).split(LOGICAL_PATH_SEPARATOR);
    if (prefix !== EMPTY_STRING) {
        physicalPath.unshift(prefix);
    }
    const logicalPath =
        logicalPathString !== EMPTY_STRING ? logicalPathString.split(LOGICAL_PATH_SEPARATOR) : [];
...
export function isLogicalPath(path: string): boolean {
    return (
        path.indexOf(LOGICAL_PATH_SEPARATOR) > -1 && path.indexOf(PHYSICAL_PATH_SEPARATOR) === -1
    );
}
