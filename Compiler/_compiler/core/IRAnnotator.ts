...
import type { ILostAttribute } from './LostAndFound';
...
// @ts-ignore FIXME: принять решение по проблеме позже
const toLostAndFoundMessage = (tag: string, lostAttributes: ILostAttribute[]) => (
    `На теге ${tag} обнаружены следующие атрибуты: ${lostAttributes.map(v => `"${v.name}"`).join(', ')}, ` +
    'которые терялись при компиляции шаблона и не попадали в верстку. ' +
    'Необходимо либо удалить совсем этот атрибут из шаблона (тогда ничего не поменяется), ' +
    'либо задать атрибут с префиксом attr:* (необходимо протестировать). ' +
    'В 23.5100 поддержано ошибочное поведение -- эти атрибуты пропускаются, но с предупреждением.' +
    'В 23.6100 ошибка будет исправлена и это предупреждение будет выключено.'
);
...
class IRAnnotator implements IAstVisitor<IContext, Ast> {
    ...
    visitComponent(node: ComponentNode, context: IContext): Ast {
        const lostAttributes = findLostAttributes(node);
        if (lostAttributes?.length > 0) {
            // FIXME: пока сообщение не выводим
            // this.errorHandler.warn(
            //     toLostAndFoundMessage(`<${node.wsPath.getFullPath()} ...>`, lostAttributes),
            //     { }
            // );
        }
    ...
    visitInlineTemplate(node: InlineTemplateNode, context: IContext): Ast {
        ...
        if (lostAttributes?.length > 0) {
            // FIXME: пока сообщение не выводим
            // this.errorHandler.warn(
            //     toLostAndFoundMessage(`<ws:partial template="${node.wsName}" ...>`, lostAttributes),
            //     { }
            // );
        }
    ...
    visitStaticPartial(node: StaticPartialNode, context: IContext): Ast {
        ...
        if (lostAttributes?.length > 0) {
            // FIXME: пока сообщение не выводим
            // this.errorHandler.warn(
            //     toLostAndFoundMessage(`<ws:partial template="${node.wsPath.getFullPath()}" ...>`, lostAttributes),
            //     { }
            // );
        }
    ...
    visitDynamicPartial(node: DynamicPartialNode, context: IContext): Ast {
        ...
        if (lostAttributes?.length > 0) {
            // FIXME: пока сообщение не выводим
            // this.errorHandler.warn(
            //     toLostAndFoundMessage(`<ws:partial template="{{ ${node.wsExpression.string} }}" ...>`, lostAttributes),
            //     { }
            // );
        }
