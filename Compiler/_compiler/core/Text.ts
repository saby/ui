    ...
    private processMarkedStatements(
        ...
        for (let index = 0; index < items.length; index++) {
            ...
            if (data.trim() === EMPTY_STRING && type !== RawTextType.TEXT) {
                ...
                if (options.allowedContent === TextContentFlags.EXPRESSION && type === RawTextType.EXPRESSION) {
                    throw new Error(`${whatExpected(options.allowedContent)}. В полученной строке отсутствует текст выражения`);
                }
                continue;
            }
            node = null;
