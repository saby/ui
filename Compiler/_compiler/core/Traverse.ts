...
import * as Nodes from '../html/Nodes';
...
import { ITextTranslator } from '../i18n/Translator';
import createValidator, { IValidator } from '../expressions/Validator';
import Config from '../Config';
import findLostAttributes from './LostAndFound';
...
export interface ITraverseConfig {
    ...
    /**
     * Should find lost attributes and remove them from AST.
     */
    removeLostAndFoundAttributes?: boolean;
}
...
function validateInlineTemplateName(name: string): void {
    const recommendation = 'Имя шаблона должно быть валидным именем переменной в JavaScript';
    ...
    if (!/^[_$a-zA-Z\u00A0-\uFFFF][_$a-zA-Z0-9\u00A0-\uFFFF]*$/gi.test(name)) {
        throw new Error(`невалидное имя inline-шаблона '${name}'. ${recommendation}`);
    }
}
...
function validateBoolean(children: Ast.TextNode[]): void {
    ...
    for (let index = 0; index < data.length; ++index) {
        const child = data[index];
        if (child instanceof Ast.TextDataNode) {
            if (child.wsContent !== 'true' && child.wsContent !== 'false') {
                throw new Error(
        ...
        if (child instanceof Ast.TranslationNode) {
            throw new Error('использование конструкции локализации недопустимо');
        }
    }
}
    ...
    for (let index = 0; index < data.length; ++index) {
        ...
        if (child instanceof Ast.TextDataNode) {
            if (isNaN(+child.wsContent)) {
                throw new Error(`получено нечисловое значение - "${child.wsContent}"`);
            }
        }
        if (child instanceof Ast.TranslationNode) {
            throw new Error('использование конструкции локализации недопустимо');
...
function validatePartialTemplate(
    ...
    const data = (option.wsValue as Ast.ValueNode).wsData;
    let value: ProgramNode | string = null;
    let current: ProgramNode | string;
    for (let index = 0; index < data.length; ++index) {
        current = null;
        if (data[index] instanceof Ast.ExpressionNode) {
            current = (data[index] as Ast.ExpressionNode).wsProgram;
        }
        if (data[index] instanceof Ast.TextDataNode) {
            current = (data[index] as Ast.TextDataNode).wsContent;
        }
...
function useIdentifierAlias(identifier: string): string {
    ...
    if (params.length > 2) {
        throw new Error(`указано более 1 алиаса для идентификатора - "${identifier}"`);
    }
    return params[1];
}
...
class Traverse implements ITraverse {
     ...
     * @private
     */
    private readonly removeLostAndFoundAttributes: boolean;
    ...
    constructor(config: ITraverseConfig) {
        ...
        this.removeLostAndFoundAttributes = config.removeLostAndFoundAttributes ?? true;
    }
    ...
    visitAll(nodes: Nodes.Node[], context: ITraverseContext): Ast.Ast[] {
        ...
        for (let index = 0; index < nodes.length; ++index) {
            ...
            const child = nodes[index].accept(this, childContext);
            if (child) {
                ...
                if (lastNode instanceof Ast.TextNode && child instanceof Ast.TextNode) {
                    ...
                    for (let index = 0; index < child.wsContent.length; ++index) {
                        child.wsContent[index].setKey(shift + index);
                    }
                    lastNode.wsContent = lastNode.wsContent.concat(child.wsContent);
                    continue;
                }
    ...
    visitText(node: Nodes.Text, context: ITraverseContext): Ast.TextNode {
        ...
        switch (context.state) {
            ...
            default:
                this.errorHandler.error(
                    `Обнаружен непредусмотренный текст "${node.data}": ${whatExpected(
                        context.state
        ...
        switch (node.name) {
            ...
            default:
                ...
                if (Resolvers.isOption(node.name)) {
                    this.errorHandler.error(`Обнаружена неизвестная директива "${node.name}"`, {
                        fileName: context.fileName,
                        position: node.position,
                    });
                    return null;
    ...
    private processConditionalAttribute(
        ...
        try {
            ...
            ast.wsTest = (value[0] as Ast.ExpressionNode).wsProgram;
            return ast;
        } catch (error) {
            this.errorHandler.critical(
    ...
    private processForAttribute(
        ...
        try {
            ...
            const cycleData = (textValue[0] as Ast.TextDataNode).wsContent;
    ...
    private processContentTagWithoutUnpacking(
        ...
        if (node.name === 'ws:partial') {
            return this.checkForLostAttributes(
                this.processPartial(node, context),
                node,
                context
            );
        }
    ...
    /**
     * Check node for lost attributes.
     * @private
     * @param ast {TContent} Node type of TContent.
     * @param node {Tag} Processing html tag node.
     * @param context {ITraverseContext} Processing context.
     */
    private checkForLostAttributes(
        ast: Ast.TContent,
        node: Nodes.Tag,
        context: ITraverseContext
    ): Ast.TContent {
        if (!this.removeLostAndFoundAttributes) {
            return ast;
        }

        // TODO: выводим предупреждение, чтобы люди могли это исправить.
        //  Спустя несколько версий убираем вывод сообщения и воспроизведение ошибки.
        if (ast instanceof Ast.BaseWasabyElement) {
            const lostAttributes = findLostAttributes(ast);
            if (!lostAttributes) {
                return ast;
            }
            const message = (
                lostAttributes.length === 1
                    ? (
                        `На теге ${node.name} обнаружен атрибут ${lostAttributes.map(v => `"${v.name}"`).join(', ')}, ` +
                        'который ранее и на данный момент не учитывается при компиляции шаблонов wml и tmpl и, соответственно, не попадает в верстку. ' +
                        'Необходимо либо совсем удалить данный атрибут из шаблона (в таком случае ничего не поменяется), ' +
                        'либо задать атрибут с префиксом attr:* (в таком случае атрибут начнет работать, поведение изменится, необходимо тестирование)'
                    )
                    : (
                        `На теге ${node.name} обнаружены атрибуты ${lostAttributes.map(v => `"${v.name}"`).join(', ')}, ` +
                        'которые ранее и на данный момент не учитываются при компиляции шаблонов wml и tmpl и, соответственно, не попадают в верстку. ' +
                        'Необходимо либо совсем удалить перечисленные атрибуты из шаблона (в таком случае ничего не поменяется), ' +
                        'либо задать перечисленные атрибуты с префиксом attr:* (в таком случае атрибуты начнут работать, поведение изменится, необходимо тестирование)'
                    )
            );
    ...
    private processProperty(
        ...
        if (canBeTypeCasted(node)) {
            ...
            // FIXME: В wml это не могло работать
        ...
        if (hasDataTypeOnly) {
            this.warnUnexpectedAttributes(node.attributes, context, node.name);
            if (content.length === 1) {
                return new Ast.OptionNode(name, content[0] as Ast.TData);
            }
            const array = new Ast.ArrayNode(content as Ast.TData[]);
        ...
        for (let index = 0; index < content.length; ++index) {
            ...
            if (
                !(property instanceof Ast.OptionNode || property instanceof Ast.ContentOptionNode)
            ) {
                this.errorHandler.critical(`Опция "${node.name}" содержит некорректные данные`, {
                    fileName: context.fileName,
                    position: node.position,
                });
                continue;
    ...
    private processText(node: Nodes.Text, context: ITraverseContext): Ast.TextNode {
        try {
            ...
            this.errorHandler.critical(`Ошибка обработки текста "${node.data}": ${error.message}`, {
                fileName: context.fileName,
                position: node.position,
            });
            return null;
    ...
    private castPropertyContentToArray(
        ...
        try {
            const name = Resolvers.resolveOption(node.name);
            const elements = new Ast.ArrayNode(this.processArrayContent(node, context, attributes));
    ...
    private castPropertyContentToNumber(
        ...
        try {
            const name = Resolvers.resolveOption(node.name);
            const value = new Ast.NumberNode(this.processNumberContent(node, context, attributes));
            value.setFlag(Ast.Flags.OBVIOUSLY_TYPE_CASTED);
            return new Ast.OptionNode(name, value);
        } catch (error) {
            this.errorHandler.critical(
    ...
    private castPropertyContentToObject(
        ...
        try {
            const name = Resolvers.resolveOption(node.name);
            const value = new Ast.ObjectNode(this.processObjectContent(node, context, attributes));
            value.setFlag(Ast.Flags.OBVIOUSLY_TYPE_CASTED);
            return new Ast.OptionNode(name, value);
        } catch (error) {
            this.errorHandler.critical(
    ...
    private castPropertyContentToString(
        ...
        try {
            const name = Resolvers.resolveOption(node.name);
            const value = new Ast.StringNode(this.processStringContent(node, context, attributes));
            value.setFlag(Ast.Flags.OBVIOUSLY_TYPE_CASTED);
            return new Ast.OptionNode(name, value);
        } catch (error) {
            this.errorHandler.critical(
    ...
    private castPropertyContentToValue(
        ...
        try {
            const name = Resolvers.resolveOption(node.name);
            const value = new Ast.ValueNode(this.processValueContent(node, context, attributes));
            value.setFlag(Ast.Flags.OBVIOUSLY_TYPE_CASTED);
            return new Ast.OptionNode(name, value);
        } catch (error) {
            this.errorHandler.critical(
    ...
    private processElement(node: Nodes.Tag, context: ITraverseContext): Ast.ElementNode {
        ...
        const content = this.visitAll(node.children, childrenContext) as Ast.TContent[];
    ...
    private testDoubleTypeDefinition(
        ...
        if (context.explicitDataType === null || !isDirectiveProvided) {
            this.errorHandler.critical(
                `Обнаружена непредусмотренная директива "${node.name}": ${whatExpected(
                    context.state
                )}`,
                {
                    fileName: context.fileName,
    ...
    private processDoubleTypeDefinition(node: Nodes.Tag, context: ITraverseContext): Ast.TextNode {
        ...
        switch (node.name) {
            ...
            case 'ws:Boolean':
                if (
                    !this.testDoubleTypeDefinition(node, context, TraverseState.BOOLEAN_DATA_TYPE)
                ) {
                    break;
                }
            ...
            default:
                this.errorHandler.error(
                    `Обнаружен непредусмотренный тег "${node.name}": ${whatExpected(
                        context.state
                    )}. Тег будет проигнорирован, его необходимо убрать`,
                    {
                        fileName: context.fileName,
    ...
    private processArray(node: Nodes.Tag, context: ITraverseContext): Ast.ArrayNode {
        try {
            return new Ast.ArrayNode(this.processArrayContent(node, context, node.attributes));
        } catch (error) {
            this.errorHandler.critical(
    ...
    private processArrayContent(
        ...
        return this.visitAll(node.children, childrenContext) as Ast.TData[];
    }
    ...
    private processBoolean(node: Nodes.Tag, context: ITraverseContext): Ast.BooleanNode {
        try {
            return new Ast.BooleanNode(this.processBooleanContent(node, context, node.attributes));
        } catch (error) {
            this.errorHandler.critical(
    ...
    private processBooleanContent(
        ...
        this.warnUnexpectedAttributes(attributes, context, node.name);
        const children = this.visitAll(node.children, childrenContext) as Ast.TextNode[];
    ...
    private processFunctionContent(
        ...
        const textNodes = this.visitAll(node.children, childrenContext) as Ast.TextNode[];
    ...
    private processNumber(node: Nodes.Tag, context: ITraverseContext): Ast.NumberNode {
        try {
            return new Ast.NumberNode(this.processNumberContent(node, context, node.attributes));
        } catch (error) {
            this.errorHandler.critical(
    ...
    private processObject(node: Nodes.Tag, context: ITraverseContext): Ast.ObjectNode | Ast.TData {
        try {
            ...
            return new Ast.ObjectNode(this.processObjectContent(node, context, node.attributes));
        } catch (error) {
            this.errorHandler.critical(
    ...
    private processObjectContent(
        ...
        for (let index = 0; index < processedChildren.length; ++index) {
            ...
            if (!(child instanceof Ast.OptionNode || child instanceof Ast.ContentOptionNode)) {
                this.errorHandler.fatal(`Тег "${node.name}" содержит некорректные данные`, {
                    fileName: context.fileName,
                    position: node.position,
                });
                continue;
    ...
    private processString(node: Nodes.Tag, context: ITraverseContext): Ast.StringNode {
        try {
            return new Ast.StringNode(this.processStringContent(node, context, node.attributes));
        } catch (error) {
            this.errorHandler.critical(
    ...
    private processValue(node: Nodes.Tag, context: ITraverseContext): Ast.ValueNode {
        try {
            return new Ast.ValueNode(this.processValueContent(node, context, node.attributes));
        } catch (error) {
            this.errorHandler.critical(
    ...
    private processIf(node: Nodes.Tag, context: ITraverseContext): Ast.IfNode {
        ...
        const consequent = this.visitAll(node.children, childrenContext) as Ast.TContent[];
        ...
        try {
            ...
            this.errorHandler.critical(`Ошибка разбора директивы "ws:if": ${error.message}`, {
                fileName: context.fileName,
                position: node.position,
            });
    ...
    private processElse(node: Nodes.Tag, context: ITraverseContext): Ast.ElseNode {
        ...
        try {
            ...
            this.errorHandler.critical(`Ошибка разбора директивы "ws:else": ${error.message}`, {
                fileName: context.fileName,
                position: node.position,
            });
            ast.setFlag(Ast.Flags.BROKEN);
    ...
    private processFor(node: Nodes.Tag, context: ITraverseContext): Ast.ForNode | Ast.ForeachNode {
        try {
            ...
            this.errorHandler.critical(`Ошибка разбора директивы "ws:for": ${error.message}`, {
                fileName: context.fileName,
                position: node.position,
            });
            return null;
    ...
    private processTemplate(node: Nodes.Tag, context: ITraverseContext): Ast.TemplateNode {
        try {
            ...
            this.errorHandler.critical(`Ошибка разбора директивы "ws:template": ${error.message}`, {
                fileName: context.fileName,
                position: node.position,
            });
            return null;
    ...
    private parseForParameters(data: string): {
        ...
        try {
            init = initExpression ? this.expressionParser.parse(initExpression) : null;
        } catch (error) {
            throw new Error(`выражение инициализации цикла "${initExpression}" задано некорректно`);
        }
        try {
            test = this.expressionParser.parse(testExpression);
        } catch (error) {
            throw new Error(`выражение условия цикла "${testExpression}" задано некорректно`);
        }
        try {
            update = updateExpression ? this.expressionParser.parse(updateExpression) : null;
        } catch (error) {
            throw new Error(`выражение обновления цикла "${updateExpression}" задано некорректно`);
        }
        return {
            init,
            test,
    ...
    private parseForeachParameters(data: string): {
        ...
        try {
            ...
            throw new Error(`итератор цикла "${iteratorString}" задан некорректно`);
        }
        try {
            const indexExpression = indexString !== null ? useIdentifierAlias(indexString) : null;
            index = indexExpression ? this.expressionParser.parse(indexExpression) : null;
        } catch (error) {
        ...
        try {
            ...
            throw new Error(`коллекция цикла "${collectionExpression}" задана некорректно`);
        }
        return {
            iterator,
            index,
    ...
    private createComponentOnly(node: Nodes.Tag, context: ITraverseContext): Ast.ComponentNode {
        ...
        const componentDescription = this.textTranslator.getComponentDescription(
            path.getFullPath()
        );
        const attributes = this.attributeProcessor.process(
            node.attributes,
    ...
    private processPartial(
        ...
        try {
            ...
            this.errorHandler.critical(`Ошибка разбора директивы "ws:partial": ${error.message}`, {
                fileName: context.fileName,
                position: node.position,
            });
        }
    ...
    private getComponentOrPartialOptions(
        ...
        if (isFirstChildContent(content)) {
            const contentOption = new Ast.ContentOptionNode('content', content as Ast.TContent[]);
    ...
    private getProgramNodeFromAttribute(
        ...
        const expressionNode = (
            this.getAttributeValue(node, attribute, TextContentFlags.EXPRESSION, context) as Ast.ExpressionNode
        );
    ...
    private getTextFromAttribute(
        ...
        const textDataNode = (
            this.getAttributeValue(node, attribute, TextContentFlags.TEXT, context) as Ast.TextDataNode
        );
