define('Compiler/_compiler/modules/utils/parse', [
   ...
   function checkRestrictedAttributes(isRestricted, restrictedAttributes, curAttribute) {
      ...
      return isRestricted ? restrictedAttributes.indexOf(curAttribute) === -1 : true;
   }
   ...
   function parseAttributesForData(attributes, data, propertyName, restricted) {
      ...
      var restrictedAttributes = [root, attribs, 'class', 'data-access', '_wstemplatename'];
      ...
      if (attrs !== undefined) {
         ...
         for (attr in attrs) {
            if (
               ...
               checkRestrictedAttributes(restricted, restrictedAttributes, attr) &&
               attrs[attr]
            ) {
   ...
   function processAttributes(attribs, data, decor, isControl, tag) {
      ...
      if (utils.checkProp(attribs, 'attributes')) {
         attrs = processDataSequence.call(this, attribs.attributes, data, undefined, {
            composite: true
         });
      ...
      for (var attr in attribs) {
         ...
         if (eventExpressions.isEvent(attr)) {
            try {
         ...
         if (SPECIAL_ATTRIBUTES_COLLECTION.indexOf(attr) > -1) {
      ...
      if (typeof attrs === 'string') {
         result.attributes = FSC.wrapAroundExec(
            TClosure.genProcessMergeAttributes(attrs, FSC.getStr(result.attributes)),
            true
         );
      }
      if (!needEventMeta) {
