define('Compiler/_compiler/modules/utils/loader', [
   'require',
   'Types/deferred',
   'Compiler/_compiler/utils/ErrorHandler',
   'Compiler/_compiler/modules/utils/common',
   'Compiler/_compiler/modules/utils/names'
], function straightFromFileLoader(require, deferredLib, ErrorHandlerLib, common, names) {
   'use strict';
   ...
   function requireWsControlFile(moduleName, fromBuilderTmpl) {
      ...
      if (fromBuilderTmpl) {
         ...
         require([moduleName], function (requiredModule) {
            if (requiredModule || requiredModule === null) {
               deferred.callback(
                  createControlNode(moduleName, moduleName, requiredModule === null)
               );
            } else {
               deferred.errback(new Error('Не удалось загрузить файл "' + moduleName + '"'));
            }
      ...
      if (fromBuilderTmpl || require.defined(libraryPath.library)) {
         ...
         require([libraryPath.library], function (requiredModule) {
            if (requiredModule || requiredModule === null) {
               ...
               deferred.errback(
                  new Error('Не удалось загрузить файл "' + libraryPath.library + '"')
               );
            }
         }, function (error) {
            deferred.errback(error);
