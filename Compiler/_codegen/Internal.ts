import { InternalNode, InternalNodeType } from '../_core/internal/Container';
import { IProgramMeta } from '../_core/internal/Storage';
...
export function generate(
    ...
    if (node.index === -1) {
        throw new Error(
            'Произведена попытка генерации Internal-функции от скрытого узла'
        );
    }
    ...
    try {
        ...
        throw new Error(
            `Тело функции "${functionName}" невалидно: ${error.message}`
        );
    }
}
...
function getCurrentConditionalIndex(node: InternalNode): number {
    ...
    if (node.prev === null) {
        throw new Error(
            `Узел типа IF недостижим. Текущий internal узел - ${node.index}`
        );
    }
...
function buildWithConditions(node: InternalNode, options: IOptions): string {
    const body =
        buildPrograms(node.storage.getMeta(), options) +
        buildAll(node.children, options);
    if (node.type === InternalNodeType.BLOCK) {
        return body;
    }
    ...
    throw new Error(
        `Получен неизвестный internal-узел с номером ${node.index}`
    );
}
...
function buildWithSequencedConditions(
    node: InternalNode,
    options: IOptions
): string {
    const body =
