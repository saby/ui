...
class PatchVisitor implements Ast.IAstVisitor {
    ...
    visitDoctype(node: Ast.DoctypeNode, context: INavigationContext): any {
        ...
        // @ts-ignore
        node.isRootTag = node.wsIsRootNode;
        return node;
    }
    ...
    visitText(node: Ast.TextNode, context: INavigationContext): any {
        ...
        // @ts-ignore
        node.data =
            node.wsContent.length === 1 &&
            node.wsContent[0] instanceof Ast.TextDataNode
                ? content[0]
                : content;
        // @ts-ignore
        node.type = 'text';
    ...
    visitTranslation(
        ...
        node.localized = true;
        // @ts-ignore
        node.name = node.wsContext
            ? `${node.wsContext} @@ ${node.wsText}`
            : node.wsText;
        // @ts-ignore
        node.type = 'var';
        // @ts-ignore
    ...
    visitAttribute(node: Ast.AttributeNode, context: INavigationContext): any {
        ...
        // @ts-ignore
        node.data =
            node.wsValue.length === 1 &&
            node.wsValue[0] instanceof Ast.TextDataNode
                ? attributeValue[0]
                : attributeValue;
        // @ts-ignore
        node.key = undefined;
    ...
    visitFunction(node: Ast.FunctionNode, context: INavigationContext): any {
        ...
        for (const optionName in node.wsOptions) {
            ...
            const optionValue = (<Ast.ValueNode>option.wsValue).accept(
                this,
                context
            );
        ...
        node.children = [
            {
                data: this.visitAll(node.wsFunctionExpression, context),
                key: context.currentKey + '0_',
                type: 'text',
            },
        ];
    ...
    private collectObjectAttributeProperties(
        ...
        for (const optionName in node.wsProperties) {
            ...
            const propertyValue = (<Ast.ValueNode>property.wsValue)
                .wsData;
            const isTextOnly =
    ...
    private processAttributesAndEvents(
        ...
        for (const attributeName in node.wsAttributes) {
            ...
            const name = !attribute.wsHasAttributePrefix
                ? cleanName
                : attributeName;
