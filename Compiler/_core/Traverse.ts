...
function validateBoolean(children: Ast.TextNode[]): void {
    ...
    for (let index = 0; index < data.length; ++index) {
        ...
        if (child instanceof Ast.TextDataNode) {
            if (
                child.__$ws_content !== 'true' &&
                child.__$ws_content !== 'false'
            ) {
                throw new Error(
                    `ожидалось одно из значений - true/false, получено - "${child.__$ws_content}"`
                );
            }
        }
    ...
    for (let index = 0; index < data.length; ++index) {
        ...
        if (child instanceof Ast.TextDataNode) {
            if (isNaN(+child.__$ws_content)) {
                throw new Error(
                    `получено нечисловое значение - "${child.__$ws_content}"`
                );
            }
        }
        if (child instanceof Ast.TranslationNode) {
        ...
        for (let index = 0; index < nodes.length; ++index) {
            ...
            if (child) {
                ...
                if (
                    ...
                    for (
                        let index = 0;
                        index < child.__$ws_content.length;
                        ++index
                    ) {
                        child.__$ws_content[index].setKey(shift + index);
                    }
                    lastNode.__$ws_content = lastNode.__$ws_content.concat(
                        child.__$ws_content
                    );
                    continue;
                }
