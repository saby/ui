...
import { canBeTranslated, splitLocalizationText } from '../_i18n/Helpers';
...
import { TranslationType } from '../_i18n/Dictionary';
...
function flushFragment(
    ...
    if (
        lastNode &&
        lastNode.type === RawTextType.TEXT &&
...
function parse(text: string): IRawTextItem[] {
    ...
    for (let cursor: number = 0; cursor < text.length; ++cursor) {
        ...
        if (isOpeningExpression || isOpeningTranslation) {
            ...
            textType = isOpeningExpression
                ? RawTextType.EXPRESSION
                : RawTextType.TRANSLATION;
            start = cursor + 2;
            ++cursor;
            continue;
        }
        ...
        const isClosingExpression =
            textType === RawTextType.EXPRESSION &&
            char === '}' &&
            nextChar === '}';
        const isClosingTranslation =
            textType === RawTextType.TRANSLATION &&
            char === ']' &&
            nextChar === '}';
...
function createTranslationNode(
    ...
    if ((options.allowedContent & TextContentFlags.TRANSLATION) === 0) {
        throw new Error(
            ...
            )}. Обнаружена конструкция локализации "${data}"`
        );
    }
    const { text, context } = splitLocalizationText(data);
    options.translationsRegistrar.registerTranslation(
        type,
        options.fileName,
        text,
        context
    ...
    private processMarkedStatements(
        ...
        for (let index = 0; index < items.length; index++) {
            ...
            const translationType =
                type === RawTextType.TRANSLATION ? 'manual' : 'auto';
    ...
    private createExpressionNode(
        ...
        if ((options.allowedContent & TextContentFlags.EXPRESSION) === 0) {
            throw new Error(
                `${whatExpected(
                    options.allowedContent
                )}. Обнаружено Mustache-выражение "${data}"`
            );
        }
        try {
