...
function getElementName(element: Ast.BaseHtmlElement): string | null {
    if (element.wsAttributes.hasOwnProperty('attr:name')) {
        return getStringValueFromText(
            element.wsAttributes['attr:name'].wsValue
        );
    }
    if (element.wsAttributes.hasOwnProperty('name')) {
        return getStringValueFromText(
            element.wsAttributes.name.wsValue
        );
    }
    return null;
}
...
function getComponentName(component: Ast.BaseWasabyElement): string | null {
    ...
    if (component.wsOptions.hasOwnProperty('attr:name')) {
        return getStringValueFromData(
            component.wsOptions['attr:name'].wsValue
        );
    }
...
function setRootNodeFlags(nodes: Ast.Ast[]): void {
    nodes.forEach((node) => {
        if (node instanceof Ast.IfNode) {
            setRootNodeFlags(node.wsConsequent);
            return;
        }
        ...
        if (node instanceof Ast.ForNode) {
            setRootNodeFlags(node.wsContent);
            return;
        }
...
function setContainerNodeFlags(nodes: Ast.Ast[]): void {
    for (const node of nodes) {
        if (node instanceof Ast.ElementNode) {
            ...
            setContainerNodeFlags(node.wsConsequent);
...
class InternalVisitor implements Ast.IAstVisitor {
    ...
    process(nodes: Ast.Ast[], scope: Scope): IResultTree {
        ...
        for (let index = 0; index < nodes.length; ++index) {
            ...
            if (!nodes[index].wsInternalTree) {
                nodes[index].wsInternalTree =
                    nodes[index].wsContainer.getInternalStructure();
    ...
    visitAttribute(node: Ast.AttributeNode, context: IContext): void {
        ...
        visitAll(node.wsValue, this, childContext);
        this.stack.pop();
    }
    ...
    visitContentOption(node: Ast.ContentOptionNode, context: IContext): void {
        ...
        visitAll(node.wsContent, this, childContext);
        setRootNodeFlags(node.wsContent);
        setPassRefFlag(node.wsContent);
        this.stack.pop();
        ...
        node.wsInternal = wrapInternalExpressions(
            node.wsInternalTree.flatten()
        );
    }
    ...
    visitElement(node: Ast.ElementNode, context: IContext): void {
        ...
        visitAll(node.wsContent, this, context);
        visitAllProperties(node.wsAttributes, this, context);
        visitAllProperties(node.wsEvents, this, context);
        this.stack.pop();
    }
    ...
    visitIf(node: Ast.IfNode, context: IContext): void {
        ...
        this.stack.push(AbstractNodeType.DIRECTIVE);
        visitAll(node.wsConsequent, this, childContext);
        this.stack.pop();
        node.wsContainer = container;
    ...
    visitArray(node: Ast.ArrayNode, context: IContext): void {
        ...
        visitAll(node.wsElements, this, context);
        this.stack.pop();
    }
    ...
    visitBoolean(node: Ast.BooleanNode, context: IContext): void {
        this.stack.push(AbstractNodeType.DATA_TYPE_DIRECTIVE);
        visitAll(node.wsData, this, context);
        this.stack.pop();
    }
    ...
    visitFunction(node: Ast.FunctionNode, context: IContext): void {
        this.stack.push(AbstractNodeType.DATA_TYPE_DIRECTIVE);
        visitAllProperties(node.wsOptions, this, context);
        visitAll(node.wsFunctionExpression, this, context);
        this.stack.pop();
    }
    ...
    visitObject(node: Ast.ObjectNode, context: IContext): void {
        this.stack.push(AbstractNodeType.DATA_TYPE_DIRECTIVE);
        visitAllProperties(node.wsProperties, this, context);
        this.stack.pop();
    }
    ...
    private processComponent(
        ...
        visitAllProperties(node.wsAttributes, this, childContext);
        visitAllProperties(node.wsEvents, this, childContext);
        visitAllProperties(node.wsOptions, this, childContext);
        visitAllProperties(node.wsContents, this, childContext);
        this.stack.pop();
        node.wsContainer = container;
        return container;
    }
